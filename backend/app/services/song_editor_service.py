...
def _number(value: Any, name: str) -> float:
    ...
    if number != number or number in (float("inf"), float("-inf")): raise ValueError(f"{name} must be finite")
    return number
...
def _int_or_none(value: Any) -> int | None:
    ...
    except (TypeError, ValueError) as exc: raise ValueError("Invalid index") from exc


_safe_int = int_or

def _indices(value: Any, fallback: Any = None) -> list[int]:
    raw = value if isinstance(value, list) else ([fallback] if fallback is not None else []); result: list[int] = []
    for item in raw:
        parsed = _int_or_none(item)
        if parsed is not None and parsed not in result: result.append(parsed)
    return sorted(result)


def _normalize_note(raw: dict[str, Any], duration: float) -> dict[str, Any]:
    start, end, midi = _number(raw.get('start'), 'note.start'), _number(raw.get('end'), 'note.end'), int(round(_number(raw.get('midi_note', raw.get('midi')), 'note.midi')))
    if start < 0 or end <= start: raise ValueError("Each note must have start >= 0 and end > start")
    if duration > 0 and end > duration + 0.05: raise ValueError("A note lies outside the song duration")
    if not 0 <= midi <= 127: raise ValueError("MIDI note must be between 0 and 127")
    velocity = int(round(_number(raw.get("velocity") or 96, "note.velocity"))); velocity = max(1, min(127, velocity))
    return {
        "start": round(start, 6),
        "end": round(end, 6),
        "midi_note": midi,
...
def _project_notes_by_syllable(notes: list[JsonObject], syllables: list[JsonObject]) -> dict[int, list[JsonObject]]:
    ...
    for note in notes:
        ...
        if not isinstance(indices, list) or not indices:
            idx = note.get("syllable_index"); indices = [] if idx is None else [idx]
        normalized = sorted({_safe_int(idx, -1) for idx in indices if _safe_int(idx, -1) >= 0})
        if not normalized: continue
        start, end = float(note["start"]), float(note["end"]); cursor = start; span = (end - start) / len(normalized)
        for pos, idx in enumerate(normalized):
            projected = dict(note); projected_end = end if pos == len(normalized) - 1 else start + span * (pos + 1); projected["start"] = round(cursor, 6); projected["end"] = round(max(cursor, projected_end), 6)
            projected["syllable_index"] = idx; projected["syllable_indices"] = [idx]; note_by_syllable[idx].append(projected); cursor = projected_end
    return note_by_syllable

def _refresh_lines(song_map: JsonObject, notes: list[JsonObject]) -> None:
    raw_syllables = [item for item in song_map.get("syllables") or [] if isinstance(item, dict)]; note_by_syllable = _project_notes_by_syllable(notes, raw_syllables)
    ...
    for word_payload in words: word_payload["syllables"] = []

    for sid, syllable in syllable_lookup.items():
        linked = sorted(note_by_syllable.get(sid, []), key=lambda n: (n["start"], n["end"])); syllable["display_notes"] = [dict(n) for n in linked]; syllable["timing_source"] = "syllable_alignment"; word_index = _safe_int(syllable.get("word_index"), -1)
        if (linked_word := word_lookup.get(word_index)) is not None: linked_word.setdefault("syllables", []).append(syllable)

    for word_payload in words: word_payload["timing_source"] = "word_alignment"

    song_map["syllables"] = [syllable_lookup[key] for key in sorted(syllable_lookup)]; song_map["words"] = words

    updated_lines: list[JsonObject] = []
    for line in song_map.get("lines") or []:
        if not isinstance(line, dict): continue
        clone = dict(line); line_words = []
        ...
        if line_words:
            clone["start"] = min(float(w.get("start") or 0.0) for w in line_words); clone["end"] = max(float(w.get("end") or 0.0) for w in line_words)
...
def _refresh_generated_note_associations(song_map: JsonObject) -> None:
    ...
    for field in ("notes", "display_notes"):
        for note in song_map.get(field) or []:
            ...
            start = float(note.get("start") or 0.0); end = float(note.get("end") or start)
            overlaps = [
                (
                    syllable,
            ...
            for syllable, overlap in overlaps: overlap_by_word[_safe_int(syllable.get("word_index"), -1)] += overlap
...
def _write_editor_generation(staging: Path, song_map: JsonObject, notes: list[JsonObject], manifest: Any) -> None:
    write_json(staging / "songMap.json", song_map); write_json(staging / "reference.json", {"notes": notes})
    midi_notes = [
    ...
    if midi_notes:
        midi_path = staging / "game.mid"; write_midi(midi_path, midi_notes, _words(song_map), _syllables(song_map), float(song_map.get("bpm") or 120.0), False)
...
def _publish_editor_generation(output_dir: Path, staging: Path, *, has_midi: bool, has_manifest: bool) -> None:
    names, delete_names, rollback = ['songMap.json', 'reference.json'] + (['game.mid'] if has_midi else []) + (['manifest.json'] if has_manifest else []), [] if has_midi else ['game.mid'], Path(tempfile.mkdtemp(prefix='editor-rollback-', dir=output_dir)); existed: dict[str, bool] = {}
    try:
        for name in names + delete_names:
            target = output_dir / name; existed[name] = target.exists()
            if target.exists(): shutil.copy2(target, rollback / name)
        for name in names: os.replace(staging / name, output_dir / name)
        for name in delete_names: (output_dir / name).unlink(missing_ok=True)
    except Exception:
        for name in names + delete_names:
        ...
        raise
    finally: shutil.rmtree(rollback, ignore_errors=True)
...
def save_editor(output_dir: Path, raw_notes: list[JsonObject]) -> JsonObject:
    output_dir = Path(output_dir); song_map, _ = load_editor(output_dir); backup = output_dir / "songMap.ai.json"
    if not backup.exists(): shutil.copy2(output_dir / "songMap.json", backup)
    duration = float(song_map.get("duration") or 0.0); notes = sorted((_normalize_note(item, duration) for item in raw_notes), key=lambda n: (n["start"], n["end"], n["midi_note"])); song_map["notes"] = notes; song_map["display_notes"] = [dict(note, display_source="editor") for note in notes]
    song_map["editor"] = {"edited": True, "source": "manual"}; stats = dict(song_map.get("display_stats") or {}); stats.update(game_note_count=len(notes), display_note_count=len(notes)); song_map["display_stats"] = stats
    _refresh_lines(song_map, notes); manifest: Any = read_json(output_dir / "manifest.json", default={})
...
def reset_editor(output_dir: Path) -> JsonObject:
    output_dir = Path(output_dir); backup = output_dir / "songMap.ai.json"
