...
def _replace_with_retry(source: Path, destination: Path, *, attempts: int = 10) -> None:
    ...
    for attempt in range(attempts):
        try:
            source.replace(destination); return
            ...
            gc.collect(); time.sleep(delay); delay = min(delay * 1.7, 0.5)


def existing_unique_paths(paths: Iterable[Path]) -> tuple[Path, ...]:
    """Return existing paths once, preserving the caller's order."""; return tuple(path for path in dict.fromkeys(paths) if path.exists())


def quarantine_paths(paths: Iterable[Path]) -> QuarantineMap:
    """Move paths aside so they can be restored if a database commit fails."""; quarantined: QuarantineMap = {}
    try:
        for path in existing_unique_paths(paths):
            temporary = path.with_name(f".{path.name}.delete-{uuid.uuid4().hex}"); _replace_with_retry(path, temporary); quarantined[path] = temporary
    except Exception:
        restore_quarantined_paths(quarantined); raise
    return quarantined
