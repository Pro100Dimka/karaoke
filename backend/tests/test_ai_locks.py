...
def test_pid_alive_posix(monkeypatch, failure, expected):
    ...
    monkeypatch.setattr(locks.os, "getpid", lambda: 1)
    ...
    assert not locks.FileLock._pid_alive(0)
    assert locks.FileLock._pid_alive(1)
    assert locks.FileLock._pid_alive(2) is expected


def kernel32(**values):
    names = ["OpenProcess", "CloseHandle", "GetExitCodeProcess", "GetLastError", "GetProcessTimes"]
    result = SimpleNamespace()
    for name in names:
        setattr(result, name, Mock(**values.get(name, {})))
    return result
...
def test_owner_identity_rules(monkeypatch):
    ...
    assert not locks.FileLock._owner_is_alive({"pid": "bad"})
    assert not locks.FileLock._owner_is_alive({"pid": 4})
    ...
    assert locks.FileLock._owner_is_alive({"pid": 4, "process_birth": 10.5})
    assert not locks.FileLock._owner_is_alive({"pid": 4, "process_birth": "bad"})
...
def test_file_lock_context_and_timeout(tmp_path):
    ...
    with pytest.raises(TimeoutError, match="Timed out"):
        locks.FileLock(path, timeout_sec=0).__enter__()
    with pytest.raises(TimeoutError):
        locks.FileLock(path, timeout_sec=0.003, poll_sec=0.001).__enter__()
...
def test_remove_stale_tolerates_filesystem_races(monkeypatch, tmp_path):
    ...
    assert not lock._remove_if_stale()
    monkeypatch.setattr(lock, "_read_owner", lambda: {"pid": 1})
    monkeypatch.setattr(lock, "_owner_is_alive", lambda _: False)
    assert not lock._remove_if_stale()
...
def test_thread_file_lock_timeout_and_file_failure(monkeypatch, tmp_path):
    ...
    held = locks.threading.Lock()
    held.acquire()
    locks.ThreadFileLock._entries[key] = (held, 0)
    try:
        with pytest.raises(TimeoutError, match="in-process"):
            locks.ThreadFileLock(path, timeout_sec=0).__enter__()
    finally:
        held.release()
    ...
    with pytest.raises(RuntimeError, match="file lock"):
        locks.ThreadFileLock(path).__enter__()
