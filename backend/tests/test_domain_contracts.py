...
def test_song_update_rejects_invalid_semantics(payload):
    with pytest.raises(ValidationError):
        schemas.SongUpdate(**payload)


def test_song_update_accepts_ordered_or_partial_note_ranges():
    assert schemas.SongUpdate(note_range_min=48, note_range_max=72).note_range_max == 72
    assert schemas.SongUpdate(note_range_min=48).note_range_min == 48
...
def test_entity_dependencies_return_found_or_stable_404(
    ...
    with pytest.raises(HTTPException) as missing:
        dependency(**{identifier_name: "missing", "db": database})
    assert missing.value.status_code == 404
    assert detail_fragment in missing.value.detail
