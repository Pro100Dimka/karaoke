...
def api_client(monkeypatch, tmp_path):
    ...
    patch_attrs(monkeypatch, app_settings_service, SETTINGS_FILE=tmp_path / 'settings.json', UI_PREFERENCES_FILE=tmp_path / 'ui-preferences.json', INSTALL_PREFERENCES_FILE=tmp_path / 'install-preferences.json')
...
def test_health_about_and_settings_contract(api_client):
    ...
    assert (about.status_code == 200) and (about.json()['name'] == 'A&D Voice')

    settings = client.get("/settings")
    assert (settings.status_code == 200) and (settings.json()['language'] == 'uk')
    updated = client.patch("/settings", json={"language": "en", "theme": "violet"})
    assert (updated.status_code == 200) and (updated.json() | {'language': 'en', 'theme': 'violet'} == updated.json()) and (client.patch('/settings', json={'thread_count': 0}).status_code == 422) and (client.patch('/settings', json={'compute_mode': 'quantum'}).status_code == 422)
...
def test_ui_preferences_validate_namespace_and_persist(api_client):
    ...
    assert (response.status_code == 200) and (response.json() == {'speed': 1.25}) and (client.get('/preferences').json() == {'karaoke': {'speed': 1.25}}) and (client.patch('/preferences/unknown', json={'x': 1}).status_code == 422) and (client.patch('/preferences/radio', json={'x': 'a' * 33000}).status_code == 422)
...
def test_history_joins_songs_recordings_and_analysis(api_client):
    ...
    items = response.json()
    assert ([item['kind'] for item in items], items[0]['status'], items[0]['duration_seconds']) == (['recording', 'processing'], 'analyzed', 12.5)
