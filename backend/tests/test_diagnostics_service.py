...
def test_torch_and_ai_detection_degrade_on_import_failure(monkeypatch):
    ...
    def blocked(name, *args, **kwargs):
        if name == "torch" or name == "AI" or name.startswith("AI."):
...
def test_pipeline_health_combines_runtime_components(monkeypatch):
    monkeypatch.setattr(diagnostics_service, "_torch_info", Mock(return_value=(True, False, "2")))
    monkeypatch.setattr(diagnostics_service, "_ffmpeg_available", Mock(return_value=True))
    monkeypatch.setattr(diagnostics_service, "_package_available", Mock(return_value=True))
    monkeypatch.setattr(diagnostics_service, "_ai_package_available", Mock(return_value=True))
...
def test_versions_reports_components_and_missing_packages(monkeypatch):
    ...
    monkeypatch.setattr(
        diagnostics_service.subprocess,
        "run",
        Mock(return_value=SimpleNamespace(stdout="ffmpeg version 8\nconfiguration")),
    )
    ...
    assert result["components"]["torch"] == "2.8"
    assert result["components"]["ffmpeg"] == "ffmpeg version 8"
    assert result["components"]["qwen_asr"] == "1"
    assert result["components"]["mido"] is None
    assert result["components"]["ai_build"]
...
def test_versions_tolerates_ai_and_ffmpeg_failures(monkeypatch):
    ...
    assert components["ai_build"] is None
    assert components["ai_pipeline"] is None
    assert components["ffmpeg"] is None
...
def test_recent_errors_serializes_and_closes_database(monkeypatch):
    ...
    songs = [
        SimpleNamespace(id="one", title="One", error_message="failed", updated_at=updated),
        SimpleNamespace(id="two", title="Two", error_message=None, updated_at=None),
    ]
    query = Mock()
