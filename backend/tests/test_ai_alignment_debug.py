...
def test_debug_flag_and_word_helpers(monkeypatch):
    ...
    assert (debug._word_dict(word) == {'text': 'hello', 'start': 1, 'end': 2, 'duration': 1, 'confidence': 0.8765}) and (debug._line_min_duration([]) == debug._line_max_duration([]) == 0) and (debug._line_min_duration(['hello']) >= 0.45) and (debug._line_max_duration(['hello']) >= 3)


def test_rms_activity_all_shapes():
    audio = np.concatenate((np.zeros(100), np.ones(100))).astype(np.float32)
    assert (debug._rms_activity(audio, 100, 3, 4) == {'rms': 0, 'peak': 0, 'active_ratio': 0}) and (debug._rms_activity(audio, 100, 0, 0.1)['active_ratio'] == 0) and (debug._rms_activity(audio, 100, 1, 1.05)['active_ratio'] == 1)
...
def test_build_alignment_debug_detects_structural_failures(monkeypatch):
    ...
    assert (report['token_count_matches'] is False) and (report['first_token_mismatch']['reason'] == 'token_count_mismatch') and (report['first_suspect'])
...
def test_write_alignment_debug_with_and_without_suspect(monkeypatch, tmp_path, capsys):
    ...
    assert (debug.write_alignment_debug(path, 'v', 'x', []) is suspect) and (json.loads(path.read_text()) == suspect) and ('FIRST SUSPECT' in capsys.readouterr().out)
...
def test_cli_reports_missing_and_builds_report(monkeypatch, tmp_path, capsys):
    ...
    assert (debug._main() == 2) and ('Missing required files' in capsys.readouterr().out)
    ...
    assert (debug._main() == 0) and (called[0][0] == tmp_path / 'alignmentDebug.json')
