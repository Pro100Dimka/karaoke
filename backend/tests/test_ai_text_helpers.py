...
def test_language_resolution(language, sample, expected): assert text.resolve_alignment_language(sample, language) == expected
...
def test_words_from_item_shapes_and_invalid_rows():
    ...
    words = text._words_from_items(items); assert ([word.text for word in words], [word.confidence for word in words]) == (['one', 'two', 'three'], [1, 0, 0.5])


def test_audio_normalization_and_chunks(): assert (text._normalize_singing_audio(np.array([], dtype=np.float32)).size == 0) and (not np.any(text._normalize_singing_audio(np.zeros(10, dtype=np.float32)))); normalized = text._normalize_singing_audio(np.array([-0.01, 0.02] * 100, dtype=np.float32)); assert (normalized.flags.c_contiguous and np.max(np.abs(normalized)) <= 0.94) and (text._singing_chunk_windows(np.array([], dtype=np.float32), 10) == []); short = np.ones(100, dtype=np.float32); assert len(text._singing_chunk_windows(short, 10)) == 1; long = np.ones(400, dtype=np.float32); long[150:170] = 0; windows = text._singing_chunk_windows(long, 10, [(0, 40)]); assert (len(windows) >= 2) and (len(text._singing_chunks(long, 10)) == len(text._singing_chunk_windows(long, 10)))
...
def test_asr_anchor_windows_and_matching():
    ...
    assert (set(anchors) == {0, 1} and anchors[0][2] == 1) and (text._asr_line_anchor_windows(['!!!'], [(0, 1, 'one')]) == {}); partial = text._asr_line_anchor_windows(["one", "missing"], [(0, 1, "one")]); assert (set(partial) == {0}) and (not text._canonical_words_match([], ['one'])) and (text._canonical_words_match([Word(0, 1, 'One!', index=0)], ['one']))


def test_complete_anchor_windows_branches(monkeypatch):
    groups, source = ['same line', 'same line', 'unique words'], np.ones(100, dtype=np.float32); monkeypatch.setattr(text, "_lossless_canonical_alignment", lambda *_: []); raw = {0: (0, 1, 0.5)}; assert text._complete_line_anchor_windows(groups, raw, source, 10, 10)[0] == raw
    baseline = [
...
def test_complete_anchor_windows_missing_blended_and_repaired(monkeypatch):
    source = np.ones(100, dtype=np.float32); assert text._complete_line_anchor_windows([], {}, source, 10, 10) == ({}, {})
...
def test_complete_anchor_windows_degenerate_observation(monkeypatch):
    source, baseline = np.ones(100, dtype=np.float32), [Word(20, 21, 'same'), Word(22, 23, 'same', index=1)]; monkeypatch.setattr(text, "_lossless_canonical_alignment", lambda *_: baseline)
    completed, provenance = text._complete_line_anchor_windows(
    ...
    baseline = [Word(20, 21, "same"), Word(22, 23, "same", index=1)]; monkeypatch.setattr(text, "_lossless_canonical_alignment", lambda *_: baseline)
    completed, provenance = text._complete_line_anchor_windows(
...
def test_complete_anchor_windows_subphrases_and_monotonic_repair(monkeypatch):
    source, baseline = np.ones(100, dtype=np.float32), [Word(0, 1, 'one'), Word(2, 3, 'one', index=1), Word(3, 4, 'two', index=2)]; monkeypatch.setattr(text, "_lossless_canonical_alignment", lambda *_: baseline)
    completed, provenance = text._complete_line_anchor_windows(
        ["one", "one two"], {0: (0.2, 1.2, 0.5)}, source, 10, 10
    )
    assert provenance[0] == "asr_vocal_blend" and provenance[1] == "vocal_baseline"

    baseline = [Word(0, 1, "first"), Word(2, 3, "second", index=1)]; monkeypatch.setattr(text, "_lossless_canonical_alignment", lambda *_: baseline)
    completed, provenance = text._complete_line_anchor_windows(
...
def test_lossless_canonical_alignment_branches(monkeypatch): source = np.ones(1000, dtype=np.float32); assert (text._lossless_canonical_alignment([], source, 100, 10) == []) and (text._lossless_canonical_alignment(['one'], source, 100, 0.01) == []); monkeypatch.setattr(text, "_activity_quantile_times", lambda *_: [5, 5.01]); result = text._lossless_canonical_alignment(["one two", "three"], source, 100, 10); assert [word.text for word in result] == ["one", "two", "three"]; patch_attrs(monkeypatch, text, _activity_quantile_times=lambda *_: [0, 10], _vocal_activity_regions=lambda *_a, **_k: [(0, 1), (3, 4)]); result = text._lossless_canonical_alignment(["one", "two"], source, 100, 10); assert result[1].start > result[0].end; short = text._lossless_canonical_alignment(["one two three four"], source, 100, 0.5); assert len(short) == 4; degenerate = text._lossless_canonical_alignment(["one"] * 10, source, 100, 0.05); assert len(degenerate) == 10


def test_lossless_canonical_alignment_weights_vowels_not_just_length(monkeypatch): source = np.ones(1000, dtype=np.float32); monkeypatch.setattr(text, "_activity_quantile_times", lambda *_: [0, 9]); result = text._lossless_canonical_alignment(["я тьмы"], source, 100, 9); assert [word.text for word in result] == ["я", "тьмы"]; span = result[1].end - result[0].start; assert (result[0].end - result[0].start == pytest.approx(span * 3 / 9)) and (result[1].end - result[1].start == pytest.approx(span * 6 / 9))


def test_atomic_line_alignment_interpolation_ctc_and_qwen(monkeypatch):
    source = np.ones(1000, dtype=np.float32); empty, stats = text._atomic_line_acoustic_alignment([], [], [], source, 100, 10); assert empty == [] and stats["lines"] == 0; patch_attrs(monkeypatch, text, _activity_quantile_times=lambda *_: [0, 10], _vocal_activity_regions=lambda *_a, **_k: [(0, 10)])
...
def test_atomic_rejects_invalid_ctc_candidates(monkeypatch):
    source = np.ones(1000, dtype=np.float32); monkeypatch.setattr(text, "_activity_quantile_times", lambda *_: [0, 10])
    invalid = [
        None,
        alignment_result((), 1),
    ...
    aligned, stats = text._atomic_line_acoustic_alignment(["one"], invalid, [], source, 100, 10); assert aligned and stats["ctc"] == 0
...
def test_atomic_short_song_uses_physical_baseline(monkeypatch): source = np.ones(1000, dtype=np.float32); monkeypatch.setattr(text, "_activity_quantile_times", lambda *_: [0, 0.05]); aligned, stats = text._atomic_line_acoustic_alignment(["one"] * 10, [], [], source, 100, 0.05); assert len(aligned) == 10 and stats["interpolated"] == 10; monkeypatch.setattr(text, "_lossless_canonical_alignment", lambda *_: []); aligned, stats = text._atomic_line_acoustic_alignment(["one"] * 10, [], [], source, 100, 0.05); assert aligned == [] and stats["line_fallbacks"] == 10


@pytest.mark.parametrize("confidences", [(0.1, 0.9), (0.9, 0.1)])
def test_atomic_prunes_both_impossible_edges(monkeypatch, confidences):
    source = np.ones(100, dtype=np.float32); monkeypatch.setattr(text, "_activity_quantile_times", lambda *_: [0, 1])
    ctc = [
        None,
...
def test_canonical_mergers_empty_and_interpolated(monkeypatch, function):
    source = np.ones(1000, dtype=np.float32); assert function([], [], [], source, 100, 10)[0] == []; patch_attrs(monkeypatch, text, _activity_quantile_times=lambda *_: [0, 10], _vocal_activity_regions=lambda *_a, **_k: [(0, 10)]); output, stats = function(["one two", "three four"], [], [], source, 100, 10)
    if function is text._anchor_preserving_canonical_alignment:
        assert output == []
    else:
        assert [word.text for word in output] == ["one", "two", "three", "four"]; assert stats["interpolated"] == 4
...
def test_canonical_mergers_keep_acoustic_candidates(monkeypatch, function): source = np.ones(1000, dtype=np.float32); monkeypatch.setattr(text, "_activity_quantile_times", lambda *_: [0, 10]); ctc_result, qwen = alignment_result((Word(1, 1.5, 'one', 0.9), Word(1.5, 2, 'two', 0.9, 1)), 0.9), [Word(5, 5.5, 'three', 0.8), Word(5.5, 6, 'four', 0.8, 1)]; output, stats = function(["one two", "three four"], [ctc_result, None], qwen, source, 100, 10); assert len(output) == 4 and stats["ctc"] + stats["qwen"] >= 2
...
def test_anchor_merger_nudges_tight_boundaries(confidences):
    ...
    output, stats = text._anchor_preserving_canonical_alignment(
        ["one two"], [alignment_result(words, 0)], [], source, 100, 10
    )
...
def test_anchor_merger_relaxed_chain_rejects_overlapping_candidate():
    output, stats = text._anchor_preserving_canonical_alignment(
        ["one two"],
        [alignment_result((Word(1, 2, 'one', 0.9),), 0.9)],
...
def test_line_aware_rejects_unrepresentable_micro_words(monkeypatch):
    tokens = " ".join(f"word{i}" for i in range(12)); monkeypatch.setattr(text, "_activity_fallback_words", lambda *_: [])
...
def test_anchor_merger_places_missing_run_on_activity_islands(monkeypatch, regions):
    source = np.ones(1000, dtype=np.float32); monkeypatch.setattr(text, "_vocal_activity_regions", lambda *_a, **_k: regions); ctc = [alignment_result((Word(0.5, 1, 'anchor', 0.9),), 0.9)]
    output, stats = text._anchor_preserving_canonical_alignment(
...
def test_anchor_merger_falls_back_when_activity_islands_cannot_partition(monkeypatch):
    patch_attrs(monkeypatch, text, _vocal_activity_regions=lambda *_a, **_k: [(2, 2.16), (5, 5.16)]); ctc = [alignment_result((Word(0.5, 1, 'anchor', 0.9),), 0.9)]
    output, stats = text._anchor_preserving_canonical_alignment(
...
def test_anchor_merger_reports_text_that_cannot_fit_song():
    ...
    output, stats = text._anchor_preserving_canonical_alignment(
        ...
        [alignment_result((Word(0, 0.06, 'one', 0.9),), 0.9)],
        [],
...
def test_anchor_merger_drops_prefix_anchor_without_capacity():
    output, stats = text._anchor_preserving_canonical_alignment(
        ["one two"],
        [alignment_result((Word(0.01, 0.08, 'two', 0.9, 1),), 0.9)],
        [],
        np.ones(1000, dtype=np.float32),
        100,
        10,
...
def test_anchor_merger_reacquires_complete_missing_line(monkeypatch):
    source, ctc = np.ones(1000, dtype=np.float32), [alignment_result((Word(0.5, 1, 'one', 0.9),), 0.9), None, alignment_result((Word(8, 8.5, 'four', 0.9),), 0.9)]; patch_attrs(monkeypatch, text, _activity_fallback_words=lambda tokens, audio, sample_rate: text._proportional_words(tokens, len(audio) / sample_rate))
    output, stats = text._anchor_preserving_canonical_alignment(
        ["one", "two three", "four"],
        ctc,
        [],
...
def test_anchor_merger_reacquisition_rejects_invalid_windows(monkeypatch, mode):
    sample_rate = 1 if mode == "empty-audio" else 100; source, ctc, windows = np.ones(10 * sample_rate, dtype=np.float32), [alignment_result((Word(0, 0.2, 'one', 0.9),), 0.9), None, alignment_result((Word(8, 8.2, 'three', 0.9),), 0.9)], {0: (0, 1, 1), 2: (8, 9, 1)}
    if mode == "empty-audio":
...
def test_line_aware_merger_conflicting_and_remote_candidates(monkeypatch):
    source = np.ones(1000, dtype=np.float32); monkeypatch.setattr(text, "_activity_quantile_times", lambda *_: [5, 5.01])
    ctc_lines = [
...
def test_line_aware_short_song_failure_boundaries(monkeypatch, duration):
    source = np.ones(1000, dtype=np.float32); monkeypatch.setattr(text, "_activity_quantile_times", lambda *_: [0, duration])
    output, stats = text._line_aware_canonical_alignment(
...
def test_line_aware_invalid_bounds_conflicts_and_empty_baseline(monkeypatch):
    ...
    for duration in (0.1, 0.5, 1.0):
        ...
        assert output == [] or len(output) == 10; assert stats["line_fallbacks"] >= 1
...
def test_canonical_mergers_adversarial_matrix(monkeypatch):
    ...
    for groups, ctc, qwen, anchors, duration in candidates:
        for merger in (
            ...
            output, stats = merger(groups, ctc, qwen, source, 100, duration, **kwargs); assert isinstance(output, list) and isinstance(stats, dict)
