...
def test_cache_loads_only_current_valid_index(tmp_path):
    ...
    dump(cache.index_path, [])
    assert cache._load_index() == {"version": cache.INDEX_VERSION, "stages": {}}
    dump(cache.index_path, {"version": cache.INDEX_VERSION, "stages": []})
    assert cache._load_index()["stages"] == {}
    dump(cache.index_path, {"version": cache.INDEX_VERSION, "stages": {"a": {}}})
...
def test_file_hash_memoization_and_optional_files(tmp_path):
    ...
    assert cache.file_hash(source) == first
    assert cache.optional_file_hash(None) is None
    assert cache.optional_file_hash(tmp_path / "missing").startswith("missing:")
    assert cache.optional_file_hash(source) == first
    cache._hash_memo.update({(str(i), i, i, i): str(i) for i in range(4097)})
    other = tmp_path / "other"
    other.write_bytes(b"other")
    assert cache.file_hash(other)
    assert len(cache._hash_memo) == 1
    assert StageCache.key("stage", {"b": 1, "a": source}) == StageCache.key(
        "stage", {"a": source, "b": 1}
    )
...
def test_cache_hit_rejects_malformed_or_changed_outputs(tmp_path):
    ...
    for entry in cases:
        dump(cache.index_path, {"version": cache.INDEX_VERSION, "stages": {"s": entry}})
