...
def test_diagnostic_primitives(): assert (clamp01(2) == 1 and clamp01(-1) == 0) and (diagnostics._median([1, float('nan'), 3]) == 2) and (diagnostics._median([], 4) == 4) and (diagnostics._pitch_regions([]) == ([], [])); regions, onsets = diagnostics._pitch_regions([pitch(0), pitch(0.01), pitch(0.02), pitch(1)]); assert (len(regions) == 2 and onsets == [0, 1]) and (diagnostics._overlap_ratio(0, 1, [(-1, 0), (0.2, 0.5), (2, 3)]) == pytest.approx(0.3)) and (diagnostics._nearest_ms(1, []) is None) and (diagnostics._nearest_ms(1, [0.9, 2]) == pytest.approx(100))
...
def test_timeline_metrics_and_candidates():
    ...
    result = diagnostics._timeline_metrics(items); assert (result['overlap_count'] == 1 and result['gap_count'] == 1) and (result['micro_interval_count'] == 2 and result['non_positive_interval_count'] == 1) and (diagnostics._candidate_for({}, 'ctc') is None) and (diagnostics._candidate_for({'ctc': []}, 'ctc') is None) and (diagnostics._candidate_for({'ctc': {'start': 0, 'end': 1}}, 'ctc') == {'start': 0, 'end': 1, 'confidence': 0}) and (diagnostics._candidate_for({'ctc': {'start': 'bad'}}, 'ctc') is None)


def candidate(kind, start, end, confidence=0.7): return {kind: {'start': start, 'end': end, 'confidence': confidence}}


def test_build_alignment_debug_comprehensive():
    words, candidates, frames, raw, syllables, notes = [Word(0, 0.01, 'zero', confidence=0.01, index=0), Word(0.2, 3.2, 'one', confidence=0.1, index=1), Word(3, 3.5, 'two', confidence=0.8, index=2), Word(4, 4.5, 'three', confidence=0.8, index=3), Word(5, 5.5, 'four', confidence=0.8, index=4), Word(5.4, 5.8, 'five', confidence=0.01, index=5)], [candidate('consensus', 0, 0.01), {**candidate('ctc', 0.2, 1), **candidate('qwen', 0.21, 1.01)}, {**candidate('ctc', 3, 3.2), **candidate('qwen', 4, 4.2)}, candidate('ctc', 4, 4.5), candidate('qwen', 5, 5.5), 'bad'], [pitch(0), pitch(0.01), pitch(0.3), pitch(1), pitch(3.1), pitch(4.2), pitch(5.5)], [pitch(0, 440), pitch(0.01, voiced=False), pitch(0.3, 220), pitch(99)], [Syllable(0, 0.01, 'z', 0, 0, 0.5), Syllable(0.2, 0.2, 'o', 1, 1, 0.6), Syllable(3, 3.5, 't', 2, 2, 0.7)], [VocalNote(0, 0.02, 60, word_index=0, syllable_index=0), VocalNote(0.01, 0.03, 72)]
    game = [
        *notes,
...
def test_build_debug_classifies_long_weak_and_separated_suspicious_regions():
    ...
    reasons = [set(row["reasons"]) for row in result["words"]]; assert ('weak_vocal_overlap' in reasons[2]) and ('very_long_duration' in reasons[3]) and (len(result['suspicious_regions']) >= 2) and (result['notes']['game_duration_quantiles'] == {'p05': 1, 'p25': 1, 'p50': 1, 'p75': 1, 'p95': 1})
...
def test_region_summary():
    ...
    result = diagnostics._region(items); assert result["words"] == 30 and len(result["text"].split()) == 24
