...
def test_load_mono_resamples_and_reports_duration(tmp_path):
    ...
    assert (rate == 8000 and len(unchanged) == 2) and (audio.duration(path) > 0)
    raises(ValueError, lambda: audio.load_mono(path, 0), match='positive')
...
def test_audio_buffer_cache_reuses_io_but_returns_independent_arrays(monkeypatch, tmp_path):
    ...
    assert (rate == second_rate == 16000 and len(reads) == 1) and (np.array_equal(second, third) and third[0] != 99)
    audio.load_mono(path, 16000)
    assert len(reads) == 2


def test_decode_audio_validates_inputs(tmp_path):
    raises(FileNotFoundError, lambda: audio.decode_audio(tmp_path / 'missing', tmp_path / 'out'))
    source = tmp_path / "source"
    source.write_bytes(b"x")
    raises(ValueError, lambda: audio.decode_audio(source, tmp_path / 'out', 0), match='positive')
...
def test_decode_audio_success(monkeypatch, tmp_path):
    ...
    patch_many(monkeypatch, (audio.subprocess, "run", run), (audio.sf, "info", lambda _: SimpleNamespace(frames=1, samplerate=44100)))
...
def test_decode_audio_wraps_process_failures(monkeypatch, tmp_path, failure, message):
    ...
    raises(AICoreError, lambda: audio.decode_audio(source, tmp_path / 'out'), match=message)
...
def test_decode_audio_rejects_bad_output(monkeypatch, tmp_path):
    ...
    raises(AICoreError, lambda: audio.decode_audio(source, tmp_path / 'out'), match='valid WAV')

    def create(command, **_):
        with open(command[-1], "wb") as stream: stream.write(b"x" * 44)

    patch_many(monkeypatch, (audio.subprocess, "run", create), (audio.sf, "info", lambda _: SimpleNamespace(frames=0, samplerate=1)))
    raises(AICoreError, lambda: audio.decode_audio(source, tmp_path / 'out'), match='empty WAV')
...
def test_validate_audio_and_json(monkeypatch, tmp_path):
    ...
    raises(InvalidArtifactError, lambda: validators.validate_audio(path), match='Empty')

    payload = tmp_path / "data.json"
    dump_json(payload, {"ok": 1})
    assert validators.validate_json(payload, ["ok"]) == {"ok": 1}
    dump_json(payload, [])
    raises(InvalidArtifactError, lambda: validators.validate_json(payload), match='object')
    dump_json(payload, {})
    raises(InvalidArtifactError, lambda: validators.validate_json(payload, ['missing']), match='required')
...
def test_validate_timeline_rejects_invalid(items, message):
    raises(InvalidArtifactError, lambda: validators.validate_timeline(items), match=message)
...
def test_validate_duration_and_pitch():
    ...
    raises(InvalidArtifactError, lambda: validators.validate_within_duration([], 0, 'words'), match='duration')
    raises(InvalidArtifactError, lambda: validators.validate_within_duration([TimeSpan(0, 2)], 1, 'words', -1), match='outside')
...
def test_json_domain_validators(tmp_path):
    ...
    dump_json(pitch, [{"time": 0, "frequency": 440, "confidence": 1, "voiced": True}])
    assert validators.validate_pitch_json(pitch)[0].frequency == 440
    words = tmp_path / "words.json"
    dump_json(words, {"text": "hi", "words": [{"start": 0, "end": 1, "text": "hi"}]})
    ...
    for kind, key, item in [
        ...
        dump_json(path, {key: [item]})
        assert len(validators.validate_derivation_json(path, kind)) == 1
    dump_json(pitch, {})
    raises(InvalidArtifactError, lambda: validators.validate_pitch_json(pitch))
    dump_json(pitch, [{"bad": True}])
    raises(InvalidArtifactError, lambda: validators.validate_pitch_json(pitch))
    dump_json(pitch, {"syllables": "bad"})
    raises(InvalidArtifactError, lambda: validators.validate_derivation_json(pitch, 'syllables'))
    dump_json(pitch, [])
    raises(InvalidArtifactError, lambda: validators.validate_derivation_json(pitch, 'syllables'))
    raises(InvalidArtifactError, lambda: validators.validate_words_json(pitch))
    dump_json(pitch, {})
    raises(InvalidArtifactError, lambda: validators.validate_derivation_json(pitch, 'unknown'))
    raises(InvalidArtifactError, lambda: validators.validate_words_json(pitch))
    dump_json(pitch, {"text": "hi", "words": [{"bad": True}]})
    raises(InvalidArtifactError, lambda: validators.validate_words_json(pitch))


def test_music_json_validation(tmp_path):
    path = tmp_path / "music.json"
    dump_json(path, {"bpm": 120})
    assert validators.validate_music_json(path) == 120
    for bpm in ("bad", 10, float("inf")):
        dump_json(path, {"bpm": bpm})
        raises(InvalidArtifactError, lambda: validators.validate_music_json(path))
