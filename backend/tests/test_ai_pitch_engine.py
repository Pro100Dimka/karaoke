...
def test_fcpe_configuration_and_model_loading(monkeypatch):
    ...
    assert (estimator._load_model() == (torch, model)) and (estimator._load_model() == (torch, model))


def test_fcpe_model_load_retries_registered_cpu_fallback(monkeypatch):
    estimator, torch, model = pitch.FCPEPitchEstimator(), torch_stub(), object()
    ...
    patch_attrs(monkeypatch, pitch, select_torch_device=lambda *_: 'cuda:0', fallback_torch_device=lambda *_: 'cpu')
    assert (estimator._load_model() == (torch, model)) and ([call.kwargs['device'] for call in spawn.call_args_list] == ['cuda:0', 'cpu'])
...
def test_fcpe_missing_dependencies(monkeypatch):
    ...
    raises(EngineUnavailableError, lambda: pitch.FCPEPitchEstimator()._load_model(), match='torchfcpe')


def test_fcpe_estimate_empty_and_confident_frames(monkeypatch):
    estimator, model = pitch.FCPEPitchEstimator(sr=100, hop=10, fmin=50, fmax=500), SimpleNamespace(infer=lambda *_args, **_kwargs: (Tensor([100, 600, np.nan]), Tensor([0.8, 0.9, 0.7])))
...
def test_fcpe_old_api_and_confidence_fallback(monkeypatch):
    ...
    model = OldModel()
    patch_many(monkeypatch, (estimator, "_load_model", lambda: (torch_stub(), model)), (pitch, "load_mono", lambda *_: (np.asarray([0.1], dtype=np.float32), 100)))
    frames = estimator.estimate("audio")
    assert (model.calls == 2) and (frames[0].confidence == 1 and frames[1].confidence == 0) and (frames[-1].energy == 0)


def test_fcpe_ignores_mismatched_confidence(monkeypatch):
    estimator, model = pitch.FCPEPitchEstimator(sr=100, hop=10, fmin=50, fmax=500), SimpleNamespace(infer=lambda *_args, **_kwargs: [Tensor([100, 110]), Tensor([0.1])])
    patch_many(monkeypatch, (estimator, "_load_model", lambda: (torch_stub(), model)), (pitch, "load_mono", lambda *_: (np.ones(10, dtype=np.float32), 100)))
...
def test_fcpe_shadow_is_diagnostic_and_cleanup_safe():
    ...
    estimator, model, production = pitch.FCPEPitchEstimator(sr=100, hop=10, fmin=50, fmax=500, shadow_policy=ShadowPolicy(True, 1), shadow_backend_factory=lambda: backend), SimpleNamespace(wav2mel=lambda *_: Tensor(np.ones((1, 2, 3))), model=SimpleNamespace(cent_table=Tensor(np.arange(4)))), [PitchFrame(0, 100, 1, True, 0.2), PitchFrame(0.1, 0, 0, False, 0.1)]
    estimator._run_shadow("song.wav", model, Tensor([0]), np.ones(10), 2, production)
    assert ((estimator.last_shadow_diagnostics['status'], estimator.last_shadow_frames) == ('compared', production)) and (backend.release.called and estimator._shadow_backend is None)
...
def test_fcpe_shadow_failure_and_resident_policy_are_isolated():
    ...
    estimator, model = pitch.FCPEPitchEstimator(shadow_policy=ShadowPolicy(True, 1, True), shadow_backend_factory=lambda: failing), SimpleNamespace(wav2mel=lambda *_: Tensor(np.ones((1, 2, 3))), model=SimpleNamespace(cent_table=Tensor(np.arange(4))))
    estimator._run_shadow("song.wav", model, Tensor([0]), np.ones(10), 2, [])
    assert ('shadow failed' in estimator.last_shadow_diagnostics['error']) and (not failing.release.called)
...
def test_pyin_fallback(monkeypatch):
    ...
    assert (frames[0].voiced and frames[0].energy == 0.3) and (not frames[1].voiced and frames[1].energy == 0)
...
def test_pyin_requires_librosa(monkeypatch):
    ...
    raises(EngineUnavailableError, lambda: pitch.PyinFallbackPitchEstimator().estimate('audio'), match='librosa')
...
def test_fcpe_shadow_backend_can_select_directml_from_env(monkeypatch):
    ...
    assert (estimator._configured_shadow_class() is pitch.OrtDirectMLFCPEBackend) and (estimator._shadow_key() == 'onnxruntime:directml:fp32')
