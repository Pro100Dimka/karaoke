...
def test_align_lines_rejects_a_line_compressed_well_below_its_expected_duration(monkeypatch):
    """An unanchored line whose best CTC path lands far short of how long it
    should take to sing must fall through, not be accepted as "close enough".

    Regression, calibrated against a real production run of one song: a
    six-word line ("Я не хочу курить после любви") was accepted at a ~1.06s
    span against a ~2.9s expected duration (36%), rendering it on screen for
    barely a second -- audibly too fast to be the real vocal -- while two
    genuinely correct neighboring lines in the same song landed at 125% and
    134% of their own expected duration. The floor needs real margin below
    the bad line and below the good ones; this locks in that a span in the
    bad line's range is rejected so the pipeline falls through to its
    ASR/Qwen fallback tier instead.
    """
    aligner = ctc.CTCWordAligner()
    monkeypatch.setattr(aligner, "available_for", lambda *_: True)
    monkeypatch.setattr(ctc, "load_mono", lambda *_: (np.zeros(16_000 * 20), 16_000))
    words = ["one", "two", "three", "four", "five", "six"]

    def compressed(_audio, _rate, group_words, _language):
        # ~1.05s total span for a line whose _expected_duration is ~2.9s
        # (36% of expected): the exact ratio observed in production, still
        # comfortably below the 60% floor.
        return ctc.CTCLineResult(
            tuple(
                Word(index * 0.18, index * 0.18 + 0.15, token, 0.8, index)
                for index, token in enumerate(group_words)
            ),
            0.8,
            0,
            1.05,
    ...
    monkeypatch.setattr(aligner, "align_window", compressed)
    result = aligner.align_lines("x", [" ".join(words)], "en")
    assert result == [None]


def test_align_lines_rejects_one_word_stranded_far_from_the_rest_of_the_line(monkeypatch):
    """A wide envelope can still hide a broken internal layout.

    Regression, observed in the same real production run: the span-ratio fix
    above widened a line's accepted envelope from 3.889-4.835s to 0.0-4.19s --
    correct at the *line* level, but the first word ("Я") had locked onto a
    noise/silence false positive at t=0.0-0.02s, then every other word in the
    line ("не хочу курить после любви") was still crammed into the last 0.85s
    (3.345-4.19s). The 3.3s dead gap between word 1 and word 2 made the
    on-screen fill jump to nearly 100% while the singer had only reached the
    second word. The envelope span alone (4.19s, comfortably over the 60%
    floor) does not catch this; a dominant internal gap must also reject it.
    """
    aligner = ctc.CTCWordAligner()
    monkeypatch.setattr(aligner, "available_for", lambda *_: True)
    monkeypatch.setattr(ctc, "load_mono", lambda *_: (np.zeros(16_000 * 20), 16_000))
    ...
    def stranded(_audio, _rate, group_words, _language):
        first, rest = group_words[0], group_words[1:]
        crammed_start = 2.6
        crammed_step = (2.89 - crammed_start) / len(rest)
        timings = [(0.0, 0.02, first)] + [
            (crammed_start + index * crammed_step, crammed_start + (index + 1) * crammed_step, token)
            for index, token in enumerate(rest)
        ]
        return ctc.CTCLineResult(
            tuple(Word(start, end, token, 0.8, i) for i, (start, end, token) in enumerate(timings)),
            0.8,
            0,
            2.89,
        )

    monkeypatch.setattr(aligner, "align_window", stranded)
    result = aligner.align_lines("x", [" ".join(words)], "en")
    assert result == [None]
