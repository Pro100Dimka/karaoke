...
def test_model_status_reports_missing_resources(monkeypatch, tmp_path):
    model = _model()
    patch_many(monkeypatch, (model_install_service, "MODELS", (model,)), (model_install_service.config, "MODELS_DIR", tmp_path / "models"))
    model_install_service._set_state(state="idle", current_model=None, error=None)

    result = model_install_service.status()

    assert (result['state'], result['ready_count'], result['models']) == ('missing', 0, [{'key': 'recovery', 'name': 'Recovery model', 'ready': False}])
...
def test_recovery_download_verifies_and_activates_models(monkeypatch, tmp_path):
    ...
    patch_attrs(monkeypatch, model_install_service, MODELS=(model,), install_one=install)
    patch_attrs(monkeypatch, model_install_service.config, configure_ai_resource_environment=lambda **options: configured.append(options))
...
def test_status_ready_overrides_stale_runtime_error(monkeypatch, tmp_path):
    model = _model()
    patch_many(monkeypatch, (model_install_service, "MODELS", (model,)), (model_install_service.config, "MODELS_DIR", tmp_path))
    patch_attrs(monkeypatch, model_install_service, is_valid=Mock(return_value=True), _progress_values=Mock(return_value={'downloaded_bytes': 42}))
    model_install_service._set_state(state="error", current_model="stale", error="previous failure")

    result = model_install_service.status()

    assert (result['state'] == 'ready') and (result['ready'] is True) and (result['current_model'] is None) and (result['error'] is None) and (result['downloaded_bytes'] == 42)


def test_status_preserves_active_download_state(monkeypatch, tmp_path):
    patch_many(monkeypatch, (model_install_service, "MODELS", (_model(),)), (model_install_service.config, "MODELS_DIR", tmp_path), (model_install_service, "is_valid", Mock(return_value=False)))
...
def test_download_worker_skips_valid_models(monkeypatch, tmp_path):
    model, reporter = _model(), Mock()
    patch_attrs(monkeypatch, model_install_service, MODELS=(model,), is_valid=Mock(return_value=True), ProgressReporter=Mock(return_value=reporter))
    install = Mock()
    patch_many(monkeypatch, (model_install_service, "install_one", install), (model_install_service.config, "configure_ai_resource_environment", Mock()))
...
def test_download_worker_reports_verification_failure(monkeypatch, tmp_path):
    reporter = Mock()
    patch_attrs(monkeypatch, model_install_service, MODELS=(_model(),), is_valid=Mock(return_value=False), install_one=Mock(), ProgressReporter=Mock(return_value=reporter))
    model_install_service._set_state(state="downloading", current_model=None, error=None)

    model_install_service._download_worker(tmp_path / "models", tmp_path / "cache")

    assert (model_install_service._state['state'], model_install_service._state['error']) == ('error', 'Model verification failed after download')
...
def test_sync_recovery_repairs_before_processing(monkeypatch, tmp_path):
    model, models_root = _model(), tmp_path / 'models'
    monkeypatch.setattr(model_install_service, "MODELS", (model,))
    patch_attrs(monkeypatch, model_install_service.config, MODELS_DIR=models_root, CACHE_DIR=tmp_path / 'cache-root', APP_LOG_DIR=tmp_path / 'logs')
    configured = Mock()
