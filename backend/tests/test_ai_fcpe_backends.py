...
def test_decode_and_nearest_resize_contract():
    ...
    assert (f0.shape == confidence.shape == (1, 2)) and (f0[0, 0] > 0 and f0[0, 1] == 0)
...
def test_ort_fcpe_load_infer_cache_release(monkeypatch, tmp_path):
    ...
    session = FakeOrtSession('mel', np.pad(np.ones((1, 2, 1), dtype=np.float32), ((0, 0), (0, 0), (4, 7))))
    ...
    assert (result.f0.shape == result.confidence.shape == (4,)) and ((result.providers[0], result.input_bytes) == ('CUDAExecutionProvider', 1 * 2 * 128 * 4)) and (session.disable_fallback.called) and (adapter._load() is session and adapter.last_initialization_sec == 0) and ('f0' not in fcpe_backends.describe_fcpe_inference(result))
    adapter.release()
...
def test_ort_fcpe_failures_are_explicit(monkeypatch, tmp_path):
    ...
    raises(EngineUnavailableError, lambda: adapter._load(), match='missing')
    monkeypatch.setattr(adapter, "availability", lambda: BackendAvailability(True, "ok"))
    monkeypatch.setitem(sys.modules, "onnxruntime", None)
    raises(EngineUnavailableError, lambda: adapter._load(), match='unavailable')
    monkeypatch.setitem(
...
def test_directml_fcpe_uses_required_session_options(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(adapter, "availability", lambda: BackendAvailability(True, "ok"))
    session, options = FakeSession(('DmlExecutionProvider', 'CPUExecutionProvider')), SimpleNamespace(graph_optimization_level=None, enable_mem_pattern=True, execution_mode=None)
...
def test_directml_fcpe_rejects_provider_fallback(monkeypatch, tmp_path):
    ...
    fake.ExecutionMode = SimpleNamespace(ORT_SEQUENTIAL="sequential")
    monkeypatch.setitem(sys.modules, "onnxruntime", fake)
    raises(EngineUnavailableError, lambda: adapter._load(), match='DmlExecutionProvider')
