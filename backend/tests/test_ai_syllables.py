...
def frame(time, frequency=220, confidence=0.9, voiced=True, energy=0.8):
    return PitchFrame(time, frequency if voiced else 0, confidence, voiced, energy)
...
def test_frame_slice_and_local_step():
    ...
    assert syllables._frame_slice(pitch, [item.time for item in pitch], 0.01, 0.02) == pitch[1:3]
    assert syllables._local_step(pitch, 1, 2) == pytest.approx(0.01)
    assert syllables._local_step([], 1.2, 2) == pytest.approx(0.1)
...
def test_proportional_and_refined_bounds(monkeypatch):
    ...
    assert expected == [3]
    assert syllables._refine_proportional_bounds(word, [], expected) == expected
...
def test_align_syllables_proportional_and_chronological(monkeypatch):
    ...
    assert [item.text for item in result] == ["test", "ма", "ма", "!!!"]
    assert [item.index for item in result] == list(range(4))
    assert result[0].confidence == pytest.approx(0.29)

    monkeypatch.setattr(
        syllables,
        "_refine_proportional_bounds",
        lambda _w, _f, values: [values[0] + 0.1] if values else values,
    )
