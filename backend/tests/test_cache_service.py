...
def test_optimize_song_handles_missing_and_complete_song(monkeypatch, tmp_path):
    ...
    def mp3(_source, target):
        target.write_bytes(b"mp3")

    def flac(_source, target):
        target.write_bytes(b"flac")

    monkeypatch.setattr(cache_service, "_encode_mp3", mp3)
    monkeypatch.setattr(cache_service, "_encode_flac", flac)
    commit = Mock()
    ...
    assert current.source_path == str(source) and source.exists()
    assert not song_wav.exists() and (out / "song.mp3").exists()
    assert not instrumental.exists() and (separated / "instrumental.flac").exists()
...
def test_optimize_commit_failure_restores_filesystem(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(cache_service.repositories, "get_song", Mock(return_value=current))
    monkeypatch.setattr(cache_service.song_service, "resolve_output_dir", Mock(return_value=out))
    monkeypatch.setattr(cache_service, "_encode_mp3", lambda _s, t: t.write_bytes(b"mp3"))
    monkeypatch.setattr(cache_service, "_encode_flac", lambda _s, t: t.write_bytes(b"flac"))
    monkeypatch.setattr(cache_service, "commit", Mock(side_effect=RuntimeError("commit failed")))

    with pytest.raises(RuntimeError, match="commit failed"):
        cache_service.optimize_song_files("song")

    assert source.exists() and song_wav.exists() and instrumental.exists()
    assert not (out / "song.mp3").exists()
    assert not (separated / "instrumental.flac").exists()
    assert not (out / cache_service._OPTIMIZATION_JOURNAL).exists()
    restored = cache_service.read_json(out / "manifest.json")
    assert restored["outputs"]["song"] == "song.wav"
    assert restored["outputs"]["instrumental"] == "separated/instrumental.wav"
...
def test_prepare_optimization_failure_is_all_or_nothing(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        cache_service,
        "_encode_flac",
        lambda _s, t: (t.write_bytes(b"partial"), (_ for _ in ()).throw(RuntimeError("fail")))[1],
    )
    with pytest.raises(RuntimeError, match="fail"):
        cache_service._prepare_optimization(tmp_path, [])
    assert (tmp_path / "song.wav").exists()
    assert (separated / "instrumental.wav").exists()
    assert not (tmp_path / "song.mp3").exists()
    assert not (separated / "instrumental.flac").exists()
    assert cache_service.read_json(tmp_path / "manifest.json") == original_manifest
...
def test_optimization_recovery_rejects_unsafe_journal_paths(tmp_path, committed, key, value):
    ...
    with pytest.raises(ValueError, match="journal|path"):
        cache_service._recover_optimization(out, committed=committed)
...
def test_optimize_partial_encoder_failure_keeps_retryable_state(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(cache_service.song_service, "resolve_output_dir", Mock(return_value=out))
    monkeypatch.setattr(cache_service, "_encode_mp3", lambda _s, t: t.write_bytes(b"mp3"))
    monkeypatch.setattr(cache_service, "_encode_flac", Mock(side_effect=RuntimeError("codec failed")))
    commit = Mock()
    monkeypatch.setattr(cache_service, "commit", commit)
    ...
    assert (out / "song.wav").exists() and (separated / "instrumental.wav").exists()
    assert not (out / "song.mp3").exists() and not (separated / "instrumental.flac").exists()
...
def test_safe_journal_path_accepts_only_contained_portable_paths(tmp_path):
    ...
    for value in (None, "", "a\\b", "C:/x", "../x", "/x", ".", "x/../y"):
        with pytest.raises(ValueError):
            cache_service._safe_journal_path(out, value)
...
def test_committed_stale_optimization_journal_never_retires_new_generation(tmp_path):
    ...
    assert (out / "song.wav").read_bytes() == b"NEW-GENERATION"
    assert (out / "separated/vocals.wav").exists()
    assert (out / "separated/instrumental.wav").exists()
    assert not (out / cache_service._OPTIMIZATION_JOURNAL).exists()


def test_startup_optimization_recovery_scans_journals(monkeypatch, tmp_path):
    database = Mock()
    current = SimpleNamespace(id="song", optimized=True)
    out = tmp_path / "song"
    out.mkdir()
    (out / cache_service._OPTIMIZATION_JOURNAL).write_text("{}", encoding="utf-8")
    monkeypatch.setattr(cache_service, "SessionLocal", Mock(return_value=database))
    monkeypatch.setattr(cache_service.song_service, "list_songs", Mock(return_value=[current]))
    monkeypatch.setattr(cache_service.song_service, "resolve_output_dir", Mock(return_value=out))
    recover = Mock()
...
def test_startup_optimization_recovery_is_fail_closed_per_song(monkeypatch, tmp_path):
    database = Mock()
    current = SimpleNamespace(id="song", optimized=False)
    out = tmp_path / "song"
    out.mkdir()
    (out / cache_service._OPTIMIZATION_JOURNAL).write_text("{}", encoding="utf-8")
    monkeypatch.setattr(cache_service, "SessionLocal", Mock(return_value=database))
