...
def test_note_values_convert_to_midi(value, expected): assert analysis_service._to_midi(value) == expected
...
def test_analysis_requires_processed_reference(monkeypatch, tmp_path): recording = models.Recording(song_id="song", filename="take.wav", path="take.wav"); raises(ValueError, lambda: analysis_service.analyze_recording(recording, domain_song()), match='нет эталонной'); patch_many(monkeypatch, (analysis_service.song_service, "resolve_output_dir", lambda _song: tmp_path), (analysis_service.ai_bridge, "get_reference_notes", lambda _path: []), (analysis_service, "read_json", lambda _path: [])); raises(ValueError, lambda: analysis_service.analyze_recording(recording, domain_song(str(tmp_path))), match='reference.json')
...
def test_analysis_filters_invalid_frames_and_calculates_sections(monkeypatch, tmp_path):
    ...
    patch_many(monkeypatch, (analysis_service.song_service, "resolve_output_dir", lambda _song: tmp_path), (analysis_service.ai_bridge, "get_reference_notes", lambda _path: reference), (analysis_service, "read_json", lambda _path: structure), (analysis_service.ai_bridge, "get_analyze_vocal", lambda: lambda _p: frames))
...
def test_analysis_returns_empty_metrics_without_comparable_frames(monkeypatch, tmp_path):
    recording = models.Recording(song_id="song", filename="take.wav", path="take.wav"); monkeypatch.setattr(analysis_service.song_service, "resolve_output_dir", lambda _song: tmp_path); patch_attrs(monkeypatch, analysis_service.ai_bridge, get_reference_notes=lambda _path: [{'start': 0, 'end': 1, 'midi': 60}]); patch_many(monkeypatch, (analysis_service, "read_json", lambda _path: {"not": "sections"}), (analysis_service.ai_bridge, "get_analyze_vocal", lambda: lambda _p: []))
