...
def upload(payload: bytes) -> UploadFile:
    return UploadFile(filename="payload.bin", file=BytesIO(payload))
...
def test_bounded_upload_read_and_save(tmp_path):
    run = asyncio.run
    assert run(read_upload_limited(upload(b"abcdef"), limit=6, chunk_size=2)) == b"abcdef"
    with pytest.raises(HTTPException) as too_large:
        run(read_upload_limited(upload(b"abcdef"), limit=5, chunk_size=2))
    assert too_large.value.status_code == 413

    destination = tmp_path / "uploads" / "payload.bin"
    assert run(save_upload_limited(upload(b"abcdef"), destination, limit=6, chunk_size=2)) == 6
    assert destination.read_bytes() == b"abcdef"

    with pytest.raises(HTTPException):
        run(save_upload_limited(upload(b"abcdef"), destination, limit=5, chunk_size=2))
