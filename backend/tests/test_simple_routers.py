...
def test_lifespan_installs_handler_initializes_and_always_cleans(monkeypatch):
    ...
    recover_imports = Mock()
    recover_optimization = Mock()
    monkeypatch.setattr(main.storage_migration, "migrate_legacy_song_storage", migrate)
    monkeypatch.setattr(main.song_package_service, "recover_import_transactions", recover_imports)
    monkeypatch.setattr(main.cache_service, "recover_optimization_transactions", recover_optimization)
...
def test_lifespan_handler_uses_loop_default_without_previous(monkeypatch):
    ...
    monkeypatch.setattr(main.song_package_service, "recover_import_transactions", Mock())
    monkeypatch.setattr(main.cache_service, "recover_optimization_transactions", Mock())
