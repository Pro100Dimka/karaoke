...
def test_song_artifact_manifest_and_fallback_resolution(tmp_path):
    ...
    assert song_artifacts.processing_outputs(tmp_path)["instrumental"] == "separated/instrumental.flac"
    assert song_artifacts.resolve_manifest_output(tmp_path, "instrumental") == flac.resolve()
    assert song_artifacts.resolve_audio_artifact(tmp_path, "instrumental") == flac.resolve()
...
def test_song_artifact_rejects_unsafe_manifest_and_uses_legacy_fallback(tmp_path):
    ...
    assert song_artifacts.resolve_manifest_output(tmp_path, "instrumental") is None
...
def test_portable_output_policy_rejects_user_local_namespaces():
    ...
    assert song_artifacts.is_portable_output_path("separated/instrumental.flac")
