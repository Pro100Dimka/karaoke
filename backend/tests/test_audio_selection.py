...
def install_devices(monkeypatch, devices, hosts):
    ...
    patch_attrs(monkeypatch, audio_service.sd, query_devices=Mock(side_effect=lambda index=None, **_kwargs: devices if index is None else devices[index]), query_hostapis=lambda index: {'name': hosts[index]})
...
def test_asio_bridge_path_and_driver_enumeration(monkeypatch, tmp_path):
    ...
    assert (audio_service._asio_bridge_path() == bridge) and (audio_service.list_asio_drivers() == [])
...
def test_resolved_device_prefers_default_then_low_latency_host(monkeypatch):
    ...
    assert (audio_service._resolved_device_index(None, 'input') == 1) and (audio_service._resolved_device_index(0, 'input') == 0)

    monkeypatch.setattr(audio_service.sd.default, "device", (-1, -1))
    assert audio_service._resolved_device_index(None, "input") == 1
    raises(RuntimeError, lambda: audio_service._resolved_device_index(None, 'output'), match='No output')
...
def test_low_latency_and_duplex_selection_match_physical_endpoint(monkeypatch):
    ...
    install_devices(monkeypatch, devices, {0: "MME", 1: "Windows WASAPI"})
    assert (audio_service._low_latency_equivalent(0, 'input') == 1) and (audio_service._matching_output_for_input(1, None) == 2) and (audio_service._low_latency_equivalent(3, 'output') == 3)
...
def test_asio_matching_rejects_empty_or_unrelated_names(monkeypatch):
    ...
    assert (audio_service._matching_asio_device_index(None, 'input') is None) and (audio_service._matching_asio_device_index('Focusrite ASIO', 'input') is None) and (audio_service._matching_asio_device_index('Generic ASIO', 'output') is None)
    ...
    assert (audio_service.preferred_input_device(None, 'auto') == 3) and (audio_service.preferred_output_device(0, 'asio', None, 'Studio') == 4)
...
def test_device_lists_expose_capabilities_and_host_api(monkeypatch):
    ...
    install_devices(monkeypatch, devices, {0: "MME", 1: "ASIO"})
    assert (audio_service.list_input_devices() == [{'index': 0, 'name': 'Microphone [MME]', 'max_input_channels': 1, 'default_samplerate': 48000, 'host_api': 'MME', 'is_asio': False}, {'index': 1, 'name': 'Interface [ASIO]', 'max_input_channels': 2, 'default_samplerate': 48000, 'host_api': 'ASIO', 'is_asio': True}]) and ([item['index'] for item in audio_service.list_output_devices()] == [1])
...
def test_monitor_worker_command_is_serializable(monkeypatch, tmp_path):
    ...
    assert (command[1:4] == ['-m', 'app.services.monitor_worker', '--config']) and (json.loads(command[-1]) == options)
