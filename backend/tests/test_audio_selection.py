...
def test_asio_bridge_path_and_driver_enumeration(monkeypatch, tmp_path):
    ...
    assert audio_service._asio_bridge_path() == bridge
    ...
    monkeypatch.setattr(audio_service.config, "IS_FROZEN", False)
    monkeypatch.setattr(audio_service.config, "PROJECT_ROOT", tmp_path)
...
def test_device_token_latency_and_asio_hint_normalization():
    assert audio_service._device_tokens("USB Audio Device Microphone Pro") == {
        "usb",
        "microphone",
        "pro",
    }
    assert audio_service._device_latency({"default_low_input_latency": "0.01"}, "input") == 0.01
    assert audio_service._device_latency({"default_low_input_latency": -1}, "input") == 0
    assert audio_service._device_latency({"default_low_input_latency": "bad"}, "input") == 1
    assert audio_service._asio_device_hint(None) == ""
    assert audio_service._asio_device_hint("  Focusrite USB ASIO Driver ") == "focusrite usb"
...
def test_resolved_device_prefers_default_then_low_latency_host(monkeypatch):
    ...
    assert audio_service._resolved_device_index(0, "input") == 0

    monkeypatch.setattr(audio_service.sd.default, "device", (-1, -1))
    assert audio_service._resolved_device_index(None, "input") == 1
    with pytest.raises(RuntimeError, match="No output"):
        audio_service._resolved_device_index(None, "output")
...
def test_low_latency_and_duplex_selection_match_physical_endpoint(monkeypatch):
    ...
    install_devices(monkeypatch, devices, {0: "MME", 1: "Windows WASAPI"})
    assert audio_service._low_latency_equivalent(0, "input") == 1
    assert audio_service._matching_output_for_input(1, None) == 2

    # Explicit unrelated endpoints are not silently remapped to another device.
    assert audio_service._low_latency_equivalent(3, "output") == 3
...
def test_asio_matching_and_preferred_device_fallbacks(monkeypatch):
    ...
    assert audio_service._matching_asio_device_index("Focusrite USB ASIO", "input") == 1
    assert audio_service.preferred_input_device(None, "asio", "Focusrite USB ASIO") == 1
    assert audio_service.preferred_input_device(0, "asio", "Focusrite USB ASIO") == 0
    assert audio_service.preferred_output_device(1, "asio", None, "Focusrite USB ASIO") == 1
    assert audio_service.preferred_output_device(None, "asio", 0, None) == 0
    assert audio_service.preferred_sample_rate(0) == 48_000

    monkeypatch.setattr(audio_service, "_AUDIO_BACKEND_AVAILABLE", False)
    assert audio_service._matching_asio_device_index("Focusrite", "input") is None
    assert audio_service.preferred_input_device(4) == 4
    assert audio_service.preferred_output_device(output_device_id=5) == 5
    assert audio_service.preferred_sample_rate(0) == audio_service.config.RECORDING_SAMPLE_RATE
...
def test_asio_matching_rejects_empty_or_unrelated_names(monkeypatch):
    ...
    assert audio_service._matching_asio_device_index(None, "input") is None
    assert audio_service._matching_asio_device_index("Focusrite ASIO", "input") is None
    assert audio_service._matching_asio_device_index("Generic ASIO", "output") is None
    assert audio_service._is_wasapi_device(devices[0]) is False
    ...
    assert audio_service.preferred_input_device(None, "auto") == 3
    assert audio_service.preferred_output_device(0, "asio", None, "Studio") == 4
...
def test_device_lists_expose_capabilities_and_host_api(monkeypatch):
    ...
    assert [item["index"] for item in audio_service.list_output_devices()] == [1]
...
def test_monitor_worker_command_is_serializable(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(audio_service.config, "IS_FROZEN", False)
    monkeypatch.setattr(audio_service.config, "BASE_DIR", tmp_path)
    options = {"input_device_id": 1, "gain": 1.5}
    audio_service._start_monitor_worker(options)
    command = launch.call_args.args[0]
    assert command[1:4] == ["-m", "app.services.monitor_worker", "--config"]
