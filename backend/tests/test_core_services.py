...
def test_transactional_resource_deletion_restores_after_commit_failure(tmp_path):
    ...
    raises(RuntimeError, lambda: resource_deletion.delete_with_files(database, object(), [path]), match='commit failed')
...
def test_transactional_resource_deletion_tolerates_cleanup_failure(monkeypatch, caplog):
    database = Mock()
    patch_attrs(monkeypatch, resource_deletion, quarantine_paths=lambda _paths: {}, purge_quarantined_paths=Mock(side_effect=OSError('locked')))
...
def test_transactional_resource_deletion_preserves_restore_error_note(monkeypatch):
    database, failure = Mock(), RuntimeError('commit failed')
    database.commit.side_effect = failure
    patch_attrs(monkeypatch, resource_deletion, quarantine_paths=lambda _paths: {}, restore_quarantined_paths=Mock(side_effect=OSError('restore failed')))
...
def test_player_state_recovers_from_concurrent_insert(monkeypatch):
    database, winner = Mock(), models.PlaybackState(song_id='song', position_sec=1)
    patch_attrs(monkeypatch, player_service.repositories, get_playback_state=Mock(side_effect=[None, winner]))
    patch_attrs(monkeypatch, player_service, commit_refresh=Mock(side_effect=IntegrityError('insert', {}, Exception('duplicate'))))
...
def test_player_state_reraises_concurrent_insert_without_winner(monkeypatch):
    ...
    raises(IntegrityError, lambda: player_service.get_state(database, 'song'))
