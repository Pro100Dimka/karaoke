...
def test_syllables_from_overlapping_words_remain_chronological():
    words = [
        Word(0.0, 1.0, "мелодия", 0.95, 0),
        Word(0.4, 0.65, "да", 0.9, 1),
    ]

    result = align_syllables(words, [])

    assert [item.start for item in result] == sorted(item.start for item in result)
    assert [item.index for item in result] == list(range(len(result)))
    assert any(item.word_index == 1 for item in result)
...
def test_wide_vibrato_stays_one_note(amplitude, hz):
    ...
    pitch = [
        _frame(i * 0.01, 69 + amplitude * math.sin(2 * math.pi * hz * i * 0.01)) for i in range(100)
    ]
...
def test_sustained_real_transition_is_preserved(delta, split):
    ...
    pitch = [_frame(i * 0.01, 69 if i * 0.01 < split else 69 + delta) for i in range(100)]
    notes = build_vocal_notes(pitch, [syllable])
    assert len(notes) == 2
...
@pytest.mark.parametrize(
    "width,center",
    [(1, 35), (1, 37), (2, 39), (2, 41), (3, 43), (3, 45), (4, 47), (4, 49), (5, 51), (5, 53)],
)
def test_short_octave_error_does_not_create_note(width, center):
    syllable = Syllable(0, 1, "ла", 0, 0, 1)
    pitch = [_frame(i * 0.01, 81 if center <= i < center + width else 69) for i in range(100)]
    notes = build_vocal_notes(pitch, [syllable])
    assert len(notes) == 1
...
def test_clear_same_pitch_reattack_is_preserved(valley_width, depth):
    ...
    for i in range(100):
        ...
        if center <= i < center + valley_width:
            ...
            energy = 0.18
        pitch.append(_frame(i * 0.01, 69, energy))
    notes = build_vocal_notes(pitch, [syllable])
    assert len(notes) == 2
...
def test_fuzzy_chunk_overlap_removes_duplicate_boundary_words():
    assert (
        _merge_transcript_parts(["мое сердце поет", "сердце поёт для тебя"])
        == "мое сердце поет для тебя"
    )


def test_candidate_language_script_quality_prefers_expected_language():
    assert _transcript_quality(
        "я люблю тебя и возвращаюсь домой", 8, "Russian"
    ) > _transcript_quality("hello wrong language text", 8, "Russian")
...
def test_midi_can_be_built_even_when_asr_returns_no_syllables():
    pitch = [_frame(i * 0.01, 69.0 if i < 50 else 71.0) for i in range(100)]
...
def test_wide_vibrato_keeps_correct_pitch_center(seed):
    ...
    syllable = Syllable(0, 1, "ла", 0, 0, 1)
    pitch = [
        _frame(i * 0.01, 69 + amplitude * math.sin(math.tau * rate * i * 0.01 + phase))
        for i in range(100)
    ]
    notes = build_vocal_notes(pitch, [syllable])
    assert len(notes) == 1
