...
def write(path, values, rate=1000):
    sf.write(path, np.asarray(values, dtype=np.float32), rate, format="WAV")
...
def test_render_analysis_variant_success(monkeypatch, tmp_path):
    ...
    with pytest.raises(FileNotFoundError):
        vocal._render_analysis_variant(tmp_path / "missing", target, "filter")
...
def test_adaptive_gate_threshold_and_variants(monkeypatch, tmp_path):
    ...
    assert result == {"denoise": tmp_path / "denoise", "tail-suppressed": tmp_path / "tail"}
    assert "agate=threshold=" in render.call_args_list[1].args[2]
...
def frame(time, hz=220, confidence=0.8, voiced=True):
...
def test_score_pitch_track_empty_short_and_discontinuous():
    ...
    assert quality.jump_rate > 0 and quality.octave_flip_rate > 0
    assert quality.micro_run_rate > 0
    constant_time = vocal.score_pitch_track([frame(0), frame(0), frame(0)])
    assert np.isfinite(constant_time.score)


def quality(score=0, voiced=0.8, confidence=0.8, jump=0.1, micro=0.1, octave=0.1):
    return vocal.PitchTrackQuality(score, voiced, confidence, jump, micro, octave)


def test_pitch_quality_selection_rules():
    original = quality(score=0.5)
    assert not vocal.prefer_cleaned_pitch(original, quality(voiced=0.5))
    assert not vocal.prefer_cleaned_pitch(original, quality(confidence=0.6))
    better = quality(score=0.6, confidence=0.9, jump=0, micro=0, octave=0)
    assert vocal._relative_wins(better, original) > 1
    assert vocal.prefer_cleaned_pitch(original, better)
    with pytest.raises(ValueError, match="original"):
        vocal.choose_best_pitch_track({"clean": better})
    assert (
        vocal.choose_best_pitch_track(
            {"original": original, "bad": quality(voiced=0.1), "weak": quality(confidence=0.1)}
        )
        == "original"
    )
    assert vocal.choose_best_pitch_track({"original": original, "clean": better}) == "clean"
...
def test_rms_envelope_levels(tmp_path):
    ...
    assert len(envelope) > 0 and levels["peak"] > 0
    assert levels["crest_factor"] >= 1 and levels["clipped_sample_ratio"] >= 0
...
def test_analyze_vocal_residuals_short_and_detailed(monkeypatch):
    levels = {"clipped_sample_ratio": 0.001}
    short = np.ones(10)
