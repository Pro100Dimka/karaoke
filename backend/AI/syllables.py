...
def _boundary_scores(word: Word, frames: list[PitchFrame], count: int) -> list[float]:
    ...
    def scale(values: list[float]) -> float:
        ...
        return 1.0 if not positive else max(positive[len(positive) // 2], positive[-1] * 0.25, 1e-06)

    jump_scale, confidence_scale, energy_scale = scale([item[0] for item in candidates]), scale([item[1] for item in candidates]), scale([item[2] for item in candidates])
...
def _refine_proportional_bounds(
    ...
    span, margin, result, prev = max(step * 4.0, word_span / max(3.0, (len(expected) + 1) * 2.2)), max(step * 2.0, word_span / max(20.0, (len(expected) + 1) * 10.0)), [], word.start
