...
class PitchTrackQuality: score: float; voiced_ratio: float; mean_confidence: float; jump_rate: float; micro_run_rate: float; octave_flip_rate: float
...
def _render_analysis_variant(source: Path, target: Path, filter_graph: str) -> Path:
    ...
    try:
        ...
        source_info = sf.info(source); target_info = sf.info(temporary)
        if target_info.frames <= 0: raise AICoreError("Vocal preprocessing produced an empty WAV")
        if target_info.samplerate != source_info.samplerate: raise AICoreError("Vocal preprocessing changed the sample rate")
        duration_delta = abs(float(target_info.duration) - float(source_info.duration))
        if duration_delta > max(0.002, 1.5 / max(1, source_info.samplerate)): raise AICoreError(f"Vocal preprocessing changed duration by {duration_delta:.6f}s")
...
def _adaptive_gate_threshold(source: Path) -> float:
    try:
        audio, _sr = sf.read(source, dtype="float32", always_2d=False); values = np.abs(np.asarray(audio, dtype=np.float32).reshape(-1)); values = values[np.isfinite(values)]
        if values.size < 256: raise ValueError
        quiet = float(np.percentile(values, 35)); body = float(np.percentile(values, 75)); threshold = quiet + max(0.0, body - quiet) * 0.08; return max(0.001, min(0.03, threshold))
    except Exception: return 0.008
...
def score_pitch_track(frames: list[PitchFrame]) -> PitchTrackQuality:
    ...
    voiced = [frame for frame in frames if frame.voiced and frame.frequency > 0]; voiced_ratio = len(voiced) / len(frames)
    ...
    for index in range(1, len(frames)):
        left = frames[index - 1]; right = frames[index]
        if not (left.voiced and right.voiced and left.frequency > 0 and right.frequency > 0): continue
        transitions += 1; delta = abs(_midi(right.frequency) - _midi(left.frequency))
    ...
    micro_runs = sum(1 for length in run_lengths if length <= micro_limit); micro_run_rate, coverage_term = micro_runs / max(1, len(run_lengths)), min(voiced_ratio, 0.8) / 0.8
    score = (
...
def choose_best_pitch_track(
    ...
    original = qualities[original_key]; viable = {original_key: original}
...
def analyze_vocal_residuals(
    ...
    vocal_env, levels = _rms_envelope(vocals); instrumental_env, _ = _rms_envelope(instrumental); denoise_env, _ = _rms_envelope(denoised); tail_env, _ = _rms_envelope(tail_suppressed)
    ...
    centered = np.diff(vocal_env); centered -= np.mean(centered); denominator = float(np.dot(centered, centered)); echo_candidates: list[tuple[float, int]] = []
    for lag in range(5, min(31, len(centered) // 4)):
        correlation = float(np.dot(centered[:-lag], centered[lag:]) / max(1e-9, denominator)); echo_candidates.append((max(0.0, correlation), lag))
    echo_peak, echo_lag = max(echo_candidates, default=(0.0, 0))

    low, high = float(np.percentile(vocal_env, 30)), float(np.percentile(vocal_env, 70)); threshold = low + 0.35 * max(0.0, high - low); decay_ratios: list[float] = []
    for index in range(1, size - 12):
        if vocal_env[index - 1] > threshold >= vocal_env[index]:
            before = float(np.mean(vocal_env[max(0, index - 4) : index])); after = float(np.mean(vocal_env[index + 3 : index + 12])); decay_ratios.append(after / max(1e-9, before))
    ...
    envelope_correlation, denoise_attenuation, tail_attenuation, echo_score = float(np.dot(vocal_centered, instrumental_centered) / correlation_scale) if correlation_scale > 1e-12 else 0.0, max(0.0, 1.0 - _ratio(denoise_env, vocal_env)), max(0.0, 1.0 - _ratio(tail_env, vocal_env)), clamp01((echo_peak - 0.08) / 0.35); reverb_score, leakage_score, noise_score, clipping_score = clamp01(0.65 * decay_persistence + 0.35 * tail_attenuation), clamp01((envelope_correlation - 0.15) / 0.7), clamp01(denoise_attenuation / 0.25), clamp01(levels['clipped_sample_ratio'] / 0.002)
    return {
        "available": True,
        "levels": levels,
        "cleanup": {
