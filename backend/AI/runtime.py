"""Minimal production hardware detection and safe backend selection.

Only backends that already passed the project's quality gate are eligible.
The module intentionally contains no benchmarking or vendor-specific planner;
future adapters plug into BackendRegistry without changing engine call sites.
"""
...
class RuntimePlan:
    ...
    def describe(self) -> dict[str, object]:
        return {
            ...
            "selected": {name: spec.describe() for name, spec in self.selected.items()},
            "fallbacks": {
                name: [spec.key for spec in specs] for name, specs in self.fallbacks.items()
            },
...
def _available(spec: BackendSpec, hardware: HardwareProfile) -> bool:
    if spec.backend == "pytorch":
        return hardware.torch_available and (spec.device != "cuda" or hardware.cuda_available)
...
def _choose(
    ...
    candidates = AI_BACKEND_REGISTRY.candidates(model, quality=PRODUCTION_QUALITY)
    failed = _failed.get(model, set())
    for spec in candidates:
        if spec.key in failed or (preference == "cpu" and spec.device != "cpu"):
            continue
        if _available(spec, hardware):
            return spec
    return None
...
def configure_runtime(
    ...
    *,
    ...
    with _lock:
        if _plan is not None and not force and _plan.preference == selected_preference:
            return _plan
        hardware = detect_hardware(torch_module)
        selected: dict[str, BackendSpec] = {}
        fallbacks: dict[str, tuple[BackendSpec, ...]] = {}
        warnings: list[str] = []
        if selected_preference == "cuda" and not hardware.cuda_available:
            warnings.append("CUDA was requested but is unavailable; using safe CPU fallbacks")
