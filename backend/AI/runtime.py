...
class GPUInfo: name: str; vendor: str; memory_bytes: int = 0


@dataclass(frozen=True, slots=True)
class HardwareProfile: cpu: str; logical_cores: int; ram_bytes: int; gpus: tuple[GPUInfo, ...]; torch_available: bool; cuda_available: bool; cuda_version: str = ""
...
def _windows_gpu_names() -> tuple[str, ...]:
    ...
    try:
        ...
        root = r"SYSTEM\CurrentControlSet\Control\Video"; names: list[str] = []
...
def detect_hardware(torch_module=None) -> HardwareProfile:
    ...
    try:
        ...
        ram_bytes = int(psutil.virtual_memory().total); cpu = cpu or platform.uname().processor
    except (ImportError, OSError, AttributeError): pass

    torch_available, cuda_available, cuda_version = False, False, ''; cuda_gpu: GPUInfo | None = None
    try:
        torch = torch_module
        if torch is None:
            import torch

        torch_available = True; cuda_available = bool(torch.cuda.is_available()); cuda_version = str(getattr(getattr(torch, "version", None), "cuda", "") or "")
        if cuda_available:
            try:
                name = str(torch.cuda.get_device_name(0)); properties = torch.cuda.get_device_properties(0)
                cuda_gpu = GPUInfo(
                    name,
                    _vendor(name),
    ...
    except (ImportError, OSError, RuntimeError, AttributeError): cuda_available = False
...
def _cpu_thread_settings(logical_cores: int) -> tuple[int, int]:
    ...
    except (ImportError, OSError, TypeError, ValueError): pass
    raw_intra = os.getenv("KARAOKE_CPU_INTRAOP_THREADS", "auto").strip().lower()
    try: intra = physical if raw_intra in {"", "auto"} else int(raw_intra)
    except ValueError: intra = physical
    try: inter = int(os.getenv("KARAOKE_CPU_INTEROP_THREADS", "1").strip())
    except ValueError: inter = 1
    logical = max(1, logical_cores); return max(1, min(intra, logical)), max(1, min(inter, logical))
...
def _apply_cpu_tuning(hardware: HardwareProfile, torch_module=None) -> tuple[int, int] | None:
    ...
    intra, inter = _cpu_thread_settings(hardware.logical_cores); os.environ["OMP_NUM_THREADS"] = str(intra); os.environ["MKL_NUM_THREADS"] = str(intra)
    try:
        torch = torch_module
        if torch is None:
            import torch
...
def _preference(value: str | None) -> ComputePreference: normalized = (value or os.getenv("SONGAPP_DEVICE", "auto")).strip().lower(); return normalized if normalized in {"auto", "cuda", "cpu"} else "auto"
...
def configure_runtime(
    ...
    *,
    ...
    global _plan; selected_preference = _preference(preference)
    with _lock:
        if _plan is not None and not force and _plan.preference == selected_preference: return _plan
        hardware = detect_hardware(torch_module); selected: dict[str, BackendSpec] = {}; fallbacks: dict[str, tuple[BackendSpec, ...]] = {}; warnings: list[str] = []
        ...
        for model in MODEL_ROLES:
            ...
            if spec is None:
                warnings.append(f"No production backend is available for {model}"); continue
            selected[model] = spec; chain = AI_BACKEND_REGISTRY.fallback_chain(model, spec.key)
...
def mark_backend_failed(model: str, key: str, error: BaseException) -> BackendSpec | None:
    global _plan
    with _lock:
        _failed.setdefault(model, set()).add(key); plan = get_runtime_plan(); replacement = _choose(model, plan.preference, plan.hardware); selected = dict(plan.selected)
        fallbacks = dict(plan.fallbacks)
        if replacement is None:
            selected.pop(model, None); fallbacks.pop(model, None)
        else:
            selected[model] = replacement; chain = AI_BACKEND_REGISTRY.fallback_chain(model, replacement.key)
            fallbacks[model] = tuple(
                item for item in chain[1:] if item.quality_status in PRODUCTION_QUALITY
            )
...
def reset_runtime_for_tests() -> None:
    global _plan
    with _lock:
        _plan = None; _failed.clear()


def format_runtime_plan(plan: RuntimePlan | None = None) -> tuple[str, ...]:
    plan = plan or get_runtime_plan(); gpu, tuning = ', '.join((item.name for item in plan.hardware.gpus)) or 'none', _cpu_thread_settings(plan.hardware.logical_cores) if plan.preference == 'cpu' and env_flag('KARAOKE_CPU_TUNING') else None
    lines = (
