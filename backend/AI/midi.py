...
def _ticks(seconds: float, tempo: int) -> int:
    return int(round(mido.second2tick(max(0.0, seconds), TICKS_PER_BEAT, tempo)))
...
def write_midi(
    ...
    if not notes:
        raise InvalidArtifactError("Cannot write MIDI without notes")
    ...
    meta_track.append(mido.MetaMessage("track_name", name="Lyrics", time=0))
    meta_track.append(mido.MetaMessage("set_tempo", tempo=tempo, time=0))
    text_events: list[tuple[int, int, mido.Message]] = []
    for word in words:
        text_events.append(
    ...
    for syllable in syllables:
        text_events.append(
            (
                _ticks(syllable.start, tempo),
                1,
                mido.MetaMessage("marker", text=f"SYL:{syllable.text}", time=0),
            )
        )
    _append_absolute_events(meta_track, text_events)
    meta_track.append(mido.MetaMessage("end_of_track", time=0))

    note_track = mido.MidiTrack()
    midi.tracks.append(note_track)
    note_track.append(mido.MetaMessage("track_name", name="Vocal Melody", time=0))
    note_track.append(mido.Message("program_change", program=53, time=0))
    if pitch_bend:
        _append_bend_range(note_track, bend_range)

    note_events: list[tuple[int, int, mido.Message]] = []
    for note in notes:
        start_tick = _ticks(note.start, tempo)
        end_tick = max(start_tick + 1, _ticks(note.end, tempo))
        midi_note = max(0, min(127, int(note.midi_note)))
        ...
        if pitch_bend:
            ...
            for relative_time, cents in note.cents:
                ...
                if tick == previous_tick or value == previous_value:
                    continue
                note_events.append((tick, 1, mido.Message("pitchwheel", pitch=value, time=0)))
                previous_tick = tick
        ...
        if pitch_bend:
            note_events.append((end_tick, 1, mido.Message("pitchwheel", pitch=0, time=0)))
...
def validate_midi(path: str | Path) -> None:
    ...
    try:
        ...
        raise InvalidArtifactError(f"Invalid MIDI {source}: {exc}") from exc
