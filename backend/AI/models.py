...
class Word(TimeSpan):
    ...
    def __post_init__(self) -> None:
        ...
        if int(self.index) < 0:
            raise ValueError("word index cannot be negative")
...
class Syllable(TimeSpan):
    ...
    def __post_init__(self) -> None:
        ...
        text = str(self.text).strip()
        if not text:
            raise ValueError("syllable text cannot be empty")
        if int(self.word_index) < 0 or int(self.index) < 0:
            raise ValueError("syllable indices cannot be negative")
        object.__setattr__(self, "text", text)
...
class PitchFrame:
    ...
    def __post_init__(self) -> None:
        time = _finite(self.time, "pitch time")
        frequency = _finite(self.frequency, "frequency")
        energy = _finite(self.energy, "energy")
        if time < 0 or frequency < 0 or energy < 0:
            raise ValueError("pitch time, frequency and energy cannot be negative")
        confidence = _confidence(self.confidence)
        voiced = bool(self.voiced)
        if voiced and frequency <= 0:
            raise ValueError("voiced pitch frame requires positive frequency")
        if not voiced and frequency != 0:
            # Keep the data model unambiguous for all downstream stages.
            frequency = 0.0
            confidence = 0.0
...
class VocalNote(TimeSpan):
    ...
    def __post_init__(self) -> None:
        ...
        if self.syllable_index is not None and int(self.syllable_index) < 0:
            raise ValueError("syllable_index cannot be negative")
        ...
        for relative_time, cents in self.cents:
            ...
            if relative < 0 or relative + 1e-9 < previous:
                raise ValueError("pitch-bend events must be sorted and non-negative")
            if relative > (self.end - self.start) + 1e-6:
                raise ValueError("pitch-bend event lies outside note duration")
