...
def _ctc_viterbi_states(log_probs, target_ids: list[int], blank_id: int):
    ...
    target, states = torch.tensor(target_ids, dtype=torch.long, device=log_probs.device), torch.full((2 * len(target_ids) + 1,), int(blank_id), dtype=torch.long, device=log_probs.device)
    states[1::2] = target
    count, neg = int(states.numel()), torch.finfo(log_probs.dtype).min
    ...
    state, path = int(last_states[torch.argmax(previous[last_states])].item()), [0] * frames
    path[-1] = state
...
class CTCWordAligner:
    ...
    def align_lines(
        ...
        total, anchors = len(source) / sample_rate, anchor_windows or {}
        ...
        typical_expected, expected_floor = float(np.median(expected_values)) if expected_values else 1.0, max(0.18, float(np.percentile(expected_values, 10)) * 0.55) if expected_values else 0.25
        expected_ceiling = (
        ...
        for line_index, group in enumerate(groups):
            ...
            def usable(item, minimum=minimum, expected=expected, cursor=cursor) -> bool:
                ...
                line_span, min_span = absolute_words[-1].end - absolute_words[0].start, max(minimum, expected * 0.18)
                return (
