...
class OrtSessionMixin:
    ...
    def _load(self):
        if self._session is not None:
            self.last_initialization_sec = 0.0; return self._session
        availability = self.availability()
        if not availability.available: raise EngineUnavailableError(availability.reason)
        try: import onnxruntime as ort
        except (ImportError, OSError) as exc: raise EngineUnavailableError(self.unavailable_message) from exc
        if self.artifact is None: raise EngineUnavailableError(self.artifact_message)
        options = ort.SessionOptions(); options.graph_optimization_level = ort.GraphOptimizationLevel.ORT_ENABLE_ALL; self._configure_options(ort, options); started = time.perf_counter()
        ...
        self.last_initialization_sec = time.perf_counter() - started; self._providers = tuple(session.get_providers())
        ...
        self._session = session; return session

    def release(self) -> None: self._session = None; self._providers = (); self.last_initialization_sec = 0.0
