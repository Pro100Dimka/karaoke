...
def _language_name(language: str | None) -> str | None:
    ...
    value = str(language).strip(); return None if not value else _LANGUAGE_NAMES.get(value.lower(), value)
...
def _qwen_transcribe_language(language: str | None) -> str | None: name = _language_name(language); return name if name in _QWEN_TRANSCRIBE_LANGUAGES else None
...
def resolve_alignment_language(text: str, language: str | None = None) -> str:
    ...
    sample = str(text or ""); lowered = sample.lower()
...
def _words_from_items(items) -> list[Word]:
    ...
    for index, item in enumerate(items or []):
        token = _first(item, ("text", "word", "token"), ""); start = _first(item, ("start", "start_time", "begin")); end = _first(item, ("end", "end_time", "finish")); confidence = _first(item, ("confidence", "score", "probability"), 1.0)
...
def _normalize_singing_audio(y: np.ndarray) -> np.ndarray:
    ...
    audio -= float(np.mean(audio)); peak = float(np.max(np.abs(audio)))
    if peak <= 1e-7: return audio
    rms = float(np.sqrt(np.mean(audio * audio) + 1e-12)); gain = min(8.0, max(1.0, 0.09 / max(rms, 1e-6))); audio *= gain; peak = float(np.max(np.abs(audio)))
...
def _singing_chunk_windows(
    ...
    hop = max(1, int(sr * 0.025)); frame, count = max(hop, int(sr * 0.05)), max(1, (len(y) + hop - 1) // hop); rms = np.empty(count, dtype=np.float32)
    for index in range(count):
        start = index * hop; chunk = y[start : min(len(y), start + frame)]; rms[index] = float(np.sqrt(np.mean(chunk * chunk) + 1e-12)) if chunk.size else 0.0

    target, search, minimum, maximum, starts, cursor_sec = 16.0, 3.5, 7.0, 22.0, [0], 0.0
    while total_sec - cursor_sec > maximum:
        ideal = cursor_sec + target; lo = max(cursor_sec + minimum, ideal - search); hi = min(cursor_sec + maximum, ideal + search); lo_i = max(0, int(lo * sr / hop))
        hi_i = min(len(rms) - 1, int(hi * sr / hop)); region = rms[lo_i : hi_i + 1]; floor = float(np.min(region)); spread = max(1e-8, float(np.max(region) - floor))
        scores = (region - floor) / spread
        if activity_hints:
            ordered_hints = sorted(activity_hints); hint_starts = [item[0] for item in ordered_hints]
            for local_index in range(len(scores)):
                timestamp = (lo_i + local_index) * hop / sr; pos = bisect_right(hint_starts, timestamp + 0.05) - 1; voiced = pos >= 0 and ordered_hints[pos][1] + 0.05 >= timestamp
                if voiced: scores[local_index] += 1.20
        local = int(np.argmin(scores)); cut_sec = (lo_i + local) * hop / sr; cut_sample = max(starts[-1] + int(minimum * sr), min(len(y), int(cut_sec * sr))); starts.append(cut_sample)
        cursor_sec = cut_sample / sr

    starts.append(len(y)); chunks: list[tuple[np.ndarray, float, float]] = []; overlap = int(0.42 * sr)
    for chunk_index, (left, right) in enumerate(zip(starts, starts[1:], strict=False)):
        padded_left = left if chunk_index == 0 else max(0, left - overlap); padded_right = right if chunk_index == len(starts) - 2 else min(len(y), right + overlap); segment = y[padded_left:padded_right]
...
def _singing_chunks(y: np.ndarray, sr: int, activity_hints: list[tuple[float, float]] | None=None) -> list[np.ndarray]: return [audio for audio, _start, _end in _singing_chunk_windows(y, sr, activity_hints)]
...
def _asr_line_anchor_windows(
    ...
    lyric_tokens: list[str] = []; line_ranges: list[tuple[int, int]] = []
    for group in groups:
        start = len(lyric_tokens); lyric_tokens.extend(_normalized_match_tokens(group)); line_ranges.append((start, len(lyric_tokens)))

    asr_tokens: list[str] = []; asr_times: list[tuple[float, float]] = []
    for raw_start, raw_end, text in sorted(segments, key=lambda item: (item[0], item[1])):
        tokens = _normalized_match_tokens(text)
        if not tokens: continue
        start = max(0.0, float(raw_start)); end = max(start + 0.05, float(raw_end)); span = end - start; weights = [max(1, len(token)) for token in tokens]
        total = max(1, sum(weights)); cursor = 0
        for token, weight in zip(tokens, weights, strict=False):
            token_start = start + span * cursor / total; cursor += weight; token_end = start + span * cursor / total; asr_tokens.append(token)
            asr_times.append((token_start, token_end))
    if not lyric_tokens or not asr_tokens: return {}

    matcher = SequenceMatcher(None, lyric_tokens, asr_tokens, autojunk=False); token_map: dict[int, tuple[float, float]] = {}
    for block in matcher.get_matching_blocks():
        for offset in range(block.size):
            li = block.a + offset; ai = block.b + offset
            if 0 <= ai < len(asr_times): token_map[li] = asr_times[ai]

    result: dict[int, tuple[float, float, float]] = {}
    for line_index, (left, right) in enumerate(line_ranges):
        matched = [token_map[index] for index in range(left, right) if index in token_map]; token_count = max(1, right - left); score = len(matched) / token_count
        if not matched: continue
        start = min(item[0] for item in matched); end = max(item[1] for item in matched)

        line_tokens = tokenize(groups[line_index]); expected_span = _expected_sung_phrase_duration(line_tokens); matched_span = max(0.0, end - start); missing_span = max(0.0, expected_span - matched_span)
        profile = _line_timing_profile(line_tokens); margin = max(profile["candidate_slack"], missing_span / 2.0); result[line_index] = (max(0.0, start - margin), end + margin, score)
    return result
...
def _complete_line_anchor_windows(
    ...
    baseline: dict[int, tuple[float, float, float]] = {}; cursor = 0
    for line_index, group in enumerate(groups):
        count = len(tokenize(group)); row = baseline_words[cursor : cursor + count]; cursor += count
        if not row: continue
        baseline[line_index] = (
    ...
    completed: dict[int, tuple[float, float, float]] = {}; provenance: dict[int, str] = {}; raw, normalized_rows = dict(raw_windows or {}), [tuple(_normalized_match_tokens(group)) for group in groups]; ambiguous_lines: set[int] = set()
    for left_index, left_tokens in enumerate(normalized_rows):
        ...
        for right_index, right_tokens in enumerate(normalized_rows):
            ...
            if left_tokens == right_tokens:
                ambiguous_lines.add(left_index); break
    ...
    for line_index, group in enumerate(groups):
        base = baseline.get(line_index); observed = raw.get(line_index)
        if base is None:
            if observed is not None:
                completed[line_index] = observed; provenance[line_index] = "asr"
            continue
        b0, b1, bscore = base
        if observed is None:
            completed[line_index] = base; provenance[line_index] = "vocal_baseline"; continue

        a0, a1, ascore = observed; profile = _line_timing_profile(tokenize(group)); baseline_mid = (b0 + b1) / 2.0
        if line_index not in ambiguous_lines:
            ...
            provenance[line_index] = "asr_unique"; continue
        asr_mid = (float(a0) + float(a1)) / 2.0; baseline_span = max(1e-9, b1 - b0)
        ...
        if abs(asr_mid - baseline_mid) > occurrence_tolerance:
            completed[line_index] = base; provenance[line_index] = "vocal_baseline_rejected_asr_repeat"; continue

        reliability = clamp01(float(ascore)); centre = baseline_mid * (1.0 - reliability) + asr_mid * reliability; asr_span = max(1e-9, float(a1) - float(a0))
        span = max(
            baseline_span,
            min(asr_span, baseline_span + float(profile["expected"])),
        )
        start = max(0.0, centre - span / 2.0); end = min(duration_sec, centre + span / 2.0)
        if end <= start:
            completed[line_index] = base; provenance[line_index] = "vocal_baseline"
        else:
            completed[line_index] = (start, end, max(bscore, reliability)); provenance[line_index] = "asr_vocal_blend"
    ...
    for line_index in range(len(groups)):
        ...
        if line_index > 0 and (prev := completed.get(line_index - 1)) is not None:
            prev_mid = (prev[0] + prev[1]) / 2.0; start = max(start, prev_mid)
        if line_index + 1 < len(groups) and (nxt := completed.get(line_index + 1)) is not None:
            next_mid = (nxt[0] + nxt[1]) / 2.0; end = min(end, next_mid)
        if end <= start:
            completed[line_index] = baseline.get(line_index, current); provenance[line_index] = "vocal_baseline_monotonic_repair"
        else:
...
def _activity_bounds(source: np.ndarray, sample_rate: int, duration_sec: float, minimum_span: float = 0.08) -> tuple[float, float]: active = _activity_quantile_times(source, sample_rate); start = max(0.0, float(active[0])) if active else 0.0; end = min(duration_sec, float(active[-1])) if active else duration_sec; return (0.0, duration_sec) if end <= start + minimum_span else (start, end)
...
def _lossless_canonical_alignment(
    ...
    line_tokens = [tokenize(group) for group in groups]; all_tokens = [token for tokens in line_tokens for token in tokens]
    if not all_tokens or duration_sec <= 0.04: return []

    active_start, active_end = _activity_bounds(source, sample_rate, duration_sec)

    line_minimum = [max(0.08, _minimum_sung_phrase_duration(tokens)) for tokens in line_tokens]; line_expected, available, minimum_total = [max(line_minimum[index], _expected_sung_phrase_duration(tokens)) for index, tokens in enumerate(line_tokens)], max(0.08, active_end - active_start), sum(line_minimum)

    if minimum_total > available + 1e-6:
        active_start = 0.0; active_end = duration_sec; available = max(0.08, duration_sec)
    ...
    if expected_total <= available:
        ...
        shortage = expected_total - available; ratio = min(1.0, shortage / max(1e-9, reducible))
        durations = [
            line_expected[i] - (line_expected[i] - line_minimum[i]) * ratio
            for i in range(len(line_tokens))
        ]

    used = sum(durations); remaining, gap_count = max(0.0, available - used), max(1, len(line_tokens) - 1); base_gap = min(1.15, remaining / gap_count) if len(line_tokens) > 1 else 0.0; remaining -= base_gap * (len(line_tokens) - 1)

    extra_gaps = [0.0] * max(0, len(line_tokens) - 1)
    if remaining > 1e-6 and extra_gaps:
        regions = _vocal_activity_regions(source, sample_rate, join_gap=0.55); pauses = []
        ...
        if pauses:
            nominal_starts = []; acc = active_start
            for duration_value in durations:
                nominal_starts.append(acc); acc += duration_value + base_gap
        ...
        if remaining > 1e-6:
            share = remaining / len(extra_gaps); extra_gaps = [value + share for value in extra_gaps]

    line_windows: list[tuple[float, float]] = []; cursor = active_start
    for index, duration_value in enumerate(durations):
        start = cursor; end = min(active_end, start + duration_value)
        if end <= start + 0.039:
            return [
    ...
    for line_index, tokens in enumerate(line_tokens):
        if not tokens: continue
        start, end = line_windows[line_index]; token_weights = [_vowel_weighted_length(token) for token in tokens]; total_weight = max(1, sum(token_weights)); cursor_weight = 0.0
        for token, weight in zip(tokens, token_weights, strict=True):
            word_start = start + (end - start) * cursor_weight / total_weight; cursor_weight += weight; word_end = start + (end - start) * cursor_weight / total_weight; output.append(Word(word_start, word_end, token, 0.008, len(output)))
    return output
...
def _atomic_line_acoustic_alignment(
    ...
    line_tokens = [tokenize(group) for group in groups]; canonical, line_count = [token for row in line_tokens for token in row], len(line_tokens)
    ...
    def physical_word_minimum(token: str) -> float: chars = max(1, len(_normalize_match_token(token))); return max(0.075, min(0.42, 0.05 + 0.03 * chars))
    ...
    line_max, typical_line_min = [min(duration_sec, max(line_expected[i] * 2.25 + line_min[i] * 0.35, line_min[i] * 2.8, line_expected[i])) for i in range(line_count)], float(np.median([value for value in line_min if value > 0.0])) if line_min else 0.0; line_gap_floor = max(1.0 / max(1, sample_rate), typical_line_min / max(24.0, line_count * 2.0))
    ...
    for line_index, result in enumerate(ctc_lines or []):
        ...
        expected = line_tokens[line_index]; actual = list(getattr(result, "words", ()) or ()); raw_ctc_words += len(actual)
        ...
        if span < line_min[line_index] * 0.65 or span > line_max[line_index] * 1.35: continue
        confidence = max(
            float(getattr(result, "confidence", 0.0) or 0.0),
            sum(word.confidence for word in words) / len(words),
        )
        priority = 10000.0 + 5000.0 * clamp01(confidence); candidates.setdefault(line_index, []).append((words, "ctc", priority))

    qwen = [word for word in (qwen_words or []) if _normalize_match_token(word.text)]
    if qwen:
        global_to_line = [(line_index, local_index) for line_index, tokens in enumerate(line_tokens) for local_index in range(len(tokens))]; per_line: dict[int, dict[int, Word]] = {}
        for canonical_index, word in _matched_word_indices(qwen, canonical):
            if canonical_index < len(global_to_line):
                line_index, local_index = global_to_line[canonical_index]; per_line.setdefault(line_index, {})[local_index] = word
        ...
        for li, mapping in per_line.items():
            ...
            if any(
                ...
                continue
            span = words[-1].end - words[0].start
            if span < line_min[li] * 0.65 or span > line_max[li] * 1.35: continue
            mean_conf = sum(word.confidence for word in words) / len(words); candidates.setdefault(li, []).append((words, "qwen", 1000.0 + 1000.0 * mean_conf))

    best: list[tuple[int, list[Word], str, float]] = []
    for li in sorted(candidates):
        words, kind, priority = max(candidates[li], key=lambda item: item[2]); best.append((li, words, kind, priority))
    ...
    if best:
        scores: list[float] = []; prev: list[int] = []
        for pos, (li, words, _kind, priority) in enumerate(best):
            best_score = priority; best_prev = -1
            for ppos in range(pos):
                pli, pwords, pkind, ppriority = best[ppos]; required = skipped_minimum(pli, li)
                if words[0].start < pwords[-1].end + required - 1e-6: continue
                score = scores[ppos] + priority
                if score > best_score:
                    best_score = score; best_prev = ppos
            scores.append(best_score); prev.append(best_prev)

        end_pos = max(range(len(best)), key=lambda pos: scores[pos]); selected_positions = []
        while end_pos >= 0:
            selected_positions.append(end_pos); end_pos = prev[end_pos]
        selected_positions.reverse(); selected = [best[pos] for pos in selected_positions]

    selected_by_line, active = {li: (words, kind, priority) for li, words, kind, priority in selected}, _activity_quantile_times(source, sample_rate); active_start, active_end = max(0.0, float(active[0])) if active else 0.0, min(duration_sec, float(active[-1])) if active else duration_sec
    if active_end <= active_start + 0.08: active_start, active_end = 0.0, duration_sec

    def edge_compatible(items):
        first_li, first_words, _kind, _priority = items[0]; prefix_need = sum(line_min[:first_li]) + line_gap_floor * first_li
        if first_words[0].start < active_start + prefix_need - 1e-6: return False
        last_li, last_words, _kind, _priority = items[-1]; suffix_need = sum(line_min[last_li + 1 :]) + line_gap_floor * (line_count - last_li - 1); return active_end >= last_words[-1].end + suffix_need - 1e-6

    while selected and not edge_compatible(selected):
        first_li, first_words, first_kind, first_priority = selected[0]; last_li, last_words, last_kind, last_priority = selected[-1]; prefix_need = sum(line_min[:first_li]) + line_gap_floor * first_li; suffix_need = sum(line_min[last_li + 1 :]) + line_gap_floor * (line_count - last_li - 1)
        first_bad = first_words[0].start < active_start + prefix_need - 1e-6; last_bad = active_end < last_words[-1].end + suffix_need - 1e-6
    ...
    def synthesize_range(
        ...
        indices = list(range(begin_line, end_line)); total_min, gap_count, available = sum((line_min[i] for i in indices)), len(indices) + 1, right_time - left_time; required = total_min + line_gap_floor * gap_count
        if available < required - 1e-6: return None

        desired = [line_expected[i] for i in indices]; desired_total, max_duration_budget = sum(desired), max(total_min, available - line_gap_floor * gap_count)
        if desired_total <= max_duration_budget:
            ...
            shortage = desired_total - max_duration_budget; ratio = min(1.0, shortage / max(1e-9, reducible))
            durations = [
                desired[k] - (desired[k] - line_min[indices[k]]) * ratio
                for k in range(len(indices))
            ]

        used = sum(durations); gap = max(line_gap_floor, (available - used) / gap_count); cursor = left_time + gap; result: list[list[Word]] = []
        for pos, li in enumerate(indices):
            duration_value = durations[pos]; line_end = min(right_time, cursor + duration_value); tokens = line_tokens[li]

            left_sample = max(0, int(cursor * sample_rate)); right_sample = min(len(source), max(left_sample + 1, int(line_end * sample_rate))); local_audio = source[left_sample:right_sample]; activity_words = _activity_fallback_words(tokens, local_audio, sample_rate)
            ...
            if (
                ...
                local = _proportional_words(tokens, max(0.04, line_end - cursor)); line_words = _offset_words(local, tokens, cursor, line_end, 0.008)
            result.append(line_words); cursor = line_end + gap
        return result
    ...
    for bidx in range(len(boundaries) - 1):
        left_li, left_start, left_end = boundaries[bidx]; right_li, right_start, right_end = boundaries[bidx + 1]; begin = left_li + 1; end = right_li
    ...
    output: list[Word] = []; ctc_count, qwen_count, interpolated_count, ctc_lines_kept, qwen_lines_kept = 0, 0, 0, 0, 0
    for li, tokens in enumerate(line_tokens):
        ...
        if source_kind == "ctc":
            ctc_lines_kept += 1; ctc_count += len(tokens)
        elif source_kind == "qwen":
            qwen_lines_kept += 1; qwen_count += len(tokens)
        else:
...
def _line_aware_canonical_alignment(
    ...
    line_tokens = [tokenize(group) for group in groups]; canonical = [token for row in line_tokens for token in row]
    ...
    baseline_lines: list[list[Word]] = []; baseline_offset, baseline_valid = 0, _canonical_words_match(baseline_words, canonical)
    for tokens in line_tokens:
        count = len(tokens)
    ...
    def word_minimum(token: str) -> float: chars = max(1, len(_normalize_match_token(token))); return max(0.09, min(0.44, 0.055 + 0.034 * chars))
    ...
    offsets: list[int] = []; cursor_index = 0
    for tokens in line_tokens:
        offsets.append(cursor_index); cursor_index += len(tokens)

    ctc_maps: list[dict[int, Word]] = [dict() for _ in line_tokens]; ctc_line_quality: list[float] = [0.0] * len(line_tokens)
    for line_index, result in enumerate(ctc_lines or []):
        if result is None or line_index >= len(line_tokens): continue
        expected = line_tokens[line_index]; actual = list(getattr(result, "words", ()) or ())
        if not expected or not actual: continue
        quality = clamp01(float(getattr(result, "confidence", 0.0) or 0.0)); ctc_line_quality[line_index] = quality
        for local_index, word in _matched_word_indices(actual, expected):
            if math.isfinite(float(word.start)) and math.isfinite(float(word.end)) and float(word.end) > float(word.start) + 0.009:
                bounded_start = max(0.0, min(duration_sec, float(word.start))); bounded_end = max(0.0, min(duration_sec, float(word.end)))
    ...
    qwen_maps: list[dict[int, Word]] = [dict() for _ in line_tokens]; qwen = [word for word in (qwen_words or []) if _normalize_match_token(word.text)]
    if qwen:
        ...
        for canonical_index, word in _matched_word_indices(qwen, canonical):
            ...
            line_index, local_index = global_to_line[canonical_index]; bounded_start = max(0.0, min(duration_sec, float(word.start))); bounded_end = max(0.0, min(duration_sec, float(word.end)))
    ...
    proposals: dict[int, tuple[float, float, str]] = {}; proposal_ends: dict[int, float] = {}; anchors = anchor_windows or {}
    for line_index, tokens in enumerate(line_tokens):
        if not tokens: continue
        cmap = ctc_maps[line_index]
        if cmap:
            matched = sorted(cmap.values(), key=lambda word: word.start); ratio = len(cmap) / max(1, len(tokens))
            proposals[line_index] = (
                max(0.0, matched[0].start),
                300.0 + 600.0 * max(ctc_line_quality[line_index], ratio),
                "ctc",
            )
            proposal_ends[line_index] = min(duration_sec, max(word.end for word in matched)); continue
        qmap = qwen_maps[line_index]
        if qmap:
            matched = sorted(qmap.values(), key=lambda word: word.start); ratio = len(qmap) / max(1, len(tokens)); mean_conf = sum(word.confidence for word in matched) / len(matched)
        ...
        if anchor is not None:
            ...
            if score >= 0.18:
                center = (float(left) + float(right)) / 2.0; estimated = center - line_expected[line_index] / 2.0
                proposals[line_index] = (
    ...
    while changed and selected_proposals:
        changed = False; ordered = sorted(selected_proposals)
        for left_idx, right_idx in zip(ordered, ordered[1:], strict=False):
            left_start, left_priority, left_kind = selected_proposals[left_idx]; right_start, right_priority, right_kind = selected_proposals[right_idx]
    ...
    fixed_items = sorted(fixed.items()); line_starts: list[float | None] = [None] * len(line_tokens)

    for pair_index in range(len(fixed_items) - 1):
        left_index, left_time = fixed_items[pair_index]; right_index, right_time = fixed_items[pair_index + 1]; begin = left_index + 1; end = right_index
        ...
        target_durations = [line_expected[idx] for idx in missing]; target_total = sum(target_durations); available = max(required, segment_end - segment_start)
        duration_budget = min(
            target_total, max(sum(line_minimum[idx] for idx in missing), available * 0.82)
        )
        scale = min(1.0, duration_budget / max(1e-6, target_total)); durations = [max(line_minimum[idx], line_expected[idx] * scale) for idx in missing]; used = sum(durations); silence = max(0.0, available - used)
        gap = silence / max(1, len(missing) + 1)

        cursor = segment_start + gap
        for local_pos, line_index in enumerate(missing):
            line_starts[line_index] = cursor; cursor += durations[local_pos] + gap

    cursor = active_start
    for line_index in range(len(line_tokens)):
        start = max(cursor, float(line_starts[line_index])); line_starts[line_index] = start; cursor = start + line_minimum[line_index] + line_gap_floor
    ...
    for line_index, _tokens in enumerate(line_tokens):
        ...
        if line_index + 1 < len(line_tokens):
            boundary_span = max(0.0, next_start - start); boundary_pad = clamp(boundary_span * 0.005, 0.008, 0.05); hard_end = min(duration_sec, next_start - boundary_pad)
        else:
    ...
    def build_line(
        ...
        for local_index, ctc_word in ctc_maps[line_index].items():
            ...
            if qwen_word is not None:
                ctc_center = (ctc_word.start + ctc_word.end) / 2.0; qwen_center = (qwen_word.start + qwen_word.end) / 2.0
        ...
        for local_index, (word, kind, priority) in candidates.items():
            wstart = max(start, min(end, word.start)); wend = max(wstart + 0.02, min(end, word.end))
        ...
        dropped = 0
        while True:
            ordered_indices = sorted(candidates); conflict: tuple[int, int] | None = None; previous_index: int | None = None; previous_word: Word | None = None
            for local_index in ordered_indices:
                ...
                if previous_index is not None and previous_word is not None:
                    ...
                    if word.start < previous_word.end + required - 1e-6:
                        conflict = (previous_index, local_index); break
                previous_index = local_index; previous_word = word
            ...
            if left_idx == right_idx:
                ...
                left = candidates[left_idx]; right = candidates[right_idx]
                if left[1] != right[1]:
                    source_rank = {"consensus": 3, "ctc": 2, "qwen": 1}; left_rank = source_rank.get(left[1], 0); right_rank = source_rank.get(right[1], 0); drop_idx = left_idx if left_rank < right_rank else right_idx
                elif left[2] != right[2]:
                    drop_idx = left_idx if left[2] < right[2] else right_idx
                else:
                    drop_idx = right_idx
            candidates.pop(drop_idx, None); dropped += 1
            if not candidates: break

        result: list[Word | None] = [None] * len(tokens); kinds: list[str | None] = [None] * len(tokens)
        ...
        if not candidates:
            left_sample = max(0, int(start * sample_rate)); right_sample = min(len(source), max(left_sample + 1, int(end * sample_rate))); local_audio = source[left_sample:right_sample]; fallback = _activity_fallback_words(tokens, local_audio, sample_rate)
        ...
        while position < len(tokens):
            if result[position] is not None:
                position += 1; continue
            run_start = position
            while position < len(tokens) and result[position] is None: position += 1
            run_end = position; left_word = result[run_start - 1] if run_start > 0 else None; right_word = result[run_end] if run_end < len(tokens) else None; left_time = left_word.end if left_word is not None else start
            right_time = right_word.start if right_word is not None else end; minima = [word_minimum(tokens[idx]) for idx in range(run_start, run_end)]; minimum_total = sum(minima)
            if right_time < left_time + minimum_total - 1e-6:
                ...
                local = _proportional_words(tokens, available); rebuilt: list[Word] = []
                for idx, word in enumerate(local):
                    word_start = start + word.start; word_end = min(duration_sec, start + word.end)
            ...
            extra = max(0.0, (right_time - left_time) - minimum_total); weights = [_vowel_weighted_length(_normalize_match_token(tokens[idx])) for idx in range(run_start, run_end)]; weight_total = max(1.0, sum(weights)); cursor = left_time
            for local_index, minimum, weight in zip(
                range(run_start, run_end), minima, weights, strict=True
            ):
                duration = minimum + extra * weight / weight_total; word_end = min(right_time, cursor + duration)
                result[local_index] = Word(
                    cursor,
                ...
                kinds[local_index] = "interpolated"; cursor = word_end

        final = [word for word in result if word is not None]; span, monotonic, micro, interior_gap = final[-1].end - final[0].start, all((right.start >= left.end - 1e-06 for left, right in zip(final, final[1:], strict=False))), any((word.end - word.start < min(0.075, word_minimum(word.text) * 0.6) for word in final)), max([right.start - left.end for left, right in zip(final, final[1:], strict=False)] or [0.0])
        if (
            not monotonic
            or micro
    ...
    def baseline_line_for(
        ...
        if len(source_line) == len(tokens):
            source_start = source_line[0].start; source_end = source_line[-1].end; source_span = max(0.04, source_end - source_start); start = max(previous_end + (0.015 if previous_end > 0 else 0.0), source_start)
            end_limit = min(duration_sec, max(start + 0.04, hard_end)); available = end_limit - start
            if available >= 0.04:
                target_span = min(source_span, available); rebuilt: list[Word] = []
                for local_index, word in enumerate(source_line):
                    rel_start = (word.start - source_start) / source_span; rel_end = (word.end - source_start) / source_span; word_start = start + target_span * rel_start; word_end = start + target_span * rel_end
                    rebuilt.append(
                        Word(
                            word_start,
    ...
    for line_index, (start, end) in enumerate(line_windows):
        ...
        if len(line) != len(tokens):
            stats["line_fallbacks"] += 1; previous_end = output[-1].end if output else 0.0
            hard_end = (
        ...
        _extend_words(output, line, tokens, duration_sec); stats["consensus"] += int(line_stats.get("consensus", 0)); stats["ctc"] += int(line_stats.get("ctc", 0)); stats["qwen"] += int(line_stats.get("qwen", 0))
        stats["interpolated"] += int(line_stats.get("interpolated", 0)); stats["dropped_word_anchors"] += int(line_stats.get("dropped", 0))
...
def _weaker_selected_anchor(
    ...
    _left_word, left_kind, left_priority = selected[left_idx]; _right_word, right_kind, right_priority = selected[right_idx]; source_strength = {"qwen": 1, "ctc": 2, "consensus": 3}
...
def _anchor_preserving_canonical_alignment(
    ...
    *,
    ...
    line_tokens = [tokenize(group) for group in groups]; tokens = [token for row in line_tokens for token in row]
    if not tokens or duration_sec <= 0.04: return [], _empty_alignment_stats()

    normalized_tokens = [_normalize_match_token(token) for token in tokens]; offsets: list[int] = []; offset = 0
    for row in line_tokens:
        offsets.append(offset); offset += len(row)
    ...
    candidates: dict[int, tuple[Word, str, float]] = {}; evidence_catalog: dict[int, dict[str, dict[str, float]]] = {}; rejection_reasons: Counter[str] = Counter()

    def add_candidate(idx: int, word: Word, kind: str, source_quality: float | None = None) -> None:
        start, end = float(word.start), float(word.end)
        if not math.isfinite(start) or not math.isfinite(end):
            rejection_reasons["non_finite_timestamp"] += 1; return
        if start < 0.0 or end > duration_sec + 0.10:
            rejection_reasons["out_of_song_bounds"] += 1; return
        if end <= start + 0.009:
            rejection_reasons["micro_or_negative_span"] += 1; return

        if anchor_windows and idx < len(token_line_index):
            line_index = token_line_index[idx]; window = anchor_windows.get(line_index)
            if window is not None and line_index < len(line_tokens):
                anchor_start, anchor_end, anchor_score = window; timing = _line_timing_profile(line_tokens[line_index]); reliability = clamp01(float(anchor_score))
                adaptive_margin = (
        ...
        confidence = clamp01(float(getattr(word, "confidence", 0.0) or 0.0)); quality, source_tiebreak, consensus_bonus = confidence if source_quality is None else max(confidence, clamp01(float(source_quality))), {'qwen': 1.0, 'ctc': 2.0, 'consensus': 3.0}.get(kind, 0.0), 2000.0 if kind == 'consensus' else 0.0; priority = consensus_bonus + quality * 1000.0 + source_tiebreak
    ...
    for line_index, result in enumerate(ctc_lines or []):
        if result is None or line_index >= len(line_tokens): continue
        expected = line_tokens[line_index]; base = offsets[line_index]; words = list(getattr(result, "words", ()) or ())
    ...
    if secondary:
        ...
        def temporally_agree(left: Word, right: Word) -> bool:
            ...
            left_span, right_span, left_mid, right_mid = max(timeline_quantum, float(left.end) - float(left.start)), max(timeline_quantum, float(right.end) - float(right.start)), (float(left.start) + float(left.end)) / 2.0, (float(right.start) + float(right.end)) / 2.0; return abs(left_mid - right_mid) <= max(left_span, right_span)
    ...
    def minimum_word_span(token: str, idx: int) -> float:
        ...
        chars = max(1, len(_normalize_match_token(token))); return max(0.10, min(0.48, 0.065 + 0.038 * chars))
        ...
        for idx, candidate in candidates.items():
            if candidate[1] == "qwen":
                if (candidate[0].end - candidate[0].start) + 1e-9 < minimum_word_span(
                    ...
                    rejection_reasons["weak_qwen_micro_anchor"] += 1; continue
                prefix_need = sum(adaptive_base_floors[:idx]) * adaptive_floor_scale; suffix_need = sum(adaptive_base_floors[idx + 1 :]) * adaptive_floor_scale
                if (
                    candidate[0].start + 1e-9 < prefix_need
                    or duration_sec - candidate[0].end + 1e-9 < suffix_need
                ):
                    rejection_reasons["weak_qwen_edge_capacity"] += 1; continue
    ...
    ordered = sorted(candidates.items()); dp_score: list[float] = []; dp_prev: list[int] = []
    for pos, (idx, (word, _kind, priority)) in enumerate(ordered):
        best_score = priority; best_prev = -1
        for prev_pos in range(pos):
            prev_idx, (prev_word, _prev_kind, _prev_priority) = ordered[prev_pos]; missing = max(0, idx - prev_idx - 1)
            if relaxed_gap_fit:
                if word.start < prev_word.end - 1e-6: continue
            else:
                required = minimum_run_span(prev_idx + 1, idx) if missing else 0.0; shortage = (prev_word.end + required) - word.start
                if shortage > (anchor_boundary_tolerance * 2.0) + 1e-6: continue
            score = dp_score[prev_pos] + priority
            if score > best_score:
                best_score = score; best_prev = prev_pos
        dp_score.append(best_score); dp_prev.append(best_prev)

    end_pos = max(range(len(ordered)), key=lambda pos: dp_score[pos]); selected_positions: list[int] = []
    while end_pos >= 0:
        selected_positions.append(end_pos); end_pos = dp_prev[end_pos]
    ...
    def nudge_conflicting_pair(left_idx: int, right_idx: int, required_gap: float) -> bool:
        left_word, left_kind, left_priority = selected[left_idx]; right_word, right_kind, right_priority = selected[right_idx]; shortage, left_room, right_room = left_word.end + required_gap - right_word.start, min(anchor_boundary_tolerance, max(0.0, left_word.end - left_word.start - anchor_min_duration)), min(anchor_boundary_tolerance, max(0.0, right_word.end - right_word.start - anchor_min_duration))
        if shortage > left_room + right_room + 1e-6: return False

        if left_priority + 1e-6 < right_priority:
            trim_left = min(shortage, left_room); trim_right = shortage - trim_left
        elif right_priority + 1e-6 < left_priority:
            trim_right = min(shortage, right_room); trim_left = shortage - trim_right
        else:
            trim_left = min(left_room, shortage / 2.0); trim_right = shortage - trim_left
            if trim_right > right_room:
                extra = trim_right - right_room; trim_right = right_room; trim_left += extra
    ...
    def activity_gap_words(
        ...
        for start, end in activity_regions:
            start = max(left_time, float(start)); end = min(right_time, float(end))
            region_floor = max(
                timeline_quantum,
                min(
        ...
        total_capacity = max(1e-9, sum(capacities)); states: dict[int, tuple[float, list[int]]] = {0: (0.0, [])}
        for region_index, capacity in enumerate(capacities):
            next_states: dict[int, tuple[float, list[int]]] = {}; target_fraction = capacity / total_capacity
            for token_index, (cost, allocation) in states.items():
                ...
                for take in range(0, remaining + 1):
                    ...
                    fraction = take / max(1, count); new_cost = cost + (fraction - target_fraction) ** 2; end_index = token_index + take; previous = next_states.get(end_index)
                    if previous is None or new_cost < previous[0]: next_states[end_index] = (new_cost, allocation + [take])
            states = next_states
            if not states: return None

        final_state = states[count]; allocation = final_state[1]

        output: list[Word] = []; token_index = run_start
        for (region_start, region_end), take in zip(regions, allocation, strict=True):
            if take <= 0: continue
            positions = list(range(token_index, token_index + take)); local_minima = [minimum_word_span(tokens[pos], pos) for pos in positions]; minimum_total = sum(local_minima); extra = max(0.0, (region_end - region_start) - minimum_total)
            weights = [_vowel_weighted_length(_normalize_match_token(tokens[pos])) for pos in positions]; total_weight = max(1.0, sum(weights)); cursor = region_start
            for pos, minimum, weight in zip(positions, local_minima, weights, strict=True):
                duration = minimum + extra * weight / total_weight; end = min(region_end, cursor + duration); output.append(Word(cursor, end, tokens[pos], 0.018, pos)); cursor = end
            token_index += take
    ...
    def reacquire_complete_lines(
        ...
        for line_index, (line_start, line_end) in enumerate(line_ranges):
            ...
            hinted_start, hinted_end, hint_score = window; local_start = max(left_time, 0.0, float(hinted_start)); local_end = min(right_time, duration_sec, float(hinted_end)); tokens_for_line = tokens[line_start:line_end]
            minimum_span = minimum_run_span(line_start, line_end)
            if not tokens_for_line or local_end - local_start < minimum_span: continue

            sample_left = max(0, min(len(source), int(round(local_start * sample_rate)))); sample_right = max(sample_left, min(len(source), int(round(local_end * sample_rate)))); local_audio = source[sample_left:sample_right]
            ...
            absolute_words: list[Word] = []; previous_end = local_start; valid = True; reacquired_confidence = max(0.02, min(0.30, float(hint_score) * 0.30))
            for local_index, local_word in enumerate(local_words):
                start = local_start + float(local_word.start); end = local_start + float(local_word.end); start = max(previous_end, start); end = min(local_end, end)
                if end <= start + 0.009:
                    valid = False; break
            ...
            for word in absolute_words:
                result[word.index] = word; source_kind[word.index] = "reacquired"
            changed = True
    ...
    def try_build() -> tuple[list[Word] | None, tuple[int, int] | None]:
        nonlocal last_built_sources; result: list[Word | None] = [None] * len(tokens); source_kind: list[str | None] = [None] * len(tokens)
        for idx, (word, kind, _priority) in selected.items():
            result[idx] = Word(word.start, word.end, tokens[idx], word.confidence, idx); source_kind[idx] = kind

        anchor_indices = sorted(selected)
        for left_idx, right_idx in zip(anchor_indices, anchor_indices[1:], strict=False):
            left_word = result[left_idx]; right_word = result[right_idx]; assert left_word is not None and right_word is not None; missing = right_idx - left_idx - 1
            required = (
                0.0
                if relaxed_gap_fit
        ...
        while index < len(tokens):
            if result[index] is not None:
                index += 1; continue
            run_start = index
            while index < len(tokens) and result[index] is None: index += 1
            run_end = index; count = run_end - run_start; left_idx = run_start - 1 if run_start > 0 else None; right_idx = run_end if run_end < len(tokens) else None
            left_word = result[left_idx] if left_idx is not None else None; right_word = result[right_idx] if right_idx is not None else None

            left_time = float(left_word.end) if left_word is not None else 0.0; right_time = float(right_word.start) if right_word is not None else duration_sec
            ...
            if activity_words is not None:
                for word in activity_words:
                    result[word.index] = word; source_kind[word.index] = "interpolated"
            else:
                minima = [minimum_word_span(tokens[pos], pos) for pos in range(run_start, run_end)]; minimum_total = sum(minima); available_span = max(timeline_quantum * max(1, count), right_time - left_time)
                if relaxed_gap_fit and minimum_total > available_span and minimum_total > 0.0:
                    scale = max(timeline_quantum, available_span) / minimum_total; minima = [max(timeline_quantum, value * scale) for value in minima]; minimum_total = sum(minima)
                extra = max(0.0, (right_time - left_time) - minimum_total)
                lexical_weights = [
                    _vowel_weighted_length(_normalize_match_token(tokens[pos]))
                    for pos in range(run_start, run_end)
                ]
                lexical_total = max(1.0, sum(lexical_weights)); cursor_time = left_time
                for pos, minimum, weight in zip(
                    ...
                    duration = minimum + extra * weight / lexical_total; start = cursor_time; end = min(right_time, start + duration)
                    result[pos] = Word(
                        start,
                        min(duration_sec, end),
                        tokens[pos],
                    ...
                    source_kind[pos] = "interpolated"; cursor_time = end

        final = [word for word in result if word is not None]; assert len(final) == len(tokens); last_built_sources = [str(kind or "interpolated") for kind in source_kind]; return final, None

    while True:
        built, conflict = try_build()
        if built is not None:
            kinds = list(last_built_sources); assert len(kinds) == len(tokens); consensus_count = sum(1 for kind in kinds if kind == "consensus")
            stats = {
            ...
            if consensus_count: stats["consensus"] = consensus_count
            reacquired_count = sum(1 for kind in kinds if kind == "reacquired")
            if reacquired_count: stats["reacquired"] = reacquired_count
            if debug_out is not None:
                debug_out.clear(); accepted_direct = sum(1 for kind in kinds if kind in {"consensus", "ctc", "qwen"}); debug_out["word_sources"] = kinds
                debug_out["word_candidates"] = [
                    {"index": idx, "text": tokens[idx], **evidence_catalog.get(idx, {})}
                    for idx in range(len(tokens))
                ]
                debug_out["candidate_acoustic_words"] = len(candidates); debug_out["accepted_acoustic_words"] = accepted_direct; debug_out["rejected_acoustic_words"] = max(0, len(candidates) - accepted_direct); debug_out["rejected_reasons"] = dict(rejection_reasons)
            return built, stats
...
def _activity_quantile_times(audio: np.ndarray, sample_rate: int) -> list[float]:
    hop = max(1, int(sample_rate * 0.04)); frame = max(hop, int(sample_rate * 0.08))
    if audio.size < frame: return [0.0, len(audio) / max(1, sample_rate)]
    values, times = [], []
    for start in range(0, max(1, len(audio) - frame + 1), hop):
        chunk = audio[start : start + frame]; values.append(float(np.sqrt(np.mean(chunk * chunk) + 1e-12))); times.append((start + frame / 2) / sample_rate)
    rms = np.asarray(values, dtype=np.float32); threshold = max(float(np.percentile(rms, 25)) * 1.8, float(np.percentile(rms, 90)) * 0.10); active = [time for time, value in zip(times, rms, strict=True) if value >= threshold]; return active if len(active) >= 2 else [0.0, len(audio) / sample_rate]


def _long_text_segments(audio, text: str) -> list[tuple[float, float, str]]:
    groups = _group_lyric_text(text)
    if not groups: return []
    source, sample_rate = load_mono(audio, 16000); duration_sec, active_times, weights = len(source) / sample_rate, _activity_quantile_times(source, sample_rate), [max(1, sum((len(token) for token in tokenize(group)))) for group in groups]; total, cursor = sum(weights), 0; output: list[tuple[float, float, str]] = []
    last_index = len(active_times) - 1
    for group, weight in zip(groups, weights, strict=True):
        start_fraction = cursor / total; cursor += weight; end_fraction = cursor / total; start = active_times[min(last_index, int(round(start_fraction * last_index)))]
        end = active_times[min(last_index, int(round(end_fraction * last_index)))]; timing = _line_timing_profile(tokenize(group)); start = max(0.0, start - timing["context"])
        end = min(
            duration_sec,
...
def _pathological_alignment(words: list[Word], span: float) -> bool:
    ...
    durations = [max(0.0, word.end - word.start) for word in words]; collapsed, compressed, implausible_long_word, implausible_held_word, overlaps, token_count, total_span = sum((duration <= 0.025 for duration in durations)), sum((duration <= 0.09 for duration in durations)), any((len(tokenize(word.text)[0]) >= 4 and duration <= 0.09 for word, duration in zip(words, durations, strict=True) if tokenize(word.text))), any((duration > min(3.2, max(1.8, 0.42 + len(tokenize(word.text)[0]) * 0.22)) for word, duration in zip(words, durations, strict=True) if tokenize(word.text))), sum((right.start < left.end - 0.015 for left, right in zip(words, words[1:], strict=False))), sum((max(1, len(tokenize(word.text))) for word in words)), max(0.0, words[-1].end - words[0].start)
    return (
        min(durations) < 0.02
...
def _proportional_words(tokens: list[str], span: float) -> list[Word]:
    weights = [max(2.0, _vowel_weighted_length(token)) for token in tokens]; total, offset, output = sum(weights), 0.0, []
    for index, (token, weight) in enumerate(zip(tokens, weights, strict=True)):
        word_start = span * offset / total; offset += weight; word_end = span * offset / total; output.append(Word(word_start, word_end, token, 0.05, index))
    return output
...
def _vocal_activity_regions(
    ...
    source, hop = np.asarray(audio, dtype=np.float32), max(1, int(sample_rate * 0.04)); frame = max(hop, int(sample_rate * 0.08))
    ...
    regions: list[tuple[float, float]] = []; region_start: float | None = None; last_active: float | None = None
    for index, value in enumerate(values):
        ...
        if value >= threshold:
            ...
            region_start = None; last_active = None
...
def _activity_fallback_words(
    ...
    if len(tokens) >= 10:
        ...
        maximum_span = sum(min(3.2, max(0.7, 0.42 + len(token) * 0.22)) for token in tokens); cluster_start = selected[0][0]; cluster_end = selected[-1][1]
    ...
    weights = [max(1, len(token)) for token in tokens]; total_weight, active_duration = max(1, sum(weights)), sum((max(0.0, end - start) for start, end in selected))
    if active_duration <= 0.02:
        start, end = selected[0][0], selected[-1][1]; local = _proportional_words(tokens, max(0.08, end - start))
        return [
    ...
    def active_offset_to_time(offset: float) -> float:
        ...
        for start, end in selected[:-1]:
            ...
            remaining -= span
        start, end = selected[-1]; return min(end, start + remaining)

    output: list[Word] = []; consumed = 0
    for index, (token, weight) in enumerate(zip(tokens, weights, strict=True)):
        start = active_offset_to_time(active_duration * consumed / total_weight); consumed += weight; end = active_offset_to_time(active_duration * consumed / total_weight); output.append(Word(start, max(start + 0.02, end), token, 0.05, index))
    return output
...
def _line_timing_profile(tokens: list[str]) -> dict[str, float]:
    minimum, expected = _minimum_sung_phrase_duration(tokens), _expected_sung_phrase_duration(tokens); context, cursor_backtrack, anchor_lead, candidate_slack, overlap_slack, min_word_duration = clamp(expected * 0.45, 0.75, 2.2), clamp(expected * 0.08, 0.1, 0.32), clamp(expected * 0.24, 0.35, 0.95), clamp(expected * 0.035, 0.05, 0.14), clamp(expected * 0.01, 0.012, 0.03), clamp(minimum / max(1, len(tokens)) * 0.18, 0.014, 0.03)
    minimum_window = max(
...
def enforce_segmented_timing_safety(
    ...
    output: list[Word] = []; offset, cursor, duration_sec = 0, 0.0, max(0.0, float(duration_sec))

    for segment in sorted(segments, key=lambda item: (float(item[0]), float(item[1]))):
        anchor_start, _anchor_end, text = segment; tokens = tokenize(text)
        if not tokens: continue
        count = len(tokens); group = words[offset : offset + count]; offset += count
        ...
        line_span = max(0.0, float(group[-1].end) - float(group[0].start)); minimum = _minimum_sung_phrase_duration(tokens)
        ...
        if valid:
            safe_group = []
            for word in group:
                start = max(cursor, float(word.start)); end = min(duration_sec, float(word.end)) if duration_sec > 0 else float(word.end)
                if end <= start + 0.019:
                    valid = False; break
                safe_group.append(Word(start, end, word.text, word.confidence, 0))
            if valid:
                _extend_words(output, safe_group); cursor = output[-1].end; continue

        start = max(cursor, max(0.0, float(anchor_start))); expected = _expected_sung_phrase_duration(tokens); target = max(minimum * 1.35, expected * 1.12)
    ...
    if offset < len(words):
        for word in words[offset:]:
            start = max(cursor, float(word.start)); end = max(start + 0.02, float(word.end))
            if duration_sec > 0: end = min(duration_sec, end)
            if end > start:
                output.append(Word(start, end, word.text, word.confidence, len(output))); cursor = end
...
def _timed_segment_fallback_words(tokens: list[str], span: float) -> list[Word]:
    ...
    if not tokens: return []
    characters = sum(len(token) for token in tokens); expected = 0.34 * len(tokens) + 0.024 * characters; usable = min(span, max(0.55, min(expected * 1.35, expected + 1.15))); return _proportional_words(tokens, usable)
...
def _long_text_line_fallback(
    ...
    *,
    ...
    if not tokens: return []
    search_span = max(0.08, float(search_span)); minimum_start = max(0.0, min(search_span, float(minimum_start)))
    ...
    if candidate_words:
        ...
        for region_start, region_end in regions:
            if region_end >= minimum_start + 0.02:
                start = max(minimum_start, region_start); break

    minimum, expected = _minimum_sung_phrase_duration(tokens), _expected_sung_phrase_duration(tokens); target, available = max(minimum * 1.35, expected * 1.12), max(0.08, search_span - start); span = min(available, target)
    return [
...
def _speech_focus_variant(audio: np.ndarray) -> np.ndarray:
    ...
    emphasized = source.copy(); emphasized[1:] = source[1:] - 0.91 * source[:-1]; mixed = source * 0.78 + emphasized * 0.22; return _normalize_singing_audio(mixed)
...
def _transcript_quality(text: str, duration_sec: float, language: str | None) -> float:
    value = _clean_transcript_part(text); tokens = tokenize(value)
    if not tokens: return 0.0
    duration_sec = max(0.5, float(duration_sec)); rate, score = len(tokens) / duration_sec, 1.0
    if rate < 0.12:
        score -= min(0.42, (0.12 - rate) * 2.5)
    elif rate > 4.0: score -= min(0.42, (rate - 4.0) * 0.18)
    long_tokens = sum(len(token) > 28 for token in tokens); score -= min(0.25, long_tokens * 0.08); repeated = sum(a.casefold() == b.casefold() for a, b in zip(tokens, tokens[1:], strict=False))
    if len(tokens) >= 4: score -= min(0.22, repeated / max(1, len(tokens) - 1) * 0.5)
    score -= max(0.0, 0.82 - _script_ratio(value, language)) * 0.45; return clamp01(score)
...
def _candidate_agreement(text: str, candidates: list[str]) -> float:
    ...
    for item in candidates:
        ...
        if not skipped_self and clean == target_clean:
            skipped_self = True; continue
        peer = " ".join(_token_key(token) for token in tokenize(item))
        if peer: peers.append(peer)
    if not peers: return 0.0
    scores = [SequenceMatcher(None, tokens, peer).ratio() for peer in peers]; return sum(scores) / len(scores)
...
def _select_candidate(candidates: list[str], duration_sec: float, language: str | None) -> str:
    ...
    for item in cleaned:
        quality = _transcript_quality(item, duration_sec, language); consensus = _candidate_agreement(item, cleaned); score = quality * 0.68 + consensus * 0.32
...
def _clean_transcript_part(text: str) -> str:
    ...
    for width in range(min(8, len(tokens) // 3), 2, -1):
        index = 0; cleaned: list[str] = []
        while index < len(tokens):
            block = tokens[index : index + width]; repeats = 1; cursor = index + width
            while tokens[cursor : cursor + width] == block:
                repeats += 1; cursor += width
...
def _trim_transcript_overlaps(parts: list[str]) -> list[str]:
    merged: list[str] = []; trimmed: list[str] = []
    for part in parts:
        tokens = _clean_transcript_part(part).split()
        if not tokens:
            trimmed.append(""); continue
        overlap = 0; maximum = min(10, len(merged), len(tokens))
        for size in range(maximum, 0, -1):
            left = [_token_key(item) for item in merged[-size:]]; right = [_token_key(item) for item in tokens[:size]]
            if left == right:
                overlap = size; break
            if size >= 2:
                similarities = [
                ...
                if min(similarities) >= 0.80 and sum(similarities) / size >= 0.90:
                    overlap = size; break
        owned = tokens[overlap:]; trimmed.append(" ".join(owned)); merged.extend(owned)
    return trimmed


def _merge_transcript_parts(parts: list[str]) -> str: return ' '.join((part for part in _trim_transcript_overlaps(parts) if part)).strip()


def _majority_language(values: list[str | None], requested: str | None) -> str | None:
    explicit = _language_name(requested)
    if explicit: return explicit
    normalized = [_language_name(value) for value in values if value]; normalized = [value for value in normalized if value]; return Counter(normalized).most_common(1)[0][0] if normalized else None
...
def _consensus_language(
    ...
    explicit = _language_name(requested)
    if explicit: return explicit
    combined = " ".join(texts).casefold(); cyrillic, latin = len(re.findall('[а-яёіїєґ]', combined)), len(re.findall('[a-z]', combined))
...
def _normalize_generation_config(model) -> None:
    ...
    for _ in range(5):
        ...
        seen.add(id(current)); generation_config = getattr(current, "generation_config", None)
...
def _load_qwen_pretrained(model_class, model_name: str, role: str, operation: str, *, cuda_kwargs=None, cpu_kwargs=None):
    ...
    device = select_torch_device(torch, role); cuda_kwargs, cpu_kwargs = cuda_kwargs or {}, cpu_kwargs or {}
    kwargs = {
    ...
    with profile_operation(operation):
        try: model = model_class.from_pretrained(model_name, **kwargs)
...
class Qwen3Transcriber(Transcriber):
    ...
    def __init__(self, model=get_model("asr").repo_id): self.model_name = model; self._model = None; self._device = "cpu"; self._call_batch_size = 1; self.last_language: str | None = None; self.last_segments: list[tuple[float, float, str]] = []; self._activity_hints: list[tuple[float, float]] = []

    def set_pitch_activity(self, frames) -> None:
        intervals: list[tuple[float, float]] = []; start, last = None, None
        for frame in sorted(frames or [], key=lambda item: item.time):
            ...
            if active:
                ...
                intervals.append((max(0.0, start - 0.02), last + 0.04)); start = last = None
        if start is not None and last is not None: intervals.append((max(0.0, start - 0.02), last + 0.04))
        self._activity_hints = intervals

    def _load(self):
        try: from qwen_asr import Qwen3ASRModel
        except ImportError as exc: raise EngineUnavailableError("Install the official qwen-asr package") from exc
        if self._model is None:
            ...
            self._call_batch_size = 2 if self._device.startswith("cuda") else 1; _normalize_generation_config(self._model)
        return self._model
    ...
    def _transcribe_batch(self, model, audios, language):
        ...
        try: return self._transcribe_batch_once(model, audios, language)
        except Exception as exc:
            fallback = fallback_torch_device("asr", self._device, exc)
            if fallback is None: raise
            self._model = None; return self._transcribe_batch_once(self._load(), audios, language)

    def transcribe(self, audio, language):
        model = self._load(); self.last_segments = []; requested_language, audio_path = _language_name(language), Path(audio) if isinstance(audio, (str, Path)) else None
        if audio_path is not None and not audio_path.is_file():
            kwargs = {"audio": str(audio)}; qwen_language = _qwen_transcribe_language(requested_language)
        ...
        y, sr = load_mono(audio, 16000); windows = _singing_chunk_windows(y, sr, self._activity_hints); chunks = [chunk for chunk, _start, _end in windows]
        ...
        initial_parts: list[str] = []; detected: list[str | None] = []; direct_words: list[Word] = []; parsed_items = []
        for raw in results[: len(inputs)]:
            item = _unwrap_single_result(raw) if raw is not None else {}; parsed_items.append(item); part = str(_first(item, ("text", "transcription"), "") or "").strip(); initial_parts.append(_clean_transcript_part(part))
        ...
        for index, (chunk, part, detected_language) in enumerate(
            ...
            chunk_duration = len(chunk) / sr; quality = _transcript_quality(part, chunk_duration, consensus_language)
            ...
            if quality < 0.78 or language_mismatch or not part:
                priority = quality - (0.30 if language_mismatch else 0.0); suspicious.append((priority, index))
        ...
        weakest = [index for _, index in sorted(suspicious) if index not in mismatched][:5]; retry_indices = sorted(mismatched | set(weakest)); retry_audio = [(_speech_focus_variant(chunks[index]), sr) for index in retry_indices]; retry_items = self._transcribe_batch(model, retry_audio, consensus_language)
        ...
        for index in second_round:
            chunk, trim = chunks[index], int(0.10 * sr); second_audio.append((_normalize_singing_audio(chunk[trim:-trim] if len(chunk) > 2 * trim else chunk), sr))
        ...
        owned_parts = _trim_transcript_overlaps(chosen_parts); text = "\n".join(part for part in owned_parts if part).strip()
        self.last_segments = [
            (start, end, part)
        ...
        self.last_language = resolve_alignment_language(text, consensus_language); return text, direct_words
...
def _adaptive_qwen_batch_size(clip_durations: list[float] | None = None) -> int:
    ...
    if configured:
        try: return max(1, min(16, int(configured)))
    ...
    try:
        ...
        if torch.cuda.is_available():
            free, _total = torch.cuda.mem_get_info(); free_gb = float(free) / (1024.0**3)
...
def _line_agreement_score(
    ...
    if not a or not b or len(a) != len(b) or not _canonical_words_match(a, [w.text for w in b]): return 0, 0.0
    deltas, scale = [abs(float(x.start) - float(y.start)) + abs(float(x.end) - float(y.end)) for x, y in zip(a, b, strict=True)], max(0.04, sum((max(0.02, float(x.end) - float(x.start)) for x in a)) / max(1, len(a))); score = sum(math.exp(-d / (2.0 * scale)) for d in deltas) / len(deltas); return len(a), float(clamp01(score))


def _write_alignment_clip(sf, root, name, source, sample_rate, start, end): left = max(0, int(start * sample_rate)); right = min(len(source), max(left + 1, int(end * sample_rate))); path = root / name; sf.write(path, source[left:right], sample_rate, subtype="PCM_16"); return path, source[left:right]


def _batched_alignment_results(align_many, jobs, batch_size, language):
    for pos in range(0, len(jobs), batch_size):
        batch = jobs[pos : pos + batch_size]; results = align_many([job["path"] for job in batch], [str(job["text"]) for job in batch], language); yield from zip(batch, results, strict=True)
...
def _record_merge_candidate(name, words, stats, canonical_tokens, attempts, candidates, debug=None):
    debug = debug or {}; ok = _canonical_words_match(words, canonical_tokens); attempts[name] = {"canonical": ok, "words": len(words), "stats": dict(stats)}
...
def _merge_evidence_rank(item):
    _name, words, stats, _debug = item; counts = _merge_stat_counts(stats, len(words))
    return (
...
class Qwen3ForcedAligner(Aligner):
    ...
    def __init__(self, model=get_model("aligner").repo_id): self.model_name = model; self._model = None; self._device = "cpu"; self._global_asr_segments: list[tuple[float, float, str]] = []; self._ctc = CTCWordAligner.from_environment(); self.last_alignment_diagnostics: dict[str, object] = {}
    ...
    def _load(self):
        try: from qwen_asr import Qwen3ForcedAligner
    ...
    def _run_alignment(self, **kwargs):
        try: return self._load().align(**kwargs)
        except Exception as exc:
            fallback = fallback_torch_device("aligner", self._device, exc)
            if fallback is None: raise
            self._model = None; return self._load().align(**kwargs)
    ...
    def align(self, audio, text, language):
        ...
        item = _unwrap_single_result(result); words = _words_from_items(item if isinstance(item, (list, tuple)) else _unwrap_items(item))
        if not words: raise InvalidArtifactError("Forced aligner returned no timed words")

        try: span = duration(audio)
        except (OSError, RuntimeError, ValueError): return words
        if _pathological_alignment(words, span):
            source, sample_rate = load_mono(audio, 16000); tokens = tokenize(text)
    ...
    def _align_many(self, audios, texts, language):
        if not audios: return []
        if len(audios) == 1:
            try: return [self.align(audios[0], texts[0], language)]
            except (InvalidArtifactError, RuntimeError, ValueError): return [[]]
        ...
        try:
            ...
            for audio_item, text_item in zip(audios, texts, strict=True):
                try: output.append(self.align(audio_item, text_item, language))
                except (InvalidArtifactError, RuntimeError, ValueError): output.append([])
            return output
    ...
    def align_segments(self, audio, segments, language):
        ...
        except ImportError as exc: raise EngineUnavailableError("soundfile is required for segmented alignment") from exc

        source, sample_rate = load_mono(audio, 16000); duration_sec = len(source) / sample_rate; output: list[Word] = []; word_sources: list[str] = []
        ...
        if ctc_available and self._ctc.available_for(
            ...
            except (EngineUnavailableError, InvalidArtifactError, RuntimeError, ValueError): ctc_line_results = [None] * len(ordered)
            finally: self._ctc.release()

        with tempfile.TemporaryDirectory(prefix="karaoke-align-") as temp_dir:
            root = Path(temp_dir); index = 0
            while index < len(ordered):
                prepared = []; batch_cursor = cursor
                for segment_index in range(index, min(len(ordered), index + batch_size)):
                    start, end, text = ordered[segment_index]; tokens = tokenize(text)
                    if not tokens:
                    ...
                    if anchor_start >= duration_sec - 0.01:
                        prepared.append(None); continue
                    anchor_end = min(duration_sec, max(anchor_start + 0.04, float(end))); anchor_span = max(0.0, anchor_end - anchor_start); plausible_anchor = _lrc_window_is_plausible(tokens, anchor_span); expected = _expected_sung_phrase_duration(tokens)

                    pre_roll = 0.0 if plausible_anchor else 1.25
                    post_roll = (
                        max(0.85, expected * 0.55) if plausible_anchor else max(5.0, expected * 2.4)
                    )
                    search_start = max(0.0, anchor_start - pre_roll); search_start = max(search_start, max(0.0, batch_cursor - 0.18))
                    search_end = min(
                        duration_sec,
                        max(
                    ...
                    left = max(0, min(len(source) - 1, int(search_start * sample_rate))); right = max(left + 1, min(len(source), int(search_end * sample_rate))); path = root / f"segment-{segment_index:03d}.wav"; sf.write(path, source[left:right], sample_rate, subtype="PCM_16")
                    prepared.append(
                        {
                ...
                for offset, item in enumerate(prepared):
                    ...
                    tokens = item["tokens"]; anchor_start = item["anchor_start"]; anchor_end = item["anchor_end"]; anchor_span = item["anchor_span"]
                    plausible_anchor = item["plausible_anchor"]; expected = item["expected"]; search_start = item["search_start"]; search_end = item["search_end"]
                    search_span = search_end - search_start; candidate = candidate_by_index.get(segment_index, [])

                    local_words: list[Word] = []; line_source = "interpolated"; candidate_ok = _segment_alignment_is_usable(candidate, tokens, search_span)
                    if candidate_ok:
                        absolute_start = search_start + candidate[0].start; absolute_end = search_start + candidate[-1].end; candidate_duration = absolute_end - absolute_start
                        candidate_ok = (
                    ...
                    if candidate_ok:
                        local_words = candidate; line_source = "qwen"
                    ...
                    if not local_words and ctc_result is not None and len(ctc_result.words) == len(tokens):
                        local_words = list(ctc_result.words); search_start = 0.0; search_end = duration_sec; line_source = "ctc"

                    if not local_words:
                        line_source = "interpolated"
                        if plausible_anchor:
                            local_words = _timed_segment_fallback_words(tokens, anchor_span); search_start = anchor_start; search_end = anchor_end
                        else:
                            fallback_start = max(cursor, anchor_start); room = max(0.08, search_end - fallback_start)
                            target = max(
                                _minimum_sung_phrase_duration(tokens) * 1.35,
                                expected * 1.12,
                            )
                            safe_span = min(room, target); local_words = _proportional_words(tokens, safe_span); search_start = fallback_start

                    line_words: list[Word] = []
                    for word in local_words:
                        word_start = max(cursor, search_start + float(word.start)); word_end = min(duration_sec, search_start + float(word.end))
                    ...
                    if len(line_words) != len(
                        ...
                        fallback_start = max(cursor, anchor_start); available = max(0.08, search_end - fallback_start)
                        safe_span = min(
                            available,
                            max(
                                _minimum_sung_phrase_duration(tokens) * 1.35,
    ...
    def align_long_text(self, audio, text, language):
        groups = _group_lyric_text(text); anchor_windows = _asr_line_anchor_windows(groups, self._global_asr_segments)
        if len(groups) <= 1: return self.align(audio, text, language)

        try: import soundfile as sf
        except ImportError as exc: raise EngineUnavailableError("soundfile is required for long-text alignment") from exc

        source, sample_rate = load_mono(audio, 16000); duration_sec = len(source) / sample_rate
        anchor_windows, anchor_window_sources = _complete_line_anchor_windows(
            groups, anchor_windows, source, sample_rate, duration_sec
        )
        output: list[Word] = []; cursor, ctc_lines, ctc_attempted, ctc_failure_reason = 0.0, [None] * len(groups), False, ''
        try:
            ctc_attempted = self._ctc.available_for(language, text)
            if ctc_attempted:
                ctc_lines = self._ctc.align_lines(audio, groups, language, anchor_windows)
            else:
                code = _language_code(language, text); resource = self._ctc.last_resource_diagnostics.get(code, {}); ctc_failure_reason = str(resource.get("reason", "CTC model unavailable"))
        except (EngineUnavailableError, InvalidArtifactError, RuntimeError, ValueError) as exc:
            ctc_lines = [None] * len(groups); ctc_failure_reason = f"{type(exc).__name__}: {exc}"
        finally: self._ctc.release()
        ...
        if require_ctc and ctc_language in {"ru", "uk"} and not ctc_attempted:
            resource = getattr(self._ctc, "last_resource_diagnostics", {}).get(ctc_language, {}); checked = resource.get("checked", []) if isinstance(resource, dict) else []
            checked_text = "; ".join(
        ...
        qwen_evidence_words: list[Word] = []; group_token_offsets: list[int] = []; _token_offset = 0
        for _group in groups:
            group_token_offsets.append(_token_offset); _token_offset += len(tokenize(_group))
        ...
        with tempfile.TemporaryDirectory(prefix="karaoke-align-lines-") as temp_dir:
            ...
            for line_index, line in enumerate(groups):
                tokens = tokenize(line); ctc_line = ctc_lines[line_index] if line_index < len(ctc_lines) else None
                if not tokens or (
                    ...
                    continue
                timing = _line_timing_profile(tokens); anchor = anchor_windows.get(line_index)
                if anchor is not None:
                    a0, a1, _score = anchor; search_start = max(0.0, a0 - timing["anchor_lead"])
                    search_end = min(
                        duration_sec,
                        max(a1 + timing["context"], search_start + timing["minimum_window"]),
                    )
                else:
                    prev_end = 0.0; next_start = duration_sec
            ...
            qwen_candidate_map: dict[int, list[Word]] = {}; qwen_window_map: dict[int, tuple[float, float]] = {}
            ...
            for job, result in _batched_alignment_results(self._align_many, qwen_jobs, batch_size, language):
                idx = int(job["line"]); qwen_candidate_map[idx] = list(result or []); qwen_window_map[idx] = (float(job["start"]), float(job["end"]))
            ...
            for line_index, line in enumerate(groups):
                ...
                lo = max(0, line_index - 1); hi = min(len(groups), line_index + 2); block_lines = groups[lo:hi]; block_tokens = [tokenize(x) for x in block_lines]
            ...
            for job, result in _batched_alignment_results(self._align_many, context_jobs, batch_size, language):
                flat, idx = list(result or []), int(job["line"]); offset = sum(len(x) for x in job["tokens"][: idx - int(job["lo"])]); count = len(job["tokens"][idx - int(job["lo"])]); piece = flat[offset : offset + count]
            ...
            self.last_alignment_diagnostics["qwen_batch_size"] = batch_size; self.last_alignment_diagnostics["qwen_batched_lines"] = len(qwen_jobs); self.last_alignment_diagnostics["context_recovered_words"] = context_recovered
            ...
            for line_index, line in enumerate(groups):
                ...
                timing = _line_timing_profile(tokens); ctc_line = ctc_lines[line_index] if line_index < len(ctc_lines) else None
                if ctc_line is not None:
                    ...
                    if (
                        ...
                        _extend_words(output, ctc_words); cursor = ctc_words[-1].end; continue

                qwen_fallback_lines += 1; anchor = anchor_windows.get(line_index)
                if anchor is not None:
                ...
                candidate: list[Word] = list(qwen_candidate_map.get(line_index, [])); local_words: list[Word] = []; candidate_window = qwen_window_map.get(line_index)
                if candidate_window is not None:
                    precomputed_start, precomputed_end = candidate_window; shift = precomputed_start - search_start; candidate_start = precomputed_start + candidate[0].start if candidate else 0.0; candidate_end = precomputed_start + candidate[-1].end if candidate else 0.0
                ...
                if not local_words:
                    ...
                    if candidate and candidate_window is not None:
                        precomputed_start, _precomputed_end = candidate_window; shift = precomputed_start - search_start
                ...
                line_words: list[Word] = []
                for word in local_words:
                    start = search_start + float(word.start); end = search_start + float(word.end); line_words.append(Word(start, end, word.text, word.confidence, 0))
                ...
                _extend_words(output, safe_line); cursor = safe_line[-1].end
        ...
        if raw_ctc_word_count > 0 or not _canonical_words_match(output, canonical_tokens):
            merge_candidates: list[tuple[str, list[Word], dict[str, int], dict[str, object]]] = []; merge_attempts: dict[str, dict[str, object]] = {}
        ...
        if not output:
            ...
            active_start = max(0.0, float(active[0])); active_end = min(duration_sec, max(active_start + 0.08, float(active[-1]))); all_tokens = [token for group in groups for token in tokenize(group)]
            if all_tokens and active_end > active_start + 0.04:
                weights = [max(1, len(token)) for token in all_tokens]; total_weight = max(1, sum(weights)); cursor_weight = 0; fallback: list[Word] = []
                span = active_end - active_start
                for index, (token, weight) in enumerate(zip(all_tokens, weights, strict=True)):
                    start = active_start + span * (cursor_weight / total_weight); cursor_weight += weight; end = active_start + span * (cursor_weight / total_weight); fallback.append(Word(start, end, token, 0.01, index))
...
def _low_confidence_regions(words: list[Word]) -> list[dict[str, object]]:
    if not words: return []
    confidences = np.asarray([float(w.confidence) for w in words], dtype=np.float32); positive = confidences[confidences > 0]
    if positive.size == 0: return []
    typical = float(np.median(positive)); floor, regions, current = max(0.015, typical * 0.3), [], []
    for word in words:
...
class UniformTextFallback(Transcriber, Aligner):
    ...
    def align(self, audio, text, language):
        ...
        if not tokens: return []
        weights = [max(1, len(token)) for token in tokens]; total_weight, cursor, output = sum(weights), 0.0, []
        for index, (token, weight) in enumerate(zip(tokens, weights, strict=False)):
            start = total_duration * cursor / total_weight; cursor += weight; end = total_duration * cursor / total_weight; output.append(Word(start, end, token, 0.05, index))
        return output
