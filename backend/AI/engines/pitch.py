...
class FCPEPitchEstimator(PitchEstimator):
    ...
    def __init__(
        ...
        *,
        ...
        self.sr = int(sr); self.hop = max(1, int(hop)); self.fmin = float(fmin); self.fmax = float(fmax)
        self._model = None; self._device = None; self._shadow_policy = shadow_policy or ShadowPolicy.from_env("KARAOKE_AI_FCPE_SHADOW"); self._shadow_backend_factory = shadow_backend_factory
        self._shadow_backend = None; self.last_shadow_diagnostics: dict[str, object] = {"selected": False}; self.last_shadow_frames: list[PitchFrame] = []
    ...
    def _load_model(self):
        try:
            import torch; import torchfcpe
        except ImportError as exc: raise EngineUnavailableError("Install torch and torchfcpe for FCPE") from exc
        if self._model is None:
            self._device = select_torch_device(torch, "fcpe")
            with profile_operation("model.load.fcpe"):
                try: self._model = torchfcpe.spawn_bundled_infer_model(device=self._device)
                except Exception as exc:
                    fallback = fallback_torch_device("fcpe", self._device, exc)
                    if fallback is None: raise
                    self._device = fallback; self._model = torchfcpe.spawn_bundled_infer_model(device=fallback)
        return torch, self._model

    @staticmethod
    def _infer(model, tensor, kwargs):
        try: return model.infer(tensor, **kwargs)
        except TypeError:
            compatible = {
                key: value
    ...
    def estimate(self, audio):
        torch, model = self._load_model(); y, _ = load_mono(audio, self.sr)
        if y.size == 0: return []
        ...
        with torch.inference_mode(), profile_operation("inference.fcpe"):
            try: result = self._infer(model, tensor, kwargs)
            except Exception as exc:
                fallback = fallback_torch_device("fcpe", self._device, exc)
                if fallback is None: raise
                self._model = None; torch, model = self._load_model()
        ...
        if isinstance(result, (tuple, list)):
            f0_tensor = result[0]; confidence_tensor = result[1] if len(result) > 1 else None
        else:
            f0_tensor = result; confidence_tensor = None
        ...
        step, energy_window = self.hop / self.sr if len(f0) == target_length else len(y) / self.sr / max(1, len(f0)), max(32, int(self.sr * 0.025)); output: list[PitchFrame] = []
        for index, value in enumerate(f0):
            hz = float(value); start = min(len(y), int(round(index * step * self.sr))); end = min(len(y), start + energy_window)
            energy = (
            ...
            if confidence is not None:
                conf = clamp01(float(confidence[index])); voiced = bool(valid and conf >= 0.05)
            else:
                voiced = bool(valid); conf = 1.0 if voiced else 0.0
            output.append(
                PitchFrame(
                    time=index * step,
        ...
        self._run_shadow(audio, model, tensor, y, target_length, output); return output
    ...
    def _shadow_key(self) -> str:
        ...
        try: return str(getattr(self._configured_shadow_class(), "key", "custom"))
        except EngineUnavailableError as exc: return f"invalid:{exc}"

    def _run_shadow(self, audio, model, tensor, y, target_length, production) -> None:
        selected = self._shadow_policy.selects(str(audio)); self.last_shadow_frames = []
        ...
        try:
            if self._shadow_backend is None:
                self._shadow_backend = self._create_shadow_backend(); self.last_shadow_diagnostics["shadow"] = self._shadow_key()
            mel = np.asarray(model.wav2mel(tensor, self.sr).detach().cpu(), dtype=np.float32); cent_table = np.asarray(model.model.cent_table.detach().cpu(), dtype=np.float32)
            candidate = self._shadow_backend.infer(
                mel,
                cent_table,
                target_length=target_length,
            )
            step = self.hop / self.sr; frames = []
            for index, value in enumerate(candidate.f0):
                hz = min(float(value), self.fmax); valid = np.isfinite(hz) and self.fmin <= hz <= self.fmax
                frames.append(
                    PitchFrame(
                        index * step,
            ...
            self.last_shadow_frames = frames; size = min(len(production), len(frames))
            voiced_equal = sum(
...
class PyinFallbackPitchEstimator(PitchEstimator):
    ...
    def __init__(self, sr=16000, hop_seconds=0.01, fmin=55, fmax=1400): self.sr = sr; self.hop_seconds = hop_seconds; self.fmin = fmin; self.fmax = fmax

    def estimate(self, audio):
        try: import librosa
        except ImportError as exc: raise EngineUnavailableError("Install librosa to use the pYIN fallback") from exc
        ...
        for index, timestamp in enumerate(times):
            hz = float(f0[index]) if np.isfinite(f0[index]) else 0.0; is_voiced = bool(voiced[index]) and hz > 0
            output.append(
                PitchFrame(
                    float(timestamp),
                    hz,
