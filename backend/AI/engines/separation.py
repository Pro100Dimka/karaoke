...
def _cpu_thread_settings() -> tuple[int, int]: return _runtime_cpu_thread_settings(max(1, os.cpu_count() or 1))
...
def _apply_torch_cpu_worker_tuning(torch, settings: tuple[int, int] | None) -> None:
    ...
    if (setter := getattr(torch, "set_num_interop_threads", None)) is not None:
        with suppress(RuntimeError): setter(inter)
    print(
...
def _run_persistent_msst_worker(
    ...
    try:
        ...
        with suppress(ValueError): sys.path.remove(str(engine_path))
        sys.stdout, sys.stderr = original_stdout, original_stderr
        for stream in owned_streams: stream.close()
    ...
    def separate(self, mix, vocals, instrumental, *, profile=None):
        ...
        with tempfile.TemporaryDirectory(
            ...
            if instrumental_candidates:
                ...
                if instrumental_rate != sample_rate: raise AICoreError("MSST instrumental sample-rate mismatch")
