...
class ResourceSnapshot: wall_time: float; rss_mb: float | None; cuda_allocated_mb: float | None; cuda_reserved_mb: float | None; warnings: tuple[str, ...] = ()


@dataclass(slots=True)
class OperationMetric: count: int = 0; elapsed_sec: float = 0.0; bytes: int = 0
...
def _torch_memory() -> tuple[float | None, float | None, list[str]]:
    ...
    except ImportError: warnings.append("torch is not installed")
    except RuntimeError as exc: warnings.append(f"Unable to read CUDA memory: {exc}")
    return None, None, warnings


def snapshot() -> ResourceSnapshot:
    warnings: list[str] = []; rss = None
    try:
        import psutil

        rss = psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024
    except ImportError: warnings.append("psutil is not installed")
    except OSError as exc: warnings.append(f"Unable to read process memory: {exc}")
    allocated, reserved, cuda_warnings = _torch_memory(); warnings.extend(cuda_warnings)
...
def environment_info() -> dict[str, object]:
    ...
    try:
        ...
        info["torch"] = torch.__version__; info["cuda_available"] = bool(torch.cuda.is_available()); info["torch_cuda"] = getattr(torch.version, "cuda", None)
        if torch.cuda.is_available():
            info["gpu"] = torch.cuda.get_device_name(0); properties = torch.cuda.get_device_properties(0); info["vram_mb"] = round(properties.total_memory / 1024 / 1024); capability = getattr(torch.cuda, "get_device_capability", lambda _index: None)(0)
            if capability is not None: info["gpu_compute_capability"] = list(capability)
    except ImportError as exc:
        info["cuda_available"] = False; info["torch_error"] = str(exc)
...
class RuntimeTelemetry:
    ...
    def __init__(self, sample_interval: float = 0.2): self.sample_interval = max(0.05, float(sample_interval)); self.started_at = time.perf_counter(); self.finished_at: float | None = None; self.operations: defaultdict[str, OperationMetric] = defaultdict(OperationMetric); self._lock = threading.Lock(); self._stop = threading.Event(); self._thread: threading.Thread | None = None; self._token = None; self._peak_rss_bytes = 0; self._peak_cuda_allocated_mb = 0.0; self._peak_cuda_reserved_mb = 0.0; self._warnings: list[str] = []; self._cpu_start: dict[int, float] = {}; self._cpu_latest: dict[int, float] = {}; self._io_start: dict[int, tuple[int, int, int, int]] = {}; self._io_latest: dict[int, tuple[int, int, int, int]] = {}

    def _process_family(self):
        try:
            import psutil

            parent = psutil.Process(os.getpid()); return [parent, *parent.children(recursive=True)]
        except (ImportError, OSError, RuntimeError): return []
    ...
    def _sample(self, *, initial: bool = False) -> None:
        ...
        for process in self._process_family():
            try: memory, cpu, io = self._process_sample(process)
            except Exception:  # A process can disappear between enumeration and sampling.
                continue
            pid = process.pid; rss += memory
            if initial or pid not in self._cpu_start:
                self._cpu_start[pid] = cpu; self._io_start[pid] = io
            self._cpu_latest[pid] = max(cpu, self._cpu_latest.get(pid, cpu)); previous = self._io_latest.get(pid, io)
            self._io_latest[pid] = tuple(
                max(left, right) for left, right in zip(previous, io, strict=True)
            )
        self._peak_rss_bytes = max(self._peak_rss_bytes, rss); allocated, reserved, warnings = _torch_memory()
    ...
    def start(self) -> RuntimeTelemetry:
        ...
        self.started_at = time.perf_counter(); self._sample(initial=True); self._token = _ACTIVE_TELEMETRY.set(self)
    ...
    def stop(self) -> None:
        ...
        self._sample(); self.finished_at = time.perf_counter()
        if self._token is not None:
            _ACTIVE_TELEMETRY.reset(self._token); self._token = None
    ...
    def record(
        ...
        *,
        ...
        with self._lock:
            metric = self.operations[name]; metric.count += int(count); metric.elapsed_sec += max(0.0, float(elapsed_sec)); metric.bytes += max(0, int(byte_count))
    ...
    def result(self, reports=()) -> dict[str, object]:
        ended = self.finished_at or time.perf_counter(); elapsed, cpu_seconds, io_delta, logical_cpus = max(0.0, ended - self.started_at), sum((value - self._cpu_start.get(pid, 0.0) for pid, value in self._cpu_latest.items())), [tuple((value - self._io_start.get(pid, (0, 0, 0, 0))[index] for index, value in enumerate(values))) for pid, values in self._io_latest.items()], os.cpu_count() or 1
        return {
            "elapsed_sec": elapsed,
