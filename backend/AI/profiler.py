...
def delta(start: ResourceSnapshot, end: ResourceSnapshot) -> dict[str, Any]:
    return {
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            if capability is not None:
                info["gpu_compute_capability"] = list(capability)
...
class RuntimeTelemetry:
    """Low-overhead end-to-end telemetry for one pipeline execution."""
    ...
    def _sample(self, *, initial: bool = False) -> None:
        ...
        if allocated is not None:
            self._peak_cuda_allocated_mb = max(self._peak_cuda_allocated_mb, allocated)
        if reserved is not None:
            self._peak_cuda_reserved_mb = max(self._peak_cuda_reserved_mb, reserved)
    ...
    def stop(self) -> None:
        ...
        if self._thread is not None and self._thread is not threading.current_thread():
            self._thread.join(timeout=max(1.0, self.sample_interval * 3))
        self._sample()
    ...
    def result(self, reports=()) -> dict[str, object]:
        ...
        elapsed = max(0.0, ended - self.started_at)
        cpu_seconds = sum(
            value - self._cpu_start.get(pid, 0.0) for pid, value in self._cpu_latest.items()
        )
        io_delta = [
            tuple(
                value - self._io_start.get(pid, (0, 0, 0, 0))[index]
                for index, value in enumerate(values)
            )
            for pid, values in self._io_latest.items()
        ]
...
def profile_operation(name: str, *, byte_count: int = 0):
    ...
    if telemetry is None:
        yield
        return
    with telemetry.operation(name, byte_count=byte_count):
        yield
