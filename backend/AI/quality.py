...
from .models import PitchFrame, Word, Syllable, VocalNote
...
def evaluate_quality(duration: float, pitch: list[PitchFrame], words: list[Word], syllables: list[Syllable], notes: list[VocalNote]) -> QualityReport:
    ...
    syllable_confidence=_clamp(statistics.mean([s.confidence for s in syllables]) if syllables else 0.0)
    ...
    overall=_clamp(0.42*pitch_score+0.33*alignment_score+0.15*syllable_confidence+0.10*note_score)
    warnings=[]
    if pitch_confidence<0.45: warnings.append('Low pitch confidence; vocal separation or F0 estimation may be unreliable')
    if not words: warnings.append('No timed words were produced')
    ...
    return QualityReport(pitch_coverage,pitch_confidence,word_coverage,alignment_confidence,syllable_confidence,note_coverage,overall,tuple(warnings))
