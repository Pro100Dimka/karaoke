...
def validate_within_duration(items, duration: float, name: str, tolerance: float = 0.1) -> None:
    if not math.isfinite(duration) or duration <= 0: raise InvalidArtifactError(f"Invalid audio duration for {name}: {duration}")
...
def validate_pitch(frames) -> None:
    ...
    for index, frame in enumerate(frames):
        ...
        if not all(math.isfinite(float(value)) for value in values): raise InvalidArtifactError(f"Non-finite pitch value at frame {index}")
        if frame.time + 1e-9 < previous_time: raise InvalidArtifactError(f"Pitch timeline is not sorted at frame {index}")
        if frame.time < 0 or frame.frequency < 0: raise InvalidArtifactError(f"Negative pitch value at frame {index}")
        if not 0 <= frame.confidence <= 1: raise InvalidArtifactError(f"Pitch confidence is out of range at frame {index}")
        if frame.voiced and frame.frequency <= 0: raise InvalidArtifactError(f"Voiced pitch frame {index} has no positive frequency")
