...
class StageCache:
    ...
    def _load_index(self) -> dict:
        ...
        if not isinstance(loaded, dict) or loaded.get("version") != self.INDEX_VERSION: return {"version": self.INDEX_VERSION, "stages": {}}
    ...
    def file_hash(self, path: str | Path, block: int = 1024 * 1024) -> str:
        ...
        with self._lock: cached = self._hash_memo.get(memo_key)
        ...
        with self._lock:
            if len(self._hash_memo) > 4096: self._hash_memo.clear()
    ...
    def key_matches(self, stage: str, key: str) -> bool: return False

    def hit(self, stage: str, key: str, outputs: list[Path], validators: dict[Path, Validator] | None=None) -> bool: return False
