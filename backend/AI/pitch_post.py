...
def fuse_pitch_with_yin(
    ...
    *,
    ...
    def protected_subharmonic(index: int, candidate_midi: float, evidence: float) -> bool:
        frame = frames[index]
        if not frame.voiced or frame.frequency <= 0: return False
...
def _midi(hz: float) -> float: return 69.0 + 12.0 * math.log2(hz / 440.0)


def _hz(midi: float) -> float: return 440.0 * 2.0 ** ((midi - 69.0) / 12.0)
...
def _repair_single_frame_holes(frames: list[PitchFrame]) -> list[PitchFrame]:
    ...
    for index in range(1, len(out) - 1):
        ...
        if nxt.time - prev.time > step * 2.8: continue
        if abs(_midi(prev.frequency) - _midi(nxt.frequency)) > 0.55: continue
