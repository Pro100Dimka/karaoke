...
class ProgressReporter:
    ...
    def write(self) -> None:
        if not self.progress_file: return
        downloaded, elapsed = self.total_bytes if self.complete else self.downloaded_bytes(), max(0.001, time.monotonic() - self.started_at)
        transferred = max(0, downloaded - self.initial_bytes)
        speed, remaining = transferred / elapsed, max(0, self.total_bytes - downloaded)
...
def main(argv: list[str] | None = None) -> int:
    ...
    reporter.start()
    workers, retries = max(1, min(args.workers, len(MODELS))), max(1, args.retries)
    LOGGER.info(
