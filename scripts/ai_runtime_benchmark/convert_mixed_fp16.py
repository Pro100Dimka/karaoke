...
def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("model", choices=("ctc_ru", "ctc_uk")); parser.add_argument("profile", choices=tuple(PROFILES)); args = parser.parse_args()
    artifacts = ROOT / "build/ai-runtime-benchmark/artifacts"; source = artifacts / f"{args.model}.onnx"; target = artifacts / f"{args.model}-{args.profile}.onnx"; started = time.perf_counter()
