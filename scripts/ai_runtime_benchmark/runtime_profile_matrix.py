"""Offline selector matrix for production AI runtime profiles.

This script never changes the real hardware profile and never enables shadow-only
backends. It injects synthetic HardwareProfile objects only inside this process
so the production selector/fallback policy can be verified safely on one PC.
"""
...
class Scenario:
    name: str
    profile: runtime.HardwareProfile
    expected_device: str


def gpu(name: str, vendor: str, gib: int) -> runtime.GPUInfo:
    return runtime.GPUInfo(name=name, vendor=vendor, memory_bytes=gib * GIB)


def profile(*, cpu: str, gpus=(), cuda: bool = False) -> runtime.HardwareProfile:
    return runtime.HardwareProfile(
        ...
        cuda_available=cuda,
        cuda_version="12.6" if cuda else "",
    )
...
def selected_summary(plan: runtime.RuntimePlan) -> str:
    return ", ".join(
        f"{runtime.MODEL_ROLES[name]}={spec.key}" for name, spec in plan.selected.items()
    )
...
def run_scenario(scenario: Scenario) -> None:
    ...
    if set(plan.selected) != set(runtime.MODEL_ROLES):
        missing = sorted(set(runtime.MODEL_ROLES) - set(plan.selected))
        raise AssertionError(f"{scenario.name}: missing stages: {missing}")
    wrong = {
        model: spec.key
...
def forced_cpu_on_nvidia() -> None:
    ...
    assert all(spec.device == "cpu" for spec in plan.selected.values())
    print("[PASS] NVIDIA profile + forced CPU -> all stages use CPU")
...
def single_stage_cuda_failure() -> None:
    ...
    assert runtime.selected_backend("asr") is not None
    assert runtime.selected_backend("asr").device == "cuda"
    print("[PASS] Single-stage CUDA failure -> FCPE falls back to CPU; ASR stays CUDA")


def directml_registration() -> None:
    spec = AI_BACKEND_REGISTRY.get("fcpe", "onnxruntime:directml:fp32")
    assert spec.quality_status == "shadow"
    assert "shadow-only" in spec.capabilities
    assert spec.vendor == "amd,intel"
...
def _plan_for(scenario: Scenario) -> runtime.RuntimePlan:
    runtime.reset_runtime_for_tests()
    with patch.object(runtime, "detect_hardware", return_value=scenario.profile):
        return runtime.configure_runtime("auto", force=True)
...
def main() -> int:
    try:
        ...
        forced_cpu_on_nvidia()
        single_stage_cuda_failure()
        directml_registration()
    except Exception as exc:  # noqa: BLE001 - command-line diagnostic
        print(f"\n[FAIL] {type(exc).__name__}: {exc}", file=sys.stderr)
        return 1
    finally:
        runtime.reset_runtime_for_tests()
    print("\nAll selector/fallback simulations passed.")
    print("Note: AMD/Intel performance is NOT simulated; real hardware is required for that.")
    return 0
