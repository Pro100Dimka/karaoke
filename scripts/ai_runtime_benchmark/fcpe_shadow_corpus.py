...
class PitchCase: name: str; source: Path; traits: tuple[str, ...]; source_url: str = ""; license: str = "local"
...
def _download_english() -> None:
    if ENGLISH.source.is_file() and sha256(ENGLISH.source) == ENGLISH_SHA256: return
    ...
    if sha256(temporary) != ENGLISH_SHA256:
        temporary.unlink(missing_ok=True); raise RuntimeError("English corpus checksum mismatch")
...
def prepare_cases() -> tuple[PitchCase, ...]:
    ...
    project = base[0]; variants = BUILD / "variants"
    generated = (
        (
    ...
    for name, audio_filter, traits in generated:
        ...
        if not target.is_file(): ffmpeg(project.source, target, '-af', audio_filter)
...
def _percentile(values: np.ndarray, percentile: float) -> float: return float(np.percentile(values, percentile)) if values.size else 0.0


def compare_tracks(
    reference: list[PitchFrame], candidate: list[PitchFrame]
) -> dict[str, object]:
    count = min(len(reference), len(candidate)); left, right = reference[:count], candidate[:count]; left_voiced = np.asarray([frame.voiced for frame in left], dtype=bool); right_voiced = np.asarray([frame.voiced for frame in right], dtype=bool)
    true_positive = int(np.sum(left_voiced & right_voiced)); precision = true_positive / max(1, int(np.sum(right_voiced))); recall = true_positive / max(1, int(np.sum(left_voiced))); both = left_voiced & right_voiced
    left_f0 = np.asarray([frame.frequency for frame in left], dtype=np.float64)[both]; right_f0 = np.asarray([frame.frequency for frame in right], dtype=np.float64)[both]
    cents = np.abs(
...
def run_case(case: PitchCase, estimator, backend, torch) -> dict[str, object]:
    audio, sample_rate = load_mono(case.source, 16_000); peak = float(np.max(np.abs(audio)))
    if peak > 0.999: audio = np.ascontiguousarray(audio * (0.999 / peak), dtype=np.float32)
    started = time.perf_counter(); production = estimator.estimate(case.source); production_sec = time.perf_counter() - started; model = estimator._model
    tensor = torch.from_numpy(audio).view(1, -1, 1).to(estimator._device)
    with torch.inference_mode():
        started = time.perf_counter(); mel = model.wav2mel(tensor, sample_rate); preprocessing_sec = time.perf_counter() - started; latent = model.model(mel).float().cpu().numpy()
    cent_table = model.model.cent_table.float().cpu().numpy(); reference_f0, reference_confidence = decode_fcpe_latent(latent, cent_table); target_length = len(production); reference_f0 = nearest_resize(reference_f0, target_length)[0]
    ...
    production_post_sec = time.perf_counter() - started; started = time.perf_counter()
...
def main() -> int:
    ...
    parser.add_argument("--unload", action="store_true"); parser.add_argument("--cases", nargs="*"); parser.add_argument("--backend", choices=("cuda", "directml"), default="cuda"); args = parser.parse_args()
    runtime_path = DEPS / ("directml" if args.backend == "directml" else "ort-gpu")
    if runtime_path.is_dir(): sys.path.insert(0, str(runtime_path))
    artifact = (
        ROOT / "build/ai-runtime-benchmark/artifacts/fcpe-fp16.onnx"
        if args.backend == "cuda"
        else ROOT / "downloads/models/optimized/fcpe/fcpe-core.onnx"
    )
    os.environ["KARAOKE_AI_FCPE_ONNX"] = str(artifact); import torch

    estimator = FCPEPitchEstimator(); backend = OrtCudaFCPEBackend() if args.backend == "cuda" else OrtDirectMLFCPEBackend(); selected = set(args.cases or ())
    corpus = tuple(
    ...
    for case in corpus:
        ...
        if args.unload: backend.release()
    ...
    write_json(args.output, payload, ensure_ascii=False); print(args.output); return 0
