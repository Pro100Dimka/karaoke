"""Record actual ONNX Runtime execution-provider assignment by node."""
...
def main():
    ...
    parser.add_argument("backend", choices=("cuda", "directml", "cpu"))
    ...
    session = ort.InferenceSession(str(ARTIFACTS / name), options, providers=providers)
    shape = (1, 1001, 128) if args.model == "fcpe" else (1, 32000)
    session.run(["output"], {"input": np.zeros(shape, dtype=np.float32)})
