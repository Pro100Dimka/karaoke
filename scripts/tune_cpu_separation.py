...
def main(argv: list[str] | None = None) -> int:
    ...
    print("A&D Voice CPU separation thread tuner")
    print(f"Sample: {args.seconds:g}s")
    ...
    with tempfile.TemporaryDirectory(prefix="advoice-cpu-thread-tune-") as temporary:
        ...
        for index, threads in enumerate(candidates, 1):
            ...
            print(f"      load       : {result['load_sec']:.3f}s")
            print(f"      separation : {result['separation_sec']:.3f}s")
            print(f"      output     : {'MATCH' if identical else 'DIFF'}")

        valid = [item for item in results if item["identical"]]
        if not valid:
            print("\n[STOP] No output-identical candidate found; cache was not changed.")
            return 2

        best = min(valid, key=lambda item: float(item["separation_sec"]))
        CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        CACHE_FILE.write_text(str(best["threads"]), encoding="ascii")
        ...
        print("start-dev-cpu.bat will use this value automatically.")
        return 0
