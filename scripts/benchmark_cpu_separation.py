...
def _run_profile(label: str, mix: Path, root: Path, *, tuned: bool, threads: str):
    ...
    try:
        ...
        separation_sec = time.perf_counter() - started
        separator.close()
    return {
        "label": label,
        "load_sec": load_sec,
        ...
        "instrumental_sha256": _sha256(instrumental),
        "vocals": vocals,
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="advoice-cpu-separation-") as temporary:
        root = Path(temporary)
        mix = root / "sample.wav"
        print(f"Preparing {args.seconds:g}s benchmark sample...")
        _convert_input(source, mix, args.seconds)

        print("\n[1/2] CPU baseline")
        baseline = _run_profile("baseline", mix, root, tuned=False, threads=args.threads)
        print(f"      model load: {baseline['load_sec']:.3f}s")
        print(f"      separation: {baseline['separation_sec']:.3f}s")

        print(f"\n[2/2] CPU tuned (threads={args.threads}, interop=1, inference_mode=on)")
        tuned = _run_profile("tuned", mix, root, tuned=True, threads=args.threads)
        print(f"      model load: {tuned['load_sec']:.3f}s")
        print(f"      separation: {tuned['separation_sec']:.3f}s")

        before = float(baseline["separation_sec"])
        after = float(tuned["separation_sec"])
        ...
        print(" RESULT")
        print("============================================================")
        print(f"Baseline separation : {before:.3f}s")
        print(f"Tuned separation    : {after:.3f}s")
        print(f"Change              : {change:+.2f}%")
        print(f"Speedup             : {speedup:.3f}x")
        print(f"Stems byte-identical: {'YES' if identical else 'NO'}")
        print(f"Vocals baseline     : {baseline['vocals_sha256']}")
        print(f"Vocals tuned        : {tuned['vocals_sha256']}")
        print(f"Instrumental base   : {baseline['instrumental_sha256']}")
        print(f"Instrumental tuned  : {tuned['instrumental_sha256']}")
        if not identical:
            print("\n[STOP] Output changed. Do not enable this tuning in production yet.")
            return 2
        if after >= before:
