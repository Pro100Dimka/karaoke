...
echo [1/6] Verifying frontend and architecture...
call npm --prefix "%ROOT%\front" run verify || exit /b 1

echo [2/6] Verifying frontend coverage and mutations...
call npm --prefix "%ROOT%\front" run test:unit:coverage || exit /b 1
call npm --prefix "%ROOT%\front" run test:mutation || exit /b 1

echo [3/6] Verifying frontend end-to-end flow...
call npm --prefix "%ROOT%\front" run test:e2e || exit /b 1

echo [4/6] Auditing JavaScript dependencies...
call npm --prefix "%ROOT%\front" audit || exit /b 1
call npm --prefix "%ROOT%\cloudflare" audit || exit /b 1

echo [5/6] Verifying online service...
call npm --prefix "%ROOT%\cloudflare" test || exit /b 1

echo [6/6] Verifying backend and AI pipeline...
pushd "%ROOT%\backend"
"%PYTHON%" "%ROOT%\scripts\backend\check.py" || exit /b 1
