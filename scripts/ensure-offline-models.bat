...
set "WHISPER_MODEL=%SONGAPP_MODELS_DIR%\whisper\large-v3-turbo.pt"
set "DEMUCS_MODEL=%HF_HOME%\hub\models--adefossez--HTDemucs\refs\main"
...
"%PYTHON%" -c "import os; from pathlib import Path; import whisper; from demucs.pretrained import get_model; root=Path(os.environ['SONGAPP_MODELS_DIR']); whisper.load_model('turbo', download_root=str(root / 'whisper')); get_model('htdemucs'); print('Offline models are ready.')"
exit /b %ERRORLEVEL%
