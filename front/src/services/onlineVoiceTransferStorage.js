...
async function removeTempEntry(root, name) {
  ...
  console.warn?.("Could not remove temporary song transfer file", name, lastError);
}
...
export const createTransferSink = (participantId, metadata) => {
  ...
  if (typeof getDirectory === "function") return (async () => {
    ...
    const close = async () => {
      ...
      state = "closing";
      try { await writable.close(); state = "closed"; }
      catch (error) { state = "open"; throw error; }
    };
    return {
      write: (chunk) => {
        if (state !== "open") throw new Error(translateSaved("Временный файл передачи уже закрыт"));
      ...
      cleanup: async () => {
        if (state === "open" || state === "closing") {
          state = "aborted";
          try { await writable.abort?.(); } catch { /* cleanup continues */ }
        }
...
export const cleanupIncomingTransfer = (transfer) => {
  ...
  Promise.resolve(transfer.sink).then((sink) => sink.cleanup()).catch((error) => {
    console.warn?.("Could not cleanup incoming song transfer", error);
  });
};
