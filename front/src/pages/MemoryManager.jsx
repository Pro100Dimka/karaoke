        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 20 }}>
            {Object.entries(size.breakdown).map(([key, bytes]) => (
              <div key={key} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 12, padding: 14 }}>
                <div className="text-muted" style={{ fontSize: 12 }}>{LABELS[key] || key}</div>
                <div style={{ fontWeight: 700, fontSize: 18 }}>{(bytes / 1024 / 1024).toFixed(1)} МБ</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <button className="btn btn-primary" onClick={clear}><Sparkles size={14} /> Очистить кэш</button>
            <button className="btn btn-ghost" onClick={deleteTemp}><FolderX size={14} /> Удалить временные файлы</button>
          </div>
