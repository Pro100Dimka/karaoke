...
function PlayButton({ isPlaying, onClick }) {
  return (
    <Button
      variant="primary"
      aria-label={isPlaying ? translateSaved("Пауза") : translateSaved("Воспроизвести")}
      title={isPlaying ? translateSaved("Пауза") : translateSaved("Воспроизвести")}
      onClick={onClick}
      sx={{
        inlineSize: 58,
        blockSize: 58,
    ...
    >
      {isPlaying ? (
        ...
        <Play size={27} strokeWidth={2.1} style={{ marginInlineStart: 2 }} />
      )}
    </Button>
  );
}
function TransportButtons({ isPlaying, onSkip, onTogglePlay, onStop }) {
  return (
    ...
    >
      ...
      <TransportButton icon={Square} label={translateSaved("Остановить")} onClick={onStop} />
