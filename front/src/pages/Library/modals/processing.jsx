...
const stageOf = (current, song, fallback) =>
  current?.progress_detail ?? current?.progress_step ?? song.progress_step ?? tr(fallback);
...
export function getProcessingFailureInfo(message) {
  ...
  const hint = /ctc|model unavailable/.test(lower)
    ? "library.alignmentModelUnavailableCheckTheAiModelInstallationAnd"
    : /timestamp|interval/.test(lower)
...
function QueueNav({ song, queue, index, onSelect }) {
...
function Failure({ failure, stage, progress }) {
  return (
    <Card
      role="alert"
      sx={{
...
function ProgressInfo({ state, active, current, stage }) {
  const text =
    state === "done"
...
export default function ProcessingModal({
  ...
  const stage = stageOf(current, song, "library.preparingTheSongProcessing");
  const errorStage = stageOf(current, song, "library.notSpecified");
    ...
    <>
      ...
      {done && (
        <>
          ...
          <Button variant="contained" startIcon={<Play />} onClick={() => onOpenKaraoke(song.id)}>
            {tr("library.open")}
          </Button>
        </>
      )}
  ...
  return (
    ...
    >
      <Stack gap={1} sx={{ padding: "var(--space-5)" }}>
        ...
        {failure ? (
          <Failure failure={failure} stage={errorStage} progress={progress} />
        ) : (
          <ProgressInfo state={state} active={active} current={current} stage={stage} />
        )}
      </Stack>
    </Modal>
  );
