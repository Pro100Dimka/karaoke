...
const textOf = (note, syllablesByIndex) => {
  ...
  return Number.isFinite(idx) ? String(syllablesByIndex.get(idx)?.text || "") : "";
};
...
const sourceIndices = (note) => {
  ...
  return [...new Set(raw.map(Number).filter(Number.isFinite))].sort((a,b)=>a-b);
};
...
export function displayTextForNote(note, syllablesByIndex, labelOwnerBySyllable) {
  if (typeof note.editor_text === "string" && note.editor_text.length) return note.editor_text;
  const idx = Number(note.syllable_index);
  if (!Number.isFinite(idx) || labelOwnerBySyllable.get(idx) !== note._id) return "";
  return String(syllablesByIndex.get(idx)?.text || "");
}

export function mergeSelectedNotes(notes, selectedIds, syllablesByIndex) {
  const selected = new Set(selectedIds);
  const chosen = notes.filter((note) => selected.has(note._id)).sort((a,b) => a.start-b.start || a.end-b.end);
  ...
  for (const note of chosen) {
    ...
    combinedText = glue(combinedText, text, previousWord != null && word != null && previousWord === word);
  ...
  const merged = {
    ...
    syllable_indices: [...new Set(chosen.flatMap(sourceIndices))].sort((a,b)=>a-b)
  };
  const result = [...notes.filter(note => !selected.has(note._id)), merged]
...
export function deleteNotesAndTransferText(notes, selectedIds, syllablesByIndex) {
  const selected = new Set(selectedIds);
  const removed = notes.filter(note => selected.has(note._id)).sort((a,b)=>a.start-b.start);
  ...
  for (const gone of removed) {
    ...
    remaining.forEach((note,index) => {
      ...
      if (distance < best - 1e-9 || (Math.abs(distance-best)<1e-9 && note.start < remaining[targetIndex].start)) {
        best=distance; targetIndex=index;
      }
    });
    ...
    const sameWord=goneWord != null && targetWord != null && goneWord===targetWord;
    target.editor_text = goneMid <= (target.start+target.end)/2
      ? glue(goneText,existing,sameWord)
      : glue(existing,goneText,sameWord);
    target.syllable_indices = [...new Set([...sourceIndices(target), ...sourceIndices(gone)])].sort((a,b)=>a-b);
  }
  return remaining.sort((a,b)=>a.start-b.start || a.midi_note-b.midi_note);
}

export function adjacentNoteId(notes, selectedIds, direction) {
  const ordered=[...notes].sort((a,b)=>a.start-b.start || a.end-b.end || a.midi_note-b.midi_note);
  if (!ordered.length) return null;
  const selected = new Set(selectedIds);
  const anchorIndex = direction > 0
    ? ordered.findIndex(note=>selected.has(note._id))
    : (() => { for(let i=ordered.length-1;i>=0;i--) if(selected.has(ordered[i]._id)) return i; return -1; })();
