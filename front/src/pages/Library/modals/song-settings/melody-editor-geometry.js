...
export function anchoredHorizontalScroll({
  time, oldZoom, newZoom, keyboardWidth, scrollLeft, clientWidth, scrollWidth
}) {
...
export function anchoredVerticalScroll({
  scrollTop, clientHeight, oldRowHeight, newRowHeight, rowCount
}) {
  const centerRow = (scrollTop + clientHeight / 2) / Math.max(1, oldRowHeight);
  const nextHeight = Math.max(1, rowCount) * newRowHeight;
  return clampEditor(centerRow * newRowHeight - clientHeight / 2, 0, Math.max(0, nextHeight - clientHeight));
}

export function autoFollowScrollLeft({
  playheadX, scrollLeft, clientWidth, keyboardWidth, scrollWidth
}) {
  ...
  return clampEditor(playheadX - triggerX, 0, Math.max(0, scrollWidth - clientWidth));
}

export function anchoredVerticalScrollToNote({
  noteMidi, maxMidi, oldRowHeight, newRowHeight, scrollTop, clientHeight, rowCount
}) {
...
export function marqueeHitIds({
  notes, x1, y1, x2, y2, keyboardWidth, zoom, rowHeight, maxMidi
}) {
