...
import { anchoredHorizontalScroll, anchoredVerticalScroll, anchoredVerticalScrollToNote, autoFollowScrollLeft, marqueeHitIds } from "./melody-editor-geometry";
import { adjacentNoteId, constrainedMoveDelta, deleteNotesAndTransferText, displayTextForNote, mergeSelectedNotes, resizeBounds } from "./melody-editor-operations";
...
const normalizeNotes = (notes = []) => notes
  .map((note, index) => ({
    ...note,
...
function ToolbarButton({ icon, label, disabled, danger, active, tone = "neutral", onClick }) {
  return (
    <IconButton
      icon={icon}
      label={label}
...
export default function MelodyEditor({ song, onClose, onSaved }) {
  ...
  const [volumes, setVolumes] = useState({ vocals: 0.7, melody: 0.9, instrumental: 0.45 });
  ...
  const [scrollState, setScrollState] = useState({ left: 0, top: 0, clientWidth: 1, clientHeight: 1, scrollWidth: 1, scrollHeight: 1 });
  ...
  const syllables = useMemo(
    () => (songMap.syllables || []).map((item, index) => ({ ...item, index: Number(item.index ?? index) })),
    [songMap.syllables]
  );
  const syllableByIndex = useMemo(() => new Map(syllables.map((item) => [item.index, item])), [syllables]);
  const labelOwnerBySyllable = useMemo(() => {
    ...
    notes.forEach((note) => {
      ...
      if (!current || note.start < current.start || (note.start === current.start && note.end < current.end)) {
  ...
  let maxMidi = Math.min(127, minMidi + wantedSpan);
  ...
  const whiteKeyGeometry = useMemo(() => {
    ...
    return white.map((midi, index) => {
      ...
      const next = index + 1 < centers.length ? centers[index + 1] : Math.min(laneHeight, center + rowHeight * 2);
  ...
  useEffect(() => {
    ...
    if (melodyGainRef.current) melodyGainRef.current.gain.value = Math.max(0.05, volumes.melody * 0.56);
  }, [volumes]);
  ...
  const auditionNote = useCallback((midi, durationMs = 360) => {
    let context = audioContextRef.current;
    if (!context) { context = new AudioContext(); audioContextRef.current = context; }
    context.resume?.();

    const nowMs = performance.now();
    if (nowMs - auditionTimerRef.current < 42 && auditionRef.current?.midi === Number(midi)) return;
    ...
    if (previous?.gain) {
      try {
        ...
        previous.oscillators?.forEach((oscillator) => {
          try { oscillator.stop(now + 0.06); } catch { /* already stopped */ }
        });
      } catch { /* stale audio node */ }
    }
  ...
  const syncScrollState = useCallback(() => {
    ...
    if (!shell) return;
    setScrollState({
      left: shell.scrollLeft, top: shell.scrollTop,
      clientWidth: shell.clientWidth || 1, clientHeight: shell.clientHeight || 1,
      scrollWidth: shell.scrollWidth || 1, scrollHeight: shell.scrollHeight || 1
    });
  }, []);
  ...
  const startScrollThumbDrag = useCallback((event, axis) => {
    ...
    scrollDragRef.current = {
      ...
      trackSize: axis === "x" ? event.currentTarget.parentElement?.clientWidth || 1 : event.currentTarget.parentElement?.clientHeight || 1,
      thumbSize: axis === "x" ? event.currentTarget.clientWidth || 1 : event.currentTarget.clientHeight || 1
    };
    event.currentTarget.setPointerCapture?.(event.pointerId);
  }, []);

  const moveScrollThumbDrag = useCallback((event) => {
    const state = scrollDragRef.current;
    const shell = rollShellRef.current;
    if (!state || !shell) return;
    event.preventDefault();
    const usable = Math.max(1, state.trackSize - state.thumbSize);
    if (state.axis === "x") shell.scrollLeft = clamp(state.left + ((event.clientX - state.x) / usable) * state.maxX, 0, state.maxX);
    else shell.scrollTop = clamp(state.top + ((event.clientY - state.y) / usable) * state.maxY, 0, state.maxY);
    syncScrollState();
  }, [syncScrollState]);
  ...
  useEffect(() => { requestAnimationFrame(syncScrollState); }, [laneHeight, laneWidth, syncScrollState]);
  useEffect(() => {
    if (!playing) rollCanvasRef.current?.style.setProperty("--editor-playhead-x", `${keyboardWidth + time * zoom}px`);
  }, [keyboardWidth, playing, time, zoom]);

  const updateSynth = useCallback((currentTime) => {
    const active = notes.find((note) => note.start <= currentTime && note.end > currentTime);
    if (!active) { stopOscillator(); return; }
    ...
    oscillatorRef.current.frequency.setTargetAtTime(frequency, context.currentTime, 0.01);
    ...
    const sync = (stamp = performance.now()) => {
      ...
      if (master && !playheadDragRef.current) {
        ...
        if (!master.paused && !master.ended) {
          if (!clock.running) { clock.media = current; clock.perf = stamp; clock.running = true; }
          current = clamp(clock.media + (stamp - clock.perf) / 1000 * (master.playbackRate || 1), 0, duration);
          if (stamp - lastReconcileAt >= 400) {
            ...
            if (Math.abs(mediaCurrent - current) > 0.12) {
              clock.media = mediaCurrent; clock.perf = stamp; current = mediaCurrent;
            }
          }
        } else {
          clock.running = false; clock.media = current; clock.perf = stamp;
        }
        ...
        if (!master.paused && !master.ended) {
          ...
          if (vocal) {
            ...
            if (Math.abs(drift) > 0.13) { vocal.currentTime = mediaTime; vocal.playbackRate = 1; }
            else if (Math.abs(drift) > 0.02) vocal.playbackRate = clamp(1 - drift * 0.16, 0.99, 1.01);
            else if (vocal.playbackRate !== 1) vocal.playbackRate = 1;
          }
          const shell = rollShellRef.current;
          if (autoScroll && shell) {
            const nextLeft = autoFollowScrollLeft({
              playheadX, scrollLeft: shell.scrollLeft, clientWidth: shell.clientWidth,
              keyboardWidth, scrollWidth: shell.scrollWidth
            });
            if (Math.abs(nextLeft - shell.scrollLeft) > 0.2) shell.scrollLeft = nextLeft;
          }
          updateSynth(current);
          if (stamp - lastStateAt >= 50) { lastStateAt = stamp; setTime(current); }
        }
      }
  ...
  const pause = useCallback(() => {
    ...
    transportClockRef.current = { media: current, perf: performance.now(), running: false };
    rollCanvasRef.current?.style.setProperty("--editor-playhead-x", `${keyboardWidth + current * zoom}px`);
    setTime(current);
    setPlaying(false);
    stopOscillator();
  ...
  const play = useCallback(async () => {
    ...
    try {
      ...
      transportClockRef.current = { media: master.currentTime || 0, perf: performance.now(), running: true };
      await Promise.allSettled([master.play(), vocal.play()]);
      setTime(master.currentTime || 0);
      setPlaying(true);
      updateSynth(master.currentTime || 0);
    } catch (error) {
      await notify(`Не удалось начать воспроизведение: ${getErrorMessage(error)}`);
    }
  ...
  useEffect(() => () => {
    ...
    stopOscillator();
    auditionRef.current?.oscillators?.forEach((oscillator) => { try { oscillator.stop(); } catch { /* already stopped */ } });
    audioContextRef.current?.close?.();
  }, [stopOscillator]);

  const seek = useCallback((next, auditionWhenStopped = true) => {
    const value = clamp(Number(next) || 0, 0, duration);
    setTime(value);
    const running = Boolean(instrumentalRef.current && !instrumentalRef.current.paused);
    transportClockRef.current = { media: value, perf: performance.now(), running };
    rollCanvasRef.current?.style.setProperty("--editor-playhead-x", `${keyboardWidth + value * zoom}px`);
    if (instrumentalRef.current) instrumentalRef.current.currentTime = value;
    if (vocalsRef.current) { vocalsRef.current.currentTime = value; vocalsRef.current.playbackRate = 1; }
  ...
  }, [auditionNote, duration, keyboardWidth, noteAtTime, stopOscillator, updateSynth, zoom]);

  const pointerTime = useCallback((clientX) => {
  ...
  const previewPlayhead = useCallback((value) => {
    const next = clamp(Number(value) || 0, 0, duration);
    setTime(next);
    rollCanvasRef.current?.style.setProperty("--editor-playhead-x", `${keyboardWidth + next * zoom}px`);
  ...
  const startPlayheadDrag = useCallback((event) => {
    ...
    if (resume) { master.pause(); vocal?.pause(); setPlaying(false); stopOscillator(); }
  ...
  const movePlayheadDrag = useCallback((event) => {
  ...
  const endPlayheadDrag = useCallback(async (event) => {
  ...
  const setHorizontalZoomAnchored = useCallback((nextZoom) => {
    const shell = rollShellRef.current;
    const next = clamp(Number(nextZoom) || zoom, 36, 220);
    if (!shell || next === zoom) { setZoom(next); return; }

    const anchorTime = instrumentalRef.current?.currentTime ?? time;
    const nextScrollWidth = Math.max(shell.clientWidth, Math.max(1180, duration * next) + keyboardWidth);
    ...
    rollCanvasRef.current?.style.setProperty("--editor-playhead-x", `${keyboardWidth + anchorTime * next}px`);
    syncScrollState();
  }, [duration, keyboardWidth, syncScrollState, time, zoom]);

  const setVerticalZoomAnchored = useCallback((nextZoom) => {
    const shell = rollShellRef.current;
    const next = clamp(Number(nextZoom) || verticalZoom, 10, 36);
    if (!shell || next === verticalZoom) { setVerticalZoom(next); return; }

    const viewportCenterY = shell.scrollTop + shell.clientHeight / 2;
    const anchorNote = notes.length ? notes.reduce((best, note) => {
  ...
  const handleRollWheel = useCallback((event) => {
    if (!event.ctrlKey) return;
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation?.();
    const direction = event.deltaY < 0 ? 1 : -1;
    if (event.shiftKey) { setHorizontalZoomAnchored(zoom + direction * 10); return; }
  ...
  const selectNote = useCallback((id, extend) => {
    setSelected((current) => extend
      ? current.includes(id) ? current.filter((value) => value !== id) : [...current, id]
      : [id]);
  }, []);
  const selectAll = useCallback(() => setSelected(notes.map((note) => note._id)), [notes]);
  const clearSelection = useCallback(() => setSelected([]), []);
  const selectAdjacentNote = useCallback((direction) => {
  ...
  const transpose = useCallback((amount) => {
    if (!selected.length) return;
    const anchor = notes.find((note) => selected.includes(note._id));
    commit((current) => current.map((note) => selected.includes(note._id)
      ? { ...note, midi_note: clamp(note.midi_note + amount, 0, 127) }
      : note));
    if (anchor) auditionNote(clamp(anchor.midi_note + amount, 0, 127), 170);
  }, [auditionNote, commit, notes, selected]);
  const nudgeSelected = useCallback((timeDelta = 0, midiDelta = 0) => {
    if (!selected.length) return;
  ...
  const addNote = useCallback(() => {
    ...
    const next = {
      ...
      end: roundTime(Math.min(duration, time + 0.4)),
      midi_note: Math.round((minMidi + maxMidi) / 2),
      velocity: 96,
      word_index: null,
      syllable_index: null
    };
    ...
    setSelected([id]);
  }, [commit, duration, maxMidi, minMidi, time]);
  const assignSyllable = useCallback((value) => {
    const syllableIndex = value === "" ? null : Number(value);
    const syllable = syllableByIndex.get(syllableIndex);
    commit((current) => current.map((note) => selected.includes(note._id)
      ? { ...note, syllable_index: syllableIndex, word_index: syllable?.word_index ?? null, editor_text: syllableIndex == null ? "" : String(syllable?.text || "") }
      : note));
  }, [commit, selected, syllableByIndex]);

  const startDrag = useCallback((event, note, mode) => {
    event.preventDefault();
    event.stopPropagation();
    const extend = event.shiftKey || event.ctrlKey || event.metaKey;
    const movingSelection = selected.includes(note._id) ? selected : extend ? [...selected, note._id] : [note._id];
  ...
  const drag = useCallback((event) => {
    ...
    if (Math.abs(event.clientX - state.x) > 1 || Math.abs(event.clientY - state.y) > 1) state.moved = true;
    const dy = event.shiftKey ? 0 : rawDy;

    if (state.mode === "move") {
      const safeDx = constrainedMoveDelta(state.snapshot, state.ids, dx, duration);
      const previewMidi = clamp((state.originals.get(state.id)?.midi_note ?? 60) + dy, 0, 127);
      if (state.previewMidi !== previewMidi) {
        state.previewMidi = previewMidi;
        auditionNote(previewMidi, 95);
      }
      replace((current) => current.map((note) => {
    ...
    replace((current) => current.map((note) => note._id === state.id
  ...
  const marqueeSelection = useCallback((state) => {
  ...
  const startMarquee = useCallback((event) => {
    if (event.button !== 0 || event.target.closest?.(".melody-editor-note, .melody-editor-piano-key, .melody-editor-playhead")) return;
    event.preventDefault();
    const canvas = rollCanvasRef.current;
    if (!canvas) return;
    const rect = canvas.getBoundingClientRect();
  ...
  const endMarquee = useCallback((event) => {
    const state = selectionRef.current;
  ...
  useEffect(() => {
    const onKeyDown = (event) => {
      ...
      if (mod && code === "KeyZ" && event.shiftKey) { consume(); redo(); return; }
      if (mod && code === "KeyZ") { consume(); undo(); return; }
      if (mod && code === "KeyY") { consume(); redo(); return; }
      if (mod && code === "KeyS") { consume(); saveRef.current?.(); return; }
      if (!editable && mod && code === "KeyA") { consume(); selectAll(); return; }
      if (!editable && mod && code === "KeyC") { consume(); copySelected(); return; }
      if (!editable && mod && code === "KeyX") { consume(); copySelected(); deleteSelected(); return; }
      if (!editable && mod && code === "KeyV") { consume(); pasteNotes(); return; }
      if (!editable && mod && code === "KeyD") { consume(); duplicateSelected(); return; }

      if (editable || tag === "select") return;
      if (event.key === "Delete" || event.key === "Backspace") { consume(); deleteSelected(); return; }
      if (event.key === "Escape") { consume(); clearSelection(); return; }
      if (event.code === "Space") { consume(); playing ? pause() : play(); return; }
      if (event.key === "Home") { consume(); seek(0); return; }
      if (event.key === "End") { consume(); seek(duration); return; }
      if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
        consume();
        const direction = event.key === "ArrowRight" ? 1 : -1;
        if (mod) { nudgeSelected(direction * (event.shiftKey ? 0.25 : 0.05), 0); return; }
        selectAdjacentNote(direction);
        return;
      }
      if (!selected.length) return;
      if (event.key === "ArrowUp") { consume(); nudgeSelected(0, event.shiftKey ? 12 : 1); return; }
      if (event.key === "ArrowDown") { consume(); nudgeSelected(0, event.shiftKey ? -12 : -1); return; }
    };
    window.addEventListener("keydown", onKeyDown, true);
    return () => window.removeEventListener("keydown", onKeyDown, true);
  }, [clearSelection, copySelected, deleteSelected, duplicateSelected, duration, nudgeSelected, pasteNotes, pause, play, playing, redo, seek, selectAdjacentNote, selectAll, selected.length, undo]);

  const save = useCallback(async () => {
    if (!song?.id) return;
    setSaving(true);
    try {
      const serializable = notes.map(({ _id, ...note }) => note);
  ...
  const restoreAi = useCallback(async () => {
    if (!payload?.ai_backup_exists || !(await confirmDialog("Вернуть исходный результат AI? Ручные изменения будут потеряны."))) return;
    try {
  ...
  }, [confirmDialog, notify, onSaved, payload?.ai_backup_exists, reset, song?.id]);

  const selectedNote = notes.find((note) => note._id === selected[0]);

  return (
    <section className="melody-editor-workspace" aria-label={`Редактор мелодии ${song?.title || ""}`}>
      <header className="melody-editor-header">
        <div className="melody-editor-title-block">
          <span className="melody-editor-eyebrow">{song?.title || "Песня"} · VOCAL MELODY EDITOR</span>
        </div>
        <div className="melody-editor-statusline">
          <span className="melody-editor-status-pill">{selected.length ? `Выбрано ${selected.length}` : "Готов к редактированию"}</span>
          <span className="melody-editor-timecode">{time.toFixed(2)} / {duration.toFixed(2)} сек</span>
        </div>
      </header>

      {loading ? <div className="melody-editor-loading">Загружаем SongMap…</div> : <div className="melody-editor-layout">
        <div className="melody-editor-stage">
          <div className="melody-editor-topdeck melody-editor-topdeck-v11">
            <div className="melody-editor-action-groups" role="toolbar" aria-label="Инструменты редактора">
              <div className="melody-editor-tool-group is-nav">
                <ToolbarButton icon={ArrowLeft} label="Назад" tone="neutral" onClick={() => { pause(); onClose?.(); }} />
                <ToolbarButton icon={Save} label={saving ? "Сохранение…" : "Сохранить"} disabled={saving} tone="pink" active onClick={save} />
              </div>
              <div className="melody-editor-tool-group is-history">
                <ToolbarButton icon={Undo2} label="Отменить" tone="blue" onClick={undo} />
                <ToolbarButton icon={Redo2} label="Вернуть отменённое" tone="blue" onClick={redo} />
              </div>
              <div className="melody-editor-tool-group is-ai">
                {payload?.ai_backup_exists && <ToolbarButton icon={RotateCcw} label="Вернуть результат AI" tone="amber" onClick={restoreAi} />}
                <ToolbarButton icon={Crosshair} label={autoScroll ? "Автопрокрутка включена" : "Автопрокрутка выключена"} tone="cyan" active={autoScroll} onClick={toggleAutoScroll} />
              </div>
              <div className="melody-editor-tool-group is-transport">
                <ToolbarButton icon={playing ? Pause : Play} label={playing ? "Стоп" : "Воспроизвести"} tone="green" active={playing} onClick={playing ? pause : play} />
              </div>
              <div className="melody-editor-tool-group is-edit">
                <ToolbarButton icon={Merge} label="Соединить выбранные" disabled={selected.length < 2} tone="amber" onClick={mergeSelected} />
                <ToolbarButton icon={Trash2} label="Удалить выбранные" disabled={!selected.length} danger tone="red" onClick={deleteSelected} />
              </div>
            </div>

            <div className="melody-editor-compact-dials">
              <EffectDial label="Вокал" value={volumes.vocals} onChange={(value) => setVolumes((v) => ({ ...v, vocals: Number(value) }))} />
              <EffectDial label="Мелодия" value={volumes.melody} accent="secondary" onChange={(value) => setVolumes((v) => ({ ...v, melody: Number(value) }))} />
              <EffectDial label="Минус" value={volumes.instrumental} onChange={(value) => setVolumes((v) => ({ ...v, instrumental: Number(value) }))} />
            </div>
            ...
            <div className={`melody-editor-inline-selection ${selected.length ? "is-active" : ""}`}>
              {selectedNote ? <>
                ...
                <span>{selected.length > 1 ? `${selected.length} нот` : `${selectedNote.start.toFixed(2)}–${selectedNote.end.toFixed(2)}с`}</span>
                <select aria-label="Текст / слог" value={selectedNote?.syllable_index ?? ""} onChange={(event) => assignSyllable(event.target.value)}>
                  <option value="">Без текста</option>
                  {syllables.map((item) => <option key={item.index} value={item.index}>{item.text} · #{item.index}</option>)}
                </select>
              </> : <span>Выберите ноту</span>}
            </div>
          </div>
          <audio ref={vocalsRef} preload="metadata" src={api.getAudioTrackUrl(song.id, "vocals")} />
          <audio
            ref={instrumentalRef}
            preload="metadata"
          ...
          <div ref={rollShellRef} className="melody-editor-roll-shell" onScroll={syncScrollState}>
            <div
              ...
              onPointerMove={(event) => { drag(event); updateMarquee(event); }}
              onPointerUp={(event) => { endDrag(); endMarquee(event); }}
              onPointerCancel={(event) => { endDrag(); endMarquee(event); }}
              onDoubleClick={(event) => {
            ...
            >
              {Array.from({ length: maxMidi - minMidi + 1 }, (_, idx) => {
                ...
                return <div key={`row-${midi}`} className={`melody-editor-pitch-row ${black ? "is-black" : "is-white"}`} style={{ left: keyboardWidth, top: idx * rowHeight, height: rowHeight }} />;
              })}

              <div className="melody-editor-keyboard" style={{ width: keyboardWidth, height: laneHeight }}>
                {whiteKeyGeometry.map(({ midi, top, height }) => (
                  <div key={`white-${midi}`} className="melody-editor-piano-key is-white" style={{ top, width: keyboardWidth, height }} onPointerDown={(event) => { event.stopPropagation(); auditionNote(midi, 220); }}><span>{noteName(midi)}</span></div>
                ))}
                {Array.from({ length: maxMidi - minMidi + 1 }, (_, idx) => maxMidi - idx)
                  .filter((midi) => BLACK_KEYS.has(((midi % 12) + 12) % 12))
                  .map((midi) => {
                    const center = (maxMidi - midi + 0.5) * rowHeight;
                    const height = rowHeight * 0.62;
                    return <div key={`black-${midi}`} className="melody-editor-piano-key is-black" style={{ top: center - height / 2, width: keyboardWidth * 0.64, height }} onPointerDown={(event) => { event.stopPropagation(); auditionNote(midi, 220); }}><span>{noteName(midi)}</span></div>;
                  })}
              </div>
              <div className="melody-editor-zero-time" style={{ left: keyboardWidth }}>0:00</div>
              ...
              {notes.map((note) => {
                ...
                const hasSyllable = note.syllable_index != null && note.syllable_index !== "";
                const syllableIndex = hasSyllable ? Number(note.syllable_index) : null;
                const displayLabel = displayTextForNote(note, syllableByIndex, labelOwnerBySyllable);
                return (
                  <div
                    key={note._id}
                  ...
                  >
                    <span className="melody-editor-note-handle is-left" onPointerDown={(event) => startDrag(event, note, "left")} />
                    <span className="melody-editor-note-label">{displayLabel}</span>
                    <span className="melody-editor-note-handle is-right" onPointerDown={(event) => startDrag(event, note, "right")} />
                  </div>
                );
              })}
          ...
          <div className="melody-editor-cubase-scrollbar is-horizontal" aria-label="Горизонтальная прокрутка">
            <div className="melody-editor-scroll-track" onPointerDown={(event) => {
              const shell = rollShellRef.current; if (!shell) return;
              const rect = event.currentTarget.getBoundingClientRect();
              const max = Math.max(1, shell.scrollWidth - shell.clientWidth);
              shell.scrollLeft = clamp(((event.clientX - rect.left) / rect.width) * max, 0, max);
              syncScrollState();
            }}>
              <span className="melody-editor-scroll-thumb" onPointerDown={(event) => startScrollThumbDrag(event, "x")} onPointerMove={moveScrollThumbDrag} onPointerUp={endScrollThumbDrag} onPointerCancel={endScrollThumbDrag} style={{
            ...
            <label className="melody-editor-inline-zoom" title="Горизонтальный масштаб · Ctrl+Shift+колесо"><MoveHorizontal size={12} /><input type="range" min="36" max="220" step="1" value={zoom} onChange={(event) => setHorizontalZoomAnchored(Number(event.target.value))} /></label>
          </div>
          <div className="melody-editor-cubase-scrollbar is-vertical" aria-label="Вертикальная прокрутка">
            <div className="melody-editor-scroll-track" onPointerDown={(event) => {
              const shell = rollShellRef.current; if (!shell) return;
              const rect = event.currentTarget.getBoundingClientRect();
              const max = Math.max(1, shell.scrollHeight - shell.clientHeight);
              shell.scrollTop = clamp(((event.clientY - rect.top) / rect.height) * max, 0, max);
              syncScrollState();
            }}>
              <span className="melody-editor-scroll-thumb" onPointerDown={(event) => startScrollThumbDrag(event, "y")} onPointerMove={moveScrollThumbDrag} onPointerUp={endScrollThumbDrag} onPointerCancel={endScrollThumbDrag} style={{
            ...
            <label className="melody-editor-inline-zoom is-vertical" title="Вертикальный масштаб · Ctrl+колесо"><MoveVertical size={12} /><input type="range" min="10" max="36" step="1" value={verticalZoom} onChange={(event) => setVerticalZoomAnchored(Number(event.target.value))} /></label>
          </div>
        </div>
      </div>}
    </section>
