...
`;
    ...
    `,
        ...
        void main() {
            ...
            // Beat pulse flash
            float beatFlash = 1.0 + uBeatEnergy * 1.2;
            color *= beatFlash;
            // Per-line alpha reacts to highMid (attack/presence energy)
            float lineAlpha = vAlpha * uOpacity * (0.4 + uHighMid * 0.5 + uBeatEnergy * 0.4);
            gl_FragColor = vec4(color, clamp(lineAlpha, 0.0, 1.0));
        }
    `,
            uniforms: {
        ...
        function updateConnections(threshold, bassBoost, highMid, mid) {
            ...
            const effectiveThreshold = threshold * (1.0 + bassBoost * 0.6) * (1.0 - highMid * 0.15);
            const thresholdSq = effectiveThreshold * effectiveThreshold;

            // Dynamic connection cap — expands with mid+highMid energy
            const dynamicMax = Math.floor(maxConnections * (0.3 + (mid + highMid) * 0.7));
        ...
        const crawlerMaterial = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                varying float vAlpha;
                void main() {
                    vec3 c = uColor * (1.2 + uBeatEnergy * 1.5);
...
`;
        ...
        gui.domElement.classList.add('hidden'); // Default closed
        ...
        gui.add(STATE, 'field', Object.keys(FIELD_TYPES)).name('⚡ Field Type').onChange(val => {
            ...
            guiTargetAccent.set(d.c1);
            guiTargetAccent2.set(d.c2);
            function step() {
        ...
        async function startMic(preferredLabel = null) {
            ...
            try {
                ...
                trackName.textContent = AUDIO.currentTrackName;
                updateUI();
                // status update handled in AUDIO.startMic
            } catch (e) {
                console.error("Global startMic failed:", e);
                const status = document.getElementById('status-bar');
                status.innerHTML = `ERROR: <span style="color:#ff6b6b">${e.message}</span>`;
            }
        }
        ...
        document.getElementById('mic-btn-2').onclick = () => startMic(null);
            ...
            if (crawlerMesh.visible) {
                ...
                crawlerMaterial.uniforms.uColor.value.lerp(
                    new THREE.Color().lerpColors(
                        new THREE.Color(FIELD_TYPES[STATE.field]?.c1 || '#ffffff'),
                        new THREE.Color(FIELD_TYPES[STATE.field]?.c3 || '#ffffff'),
                        material.uniforms.uSpectralCentroid.value
                    ), 0.05
                );
        ...
        // Auto-start System Audio (BlackHole)
        window.addEventListener('load', () => {
            setTimeout(() => {
                ...
                Promise.resolve().catch(e => {
                    ...
                    const status = document.getElementById('status-bar');
                    if (status) status.innerHTML = "SYSTEM: <span>CLICK TO START</span>";
                });
            }, 1000);
        });
