...
import * as THREE from 'three';
        ...
        const AUDIO = {
            ...
            mode: null, micStream: null, external: false,
            ...
            beatThreshold: 0.15, beatDecay: 0.93, beatEnergy: 0,
            ...
            gateState: {
                isOpen: false, envelope: 0, holdTime: 0, noiseFloor: 0.05,
                calibrationSamples: [], isCalibrating: true, calibrationFrames: 30
            },
            ...
            async startMic(preferredLabel = null) {
                ...
                try {
                    ...
                    if (preferredLabel) {
                        ...
                        console.log("Audio Devices Found:", devices.length, devices.map(d => `${d.label} (${d.kind})`));

                        target = devices.find(d => d.kind === 'audioinput' &&
                    ...
                    if (target) {
                        ...
                        if (preferredLabel) console.warn(`Preferred device '${preferredLabel}' not found. Using default.`);
                    ...
                    if (status) status.innerHTML = 'SYSTEM: <span>LISTENING (' + (preferredLabel ? 'Loopback' : 'Mic') + ')</span>';
                    updateUI();
            ...
            applyExternalSpectrum(levels, radioBass, isActive) {
                ...
                Object.entries(mapped).forEach(([band, value]) => {
                    ...
                    this.gatedBands[band] = (band === 'subBass' || band === 'bass' || band === 'lowMid') ? this.applyBassGate(value) : value;
                });

                const bandValues = Object.values(mapped);
                const total = bandValues.reduce((sum, value) => sum + value, 0);
                this.spectralCentroid = total > 0
                    ? bandValues.reduce((sum, value, index) => sum + value * index, 0) / total / (bandValues.length - 1)
                    : 0;
                ...
                const kickRise = Math.max(0, mapped.subBass - previous.subBass) * 0.7
                    + Math.max(0, mapped.bass - previous.bass);
                const clapRise = Math.max(0, mapped.lowMid - previous.lowMid) * 0.45
                    + Math.max(0, mapped.mid - previous.mid) * 0.7
                    + Math.max(0, mapped.highMid - previous.highMid);
                const tickRise = Math.max(0, mapped.highMid - previous.highMid) * 0.35
                    + Math.max(0, mapped.high - previous.high)
                    + Math.max(0, mapped.ultraHigh - previous.ultraHigh) * 0.8;
                const transientScore = Math.max(
                    kickRise * 4.5,
                    clapRise * 5.5,
                    tickRise * 7.0,
                    this.spectralFlux * 5.0
                ) * STATE.sensitivity;
                ...
                const averageEnergy = this.peakHistory.reduce((sum, value) => sum + value, 0) / this.peakHistory.length;
                const variance = this.peakHistory.reduce((sum, value) => sum + Math.pow(value - averageEnergy, 2), 0) / this.peakHistory.length;
            ...
            update() {
                ...
                const ranges = { subBass: [50, 120], bass: [120, 200], lowMid: [200, 450], mid: [450, 2000], highMid: [2000, 3500], high: [3500, 12000], ultraHigh: [12000, 20000] };
        ...
        window.addEventListener('keydown', (e) => {
            ...
            else if (e.key === 'f' || e.key === 'F') { STATE.dofEnabled = !STATE.dofEnabled; customDOFPass.enabled = STATE.dofEnabled; if (dofController) dofController.updateDisplay(); }
        });
...
void main() {
    ...
    `
        ...
        const FIELD_TYPES = {
            'Photon (EM)': { c1: '#00f2ff', c2: '#0051ff', c3: '#ffffff', s: 1.0, curl: 1.0, sz: 3.0, sc1: '#66d9ff', sc2: '#99eeff' },
            'Gluon (Strong)': { c1: '#ff0055', c2: '#7000ff', c3: '#ff8800', s: 2.5, curl: 2.4, sz: 3.4, sc1: '#cc66ff', sc2: '#ff6699' },
            'Higgs (Mass)': { c1: '#ffae00', c2: '#ff4800', c3: '#ffff00', s: 0.6, curl: 0.3, sz: 4.0, sc1: '#ffcc66', sc2: '#ffdd99' },
            'Gravity': { c1: '#ffffff', c2: '#4466ff', c3: '#8844ff', s: 0.75, curl: 0.18, sz: 2.8, sc1: '#aabbff', sc2: '#ccaaff' },
            'Dark Energy': { c1: '#1a0033', c2: '#6600ff', c3: '#ff00ff', s: 1.9, curl: 1.6, sz: 3.1, sc1: '#9933ff', sc2: '#cc66ff' },
            'Neutrino': { c1: '#00ff88', c2: '#00ffcc', c3: '#88ffff', s: 3.0, curl: 3.0, sz: 2.6, sc1: '#66ffbb', sc2: '#99ffdd' },
            // === ENHANCEMENT: Waveform Terrain Field ===
            'Waveform Terrain': { c1: '#ff3366', c2: '#33ff99', c3: '#6633ff', s: 0.4, curl: 0.15, sz: 2.4, sc1: '#ff6699', sc2: '#66ffcc', terrain: true }
        };
...
`;
    ...
    `,
        ...
        function updateConnections(threshold, bassBoost, highMid, mid) {
            ...
            const dynamicMax = Math.floor(maxConnections * Math.min(1.0, 0.22 + bassBoost * 0.55 + mid * 0.45 + highMid * 0.65));
            ...
            for (let i = 0; i < positions.length && connIdx < dynamicMax; i++) {
                for (let j = i + 1; j < positions.length && connIdx < dynamicMax; j++) {
                    ...
                    if (distSq < thresholdSq && distSq > 0.1) {
                        ...
                        const centreProximity = 1.0 - Math.min((positions[i].length() + positions[j].length()) * 0.5 / 90.0, 1.0);
        ...
        function updateCrawlers(dt, highMid, beatEnergy, spectralCentroid) {
            ...
            function nearestNeighbour(fromIdx, excludeIdx) {
                ...
                for (let k = start; k < start + check; k++) {
                    ...
                    if (d < threshSq && d < bestDist) { bestDist = d; bestIdx = ki; }
                }
            ...
            for (let i = 0; i < crawlers.length; i++) {
                ...
                for (let s = 0; s < tailLen - 1 && segIdx < crawlerMaxSegs; s++) {
                    ...
                    crawlerPos[segIdx * 6 + 0] = p0.x; crawlerPos[segIdx * 6 + 1] = p0.y; crawlerPos[segIdx * 6 + 2] = p0.z;
                    crawlerPos[segIdx * 6 + 3] = p1.x; crawlerPos[segIdx * 6 + 4] = p1.y; crawlerPos[segIdx * 6 + 5] = p1.z;
...
`;
        ...
        function applyGuiTheme(accent, accent2) {
            ...
            el.style.setProperty('--hover-color',  `rgb(${Math.min(wR+25,255)},${Math.min(wG+25,255)},${Math.min(wB+25,255)})`);
        }
        ...
        function applyFieldAppearance(val) {
            ...
            const c1 = new THREE.Color(palette.c1), c2 = new THREE.Color(palette.c2), c3 = new THREE.Color(palette.c3);
            const sc1 = new THREE.Color(palette.sc1), sc2 = new THREE.Color(palette.sc2);
        ...
        window.addEventListener('message', (event) => {
            ...
            const renderColor = value => {
                ...
                const inverted = [0, 2, 4].map(offset =>
        ...
        gui.add(STATE, 'density', 0.017, 0.14).name('⭐ Density').onChange(v => geometry.setDrawRange(0, Math.floor((CONFIG.maxParticles + CONFIG.secondaryParticles) * v)));
        ...
        const dofController = f_lens.add(STATE, 'dofEnabled').name('Depth of Field').onChange(v => customDOFPass.enabled = v);
        ...
        f_post.add(STATE, 'anamorphicStretch', 0.0, 1.0).name('Anamorphic').onChange(v => {
        ...
        function togglePlayPause() {
            ...
            updateUI();
            if (status) status.innerHTML = AUDIO.isPlaying ? 'SYSTEM: <span>PLAYING</span>' : 'SYSTEM: <span>PAUSED</span>';
        }
        ...
        async function loadTrack(file) {
            ...
            AUDIO.play();
            if (trackName) trackName.textContent = file.name.length > 25 ? file.name.slice(0, 22) + '...' : file.name;
            updateUI();
        ...
        if (fileInput2) fileInput2.onchange = e => { if (e.target.files[0]) loadTrack(e.target.files[0]); e.target.value = ''; };
        ...
        function animate(timestamp) {
            ...
            material.uniforms.uAudioActivity.value = THREE.MathUtils.lerp(material.uniforms.uAudioActivity.value, audioActivity, 0.08);
            ...
            // Update crawlers
            crawlerMesh.visible = STATE.crawlersEnabled && STATE.connectionsEnabled && AUDIO.active && (AUDIO.isPlaying || AUDIO.mode === 'mic');
            ...
            controls.autoRotateSpeed = 0.35 + material.uniforms.uMid.value * 0.8 + material.uniforms.uBeatEnergy.value * 0.8;
            const bloomBase = STATE.bloom * (audioActivity > 0.5 ? 0.55 : 1.0);
            bloomPass.strength = bloomBase + (AUDIO.gatedBands.bass + AUDIO.gatedBands.mid + AUDIO.beatEnergy) / 24;
            if (anamorphicPass.enabled) anamorphicPass.uniforms.uIntensity.value = 0.3 + AUDIO.gatedBands.bass * 0.4;
