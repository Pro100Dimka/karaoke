...
export default function Settings() {
  ...
  useEffect(() => {
    api
      .getAppSettings()
      .then(setForm)
      .catch((err) => notify(`Не удалось загрузить настройки: ${err.message}`));
  }, [notify]);
  useEffect(() => {
                ...
                <div className="settings-field-grid">
                  ...
                  >
                    <input
                      ...
                      onChange={(event) =>
                        set("online_name")(event.target.value)
                      }
                      onBlur={(event) => saveOnlineName(event.target.value)}
                    />
                  </SettingField>
                  <ToggleField
                ...
                <div className="settings-field-grid">
                  ...
                  >
                    <input
                      ...
                      onChange={(event) =>
                        set("thread_count")(Number(event.target.value))
                      }
                    />
                  </SettingField>
                  <ToggleField
                    ...
                    onChange={set("use_gpu")}
                  />
                  <ToggleField
                    label="Использовать CPU"
                    hint="Резервный режим обработки"
                    checked={form.use_cpu}
                    onChange={set("use_cpu")}
                  />
                </div>
              )}
              {tab === "storage" && (
                <div className="settings-field-grid settings-path-grid">
                  <SettingField label="Папка с песнями">
                    <input
                      className="input"
                      value={form.songs_folder || ""}
                      readOnly
                    />
                  </SettingField>
                  <SettingField label="Папка AI">
                    <input
                      className="input"
                      value={form.ai_folder || ""}
                      readOnly
                    />
                  </SettingField>
                  <SettingField label="Папка записей">
                    <input
                      className="input"
                      value={form.recordings_folder || ""}
                      readOnly
                    />
                  </SettingField>
                  <SettingField label="Папка кэша">
                    <input
                      className="input"
                      value={form.cache_folder || ""}
                      readOnly
                    />
                  </SettingField>
                </div>
...
function ServiceScreen({ view, onBack }) {
  ...
  return (
    <div className="settings-service-screen">
      <button
        type="button"
        className="btn btn-ghost settings-service-back"
        onClick={onBack}
      >
