...
export default function KeyboardLighting() {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      busy = true;
      try {
        const current = latest.current, music = readLightingMusic();
        ...
        await sendLightingFrame({ active: theme || music.active, rgb: lightingColor(color, current.keyboard_lighting_brightness, music.level, current.keyboard_lighting_mode) });
      } catch { /* Optional peripheral failures never affect audio. */ }
      finally { busy = false; }
    };
    const timer = setInterval(tick, 50);
    const stop = () => { cancelled = true; clearInterval(timer); configureLighting(false).catch(() => {}); };
