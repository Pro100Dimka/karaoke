export function openingTags(source, tagName) {
  ...
  for (let start = source.indexOf(prefix); start >= 0; start = source.indexOf(prefix, start + 1)) {
...
export function duplicateAttributes(tag) {
  const body = tag
    ...
    .replace(/^<\S+/, "")
    .replace(/\/?\s*>$/, "");
  const names = [...body.matchAll(/(?:^|\s)([A-Za-z_:][\w:.-]*)\s*(?==|\s|$)/g)]
    .map((match) => match[1]);
...
export function importedAndDeclaredNames(source) {
  ...
  for (const pattern of [/(?:function|class)\s+([A-Z]\w*)/g, /const\s+([A-Z]\w*)\s*=/g]) {
    for (const match of source.matchAll(pattern)) names.add(match[1]);
  }
  for (const destructuring of source.matchAll(/\{([^{}]*?)\}|\(\s*\[([^\]]+)\]\s*\)\s*=>/g)) {
    for (const token of (destructuring[1] ?? destructuring[2]).matchAll(/\b([A-Z]\w*)\b/g)) names.add(token[1]);
  }
  return names;
}
