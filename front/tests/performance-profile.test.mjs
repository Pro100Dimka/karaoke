...
test("performance profile detects weak hardware and updates the root", async () => {
  ...
  equal([detectReducedPerformance(environment), true], [detectReducedPerformance({}), false], [applyPerformanceProfile(environment), true], [documentElement.dataset.performance, "reduced"]);
});
test("performance profile leaves capable hardware fully animated", async () => {
  ...
  equal([applyPerformanceProfile(environment), false], [documentElement.dataset.performance, undefined], [detectReducedPerformance({ navigator: { hardwareConcurrency: 4, deviceMemory: 16 } }), true], [detectReducedPerformance({ navigator: { hardwareConcurrency: 12, deviceMemory: 4 } }), true]);
  for (const value of [0, -1, 5, Number.NaN, Number.POSITIVE_INFINITY]) {
    equal([detectReducedPerformance({ navigator: { hardwareConcurrency: value, deviceMemory: 16 } }), false], [detectReducedPerformance({ navigator: { hardwareConcurrency: 12, deviceMemory: value } }), false]);
  }
});
test("performance profile honors reduced motion and safely skips missing APIs", async () => {
  ...
  equal([detectReducedPerformance({ navigator: { hardwareConcurrency: 12, deviceMemory: 16 }, matchMedia }), true], [detectReducedPerformance({ navigator: { hardwareConcurrency: 12, deviceMemory: 16 }, matchMedia: () => undefined }), false], [applyPerformanceProfile({ navigator: {} }), false]);
});
