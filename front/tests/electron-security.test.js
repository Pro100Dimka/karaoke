...
test("packaged navigation accepts only the packaged index file", () => {
  ...
  assert.equal(
    isAllowedRendererUrl(`${packagedIndexUrl}#/karaoke`, options),
    true
  );
  assert.equal(
    isAllowedRendererUrl("file:///C:/Windows/System32/calc.exe", options),
    false
  );
...
test("IPC requests are accepted only from the active renderer", () => {
  ...
  assert.equal(
    isTrustedIpcEvent({ sender }, { isDestroyed: () => true }),
    false
  );
});
