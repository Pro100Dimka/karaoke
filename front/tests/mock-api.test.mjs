...
test("mock fixtures preserve the complete development data contract", async () => {
  ...
  deepEqual([mockSongs, [ { id: "mock-song-1", title: "Тестовая песня", artist: "A&D Voice", genre: "Pop", status: "done", progress_percent: 100, duration_sec: 185, key: "C", tempo: 120, video_url: "" }, { id: "mock-song-processing", title: "Песня в обработке", artist: "Demo", status: "processing", progress_percent: 48, progress_step: "Разделение дорожек" } ]], [mockSongEditor, { ai_backup_exists: true, song_map: { duration: 10, syllables: [ { index: 0, text: "A&D", word_index: 0 }, { index: 1, text: "Voice", word_index: 1 } ], notes: [ { _id: "mock-note-1", start: 0.5, end: 1.5, midi_note: 60, velocity: 96, syllable_index: 0, word_index: 0 }, { _id: "mock-note-2", start: 1.5, end: 2.5, midi_note: 64, velocity: 96, syllable_index: 1, word_index: 1 } ] } }], [mockAppSettings, { online_name: "Тестовый пользователь", theme: "dark", accent: "purple", density: "comfortable", animations: true }], [mockAudioSettings, { volume: 1, reverb: 0, echo: 0, delay: 0, audio_driver: "auto", asio_driver_name: "", buffer_size: 64, monitoring_enabled: false }]);
  equal([mockKaraokeResult.lyrics_sync.text, "Добро пожаловать в A&D Voice\nИнтерфейс работает без backend"], [mockKaraokeResult.lyrics_sync.words.length, 9], [mockKaraokeResult.reference_notes.length, 4]);
});
