...
describe("microphoneChannelStrip", () => {
  test("connects one highpass -> presence -> compressor -> makeup -> limiter chain", () => {
    ...
    verify([created.compressors, 'toHaveLength', 1], [created.compressors[0].threshold.value, 'toBe', -16], [created.gains, 'toHaveLength', 1], [created.gains[0].gain.value, 'toBe', 1.08], [created.shapers, 'toHaveLength', 2], [nodes.noiseGate.curve, 'toHaveLength', 4096], [nodes.limiter.curve, 'toHaveLength', 1024]);
    // Every node hands off to exactly the next stage, ending at destination.
    same([source.target, nodes.highpass], [nodes.highpass.target, nodes.noiseGate], [nodes.noiseGate.target, nodes.presence], [nodes.presence.target, nodes.compressor], [nodes.compressor.target, nodes.makeup], [nodes.makeup.target, nodes.limiter], [nodes.limiter.target, destination]);
  });
