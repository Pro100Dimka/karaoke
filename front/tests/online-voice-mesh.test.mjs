...
class FakePeer {
  ...
  constructor(configuration) {
    ...
    this.senders = [];
    this.addTrack = vi.fn((mediaTrack) => { this.senders.push({ track: mediaTrack }); });
    ...
    this.setLocalDescription = vi.fn(async (description) => { this.localDescription = description; });
...
const makeMesh = () =>
  new OnlineVoiceMesh({ send: vi.fn(() => true) });

beforeEach(async () => {
  vi.resetModules();
  ({ default: OnlineVoiceMesh } =
    await import("../src/services/onlineVoiceMesh.js"));
  FakePeer.instances = [];
...
describe("online voice mesh", () => {
  ...
  test("normalizes missing outbound metadata and validates every input boundary", async () => {
    ...
    for (const participantId of [null, 1, {}, "", "g".repeat(129)]) {
      await expect(mesh.sendFile(participantId, new Blob([]))).rejects.toThrow(
        "Для передачи нужны участник и файл"
      );
    }
    ...
    await expect(mesh.sendFile("guest", oversized)).rejects.toThrow(
      "Файл слишком большой для передачи через комнату"
    );
  ...
  test("reports the exact incoming lifecycle and protocol errors", async () => {
    ...
    expect(JSON.parse(channel.send.mock.calls.at(-1)[0])).toEqual({
      type: "file-error",
      transferId: "wrong",
      error: "Получен неполный файл песни"
    });

    channel.onmessage({ data: JSON.stringify({ type: "file-start", transferId: "stall", size: 1 }) });
  ...
  test("distinguishes unavailable, stale, live and structurally partial captures", async () => {
    ...
    for (const value of [undefined, {}, { mediaDevices: {} }]) {
      ...
      await expect(mesh.start()).rejects.toThrow(
  ...
  test("keeps peer removal idempotent and stop invalidates every resource", () => {
    ...
    mesh.channels.set("channel", channelOnly);
  ...
  test.each(["lifecycle", "replacement", "closed"])(
    "cancels an invitation when only its %s invariant changes",
    async (invariant) => {
      let release;
      globalThis.RTCPeerConnection = class DeferredOfferPeer extends FakePeer {
        constructor(configuration) {
          super(configuration);
          this.createOffer = vi.fn( () => new Promise((resolve) => { release = resolve; })
          );
        }
      };
      const mesh = makeMesh();
            ...
            (description) =>
              new Promise((resolve) => {
                this.localDescription =
                  invariant === "missing-local" ? null : description;
                release = resolve;
              })
          );
        }
  ...
  test.each(["lifecycle", "version", "replacement", "closed"])(
    ...
    async (invariant) => {
      let release;
      globalThis.RTCPeerConnection = class DeferredDescriptionPeer extends (
        FakePeer
      ) {
        constructor(configuration) {
          super(configuration);
          this.setRemoteDescription = vi.fn(
            (description) =>
              new Promise((resolve) => { this.remoteDescription = description; release = resolve; })
          );
        }
      };
      const mesh = makeMesh();
  ...
  test("distinguishes candidates, answers and offers without cross-work", async () => {
    ...
    expect( await mesh.accept("guest", { description: { type: "answer", sdp: "answer" } })
    ).toBe(true);
  ...
  test("validates each incoming start boundary independently", () => {
    ...
    for (const size of [0, 512 * 1024 * 1024]) {
      const mesh = makeMesh();
      const channel = new FakeChannel();
      mesh.setupDataChannel("host", channel);
  ...
  test("matches confirmations by transfer and participant with exact errors", () => {
    ...
    const resolve = vi.fn();
    const reject = vi.fn();
    const pending = { participantId: "host", resolve, reject, timer: 10 };
    ...
    expect(reject).toHaveBeenLastCalledWith(
      expect.objectContaining({ message: "Получатель не смог импортировать песню" })
    );
  });
  ...
  test("ignores unrelated binary data and duplicate active starts", () => {
    const mesh = makeMesh();
    const channel = new FakeChannel();
    mesh.setupDataChannel("host", channel);
  ...
  test.each(["before-read", "after-read", "after-chunk"])(
    "cancels a transfer from lifecycle invalidation %s",
    async (phase) => {
      const mesh = makeMesh();
      const channel = new FakeChannel();
      mesh.setupDataChannel("guest", channel);
      if (phase === "before-read") channel.send.mockImplementationOnce(() => { mesh.lifecycleVersion += 1; });
      ...
      await expect( mesh.sendFile("guest", new InvalidatingBlob(["x"]))
      ).rejects.toThrow("Передача файла отменена");
    }
  );
  ...
  test("clears only owned timers and reports peer removal exactly", () => {
    ...
    mesh.incomingFiles.set("guest", { timer: 22 });
    mesh.channels.set("guest", new FakeChannel());
    const reject = vi.fn();
    mesh.pendingTransferConfirmations.set("owned", {
      participantId: "guest",
      timer: 23,
      reject,
      resolve: vi.fn()
  ...
  test("keeps foreign in-flight promise ownership intact", async () => {
    ...
    Object.defineProperty(globalThis, "navigator", {
      ...
      value: {
        mediaDevices: {
          getUserMedia: vi.fn( () => new Promise((resolve) => { releaseCapture = resolve; })
          )
        }
      }
    });
    ...
    let releasePrevious;
    mesh.signalPromises.set( "signal", new Promise((resolve) => { releasePrevious = resolve; })
    );
  ...
  test("tolerates missing optional event hooks on current and stale peers", () => {
    ...
    mesh.removePeer("guest");
    expect(() => peer.ontrack({ streams: [{ getTracks: undefined }] })
    ).not.toThrow();
  ...
  test("accepts the maximum control-message size and rejects one byte more", () => {
    ...
    expect(acceptedMesh.incomingFiles.has("host")).toBe(true);
    ...
    expect(rejectedMesh.incomingFiles.size).toBe(0);
  });
  ...
  test("does not let stale channels clear a replacement channel", () => {
    ...
    first.onclose();
    first.onerror();
    expect(mesh.channels.get("guest")).toBe(second);
    second.onclose();
  ...
  test.each([
    ...
    { failure: () => { throw new Error("sync"); }, sent: "sync" }
  ...
  test("completes an import without a callback and suppresses only the closed reply", async () => {
    const mesh = makeMesh();
    const channel = new FakeChannel();
    mesh.onTransferProgress = vi.fn();
    mesh.setupDataChannel("host", channel);
  ...
  test("normalizes polling timeouts and distinguishes closing channels", async () => {
    vi.useFakeTimers();
    const mesh = makeMesh();
    mesh.channels.set("closing", new FakeChannel("closing"));
  ...
  test("does not process stale connection or queued-signal work", async () => {
    ...
    let release;
    mesh.signalPromises.set( "queued", new Promise((resolve) => { release = resolve; })
    );
  ...
  test("rejects arrays before peer creation and has no phantom ICE replay", async () => {
    ...
    expect( await mesh.accept("answer", { description: { type: "answer", sdp: "answer" } })
    ).toBe(true);
  ...
  test("does not set a local answer after answer creation becomes stale", async () => {
    ...
    globalThis.RTCPeerConnection = class DeferredAnswerPeer extends FakePeer {
      constructor(configuration) {
        super(configuration);
        this.createAnswer = vi.fn( () => new Promise((resolve) => { release = resolve; })
        );
      }
    };
    const mesh = makeMesh();
  ...
  test("tolerates optional channel methods and timer owners", () => {
    vi.useFakeTimers();
    const mesh = makeMesh();
    mesh.channels.set("guest", { readyState: "open" });
    expect(() => mesh.setupDataChannel("guest", new FakeChannel())
    ).not.toThrow();
    const timer = mesh.createIncomingTransferTimer("missing", "t");
    expect(() => vi.advanceTimersByTime(30_000)).not.toThrow();
    clearTimeout(timer);
    mesh.incomingFiles.set("orphan", { metadata: { transferId: "t" }, timer: 0 });
    const orphan = mesh.createIncomingTransferTimer("orphan", "t");
    expect(() => vi.advanceTimersByTime(30_000)).not.toThrow();
    clearTimeout(orphan);
  });
  ...
  test("uses exact generated transfer ids and accepts the maximum file size", async () => {
    ...
    channel.onmessage({ data: JSON.stringify({ type: "file-complete", transferId: "exact-uuid" }) });
    await sending;
    ...
    maximumMesh.waitForDataChannel = vi.fn().mockResolvedValue(closed);
    await expect(maximumMesh.sendFile("guest", maximum)).rejects.toThrow(
      "Канал передачи песни закрыт"
    );
  });
  ...
  test("emits receive progress only when its integer percent changes", () => {
    ...
    mesh.onTransferProgress = vi.fn();
    mesh.setupDataChannel("host", channel);
    channel.onmessage({ data: JSON.stringify({ type: "file-start", transferId: "t", size: 1_000 }) });
    channel.onmessage({ data: new Uint8Array([1]) });
  ...
  test("validates the Blob runtime and truncates outbound MIME types", async () => {
    ...
    await expect(makeMesh().sendFile("guest", {})).rejects.toThrow(
      "Для передачи нужны участник и файл"
    );
    ...
    expect( channel.send.mock.calls.filter(([value]) => value instanceof ArrayBuffer)
    ).toHaveLength(0);
    channel.onmessage({
  ...
  test.each(["lifecycle", "channel"])(
    "stops before reading a chunk when %s becomes invalid",
    async (invariant) => {
      const mesh = makeMesh();
      const channel = new FakeChannel();
      ...
      await expect( mesh.sendFile("guest", new RestoringBlob(["x"]))
      ).rejects.toThrow("Передача файла отменена");
      expect( channel.send.mock.calls.some(([value]) => value instanceof ArrayBuffer)
      ).toBe(false);
    }
  );
  ...
  test("ignores unknown control types and clears a completed transfer timer", () => {
    ...
    expect(() => channel.onmessage({ data: JSON.stringify({ type: "__proto__" }) })
    ).not.toThrow();
    expect(channel.send).not.toHaveBeenCalled();
    expect(mesh.incomingFiles.size).toBe(0);

    channel.onmessage({ data: JSON.stringify({ type: "file-start", transferId: "t", size: 0 }) });
    const timer = mesh.incomingFiles.get("host").timer;
    const clear = vi.spyOn(globalThis, "clearTimeout");
    channel.onmessage({ data: JSON.stringify({ type: "file-end", transferId: "t" }) });
  ...
  test.each(["lifecycle", "channel"])(
    "stops after reading a chunk when %s becomes invalid",
    async (invariant) => {
      const mesh = makeMesh();
      const channel = new FakeChannel();
      const initialVersion = mesh.lifecycleVersion;
  ...
  test.each(["lifecycle", "channel"])(
    "stops inside backpressure when %s becomes invalid",
    async (invariant) => {
      const mesh = makeMesh();
      const channel = new FakeChannel();
      const initialVersion = mesh.lifecycleVersion;
  ...
  test("deduplicates concurrent microphone startup and cancels stale capture", async () => {
    ...
    Object.defineProperty(globalThis, "navigator", {
      ...
      value: {
        mediaDevices: {
          getUserMedia: vi.fn( () => new Promise((resolve) => { resolveCapture = resolve; })
          )
        }
      }
    });
  ...
  test("optimizes audio senders without rejecting unsupported senders", async () => {
    ...
    await expect( mesh.optimizeAudioSenders({ getSenders: () => [configured, rejected, {}] })
    ).resolves.toBeUndefined();
    expect( configured.setParameters.mock.calls[0][0].encodings[0]
    ).toMatchObject({ maxBitrate: 256_000, networkPriority: "high" });
  });
  ...
  test("queues ICE candidates and answers offers in arrival order", async () => {
    ...
    expect( await mesh.accept("guest", { description: { type: "offer", sdp: "x" } })
    ).toBe(true);
    const peer = FakePeer.instances[0];
    ...
    expect( await mesh.accept("guest", { description: { type: "answer" } })
    ).toBe(true);
    expect(await mesh.accept("guest", {})).toBe(false);
  });
  ...
  test("receives files, reports import progress and confirms completion", async () => {
    ...
    await vi.waitFor(() =>
      expect( progress.mock.calls.some(([event]) => event.stage === "complete")
      ).toBe(true)
    );
  ...
  test("rejects malformed, incomplete and failed incoming transfers", async () => {
    ...
    channel.onmessage({ data: JSON.stringify({ type: "file-start", transferId: "short", size: 2 }) });
    channel.onmessage({ data: new Uint8Array([1]) });
    ...
    mesh.onFile = vi.fn(() => { throw new Error("synchronous import failure"); });
    channel.onmessage({
  ...
  test("waits for channels and reports closed, cancelled and timed out states", async () => {
    ...
    await expect(mesh.waitForDataChannel("open")).resolves.toBe( mesh.channels.get("open")
    );
    ...
    await expect(mesh.waitForDataChannel("missing", 0)).rejects.toThrow(
      "Канал передачи песни не готов"
    );
  ...
  test("cancels stale invites and direct ICE work", async () => {
    ...
    globalThis.RTCPeerConnection = class DeferredPeer extends FakePeer {
      constructor(configuration) {
        super(configuration);
        this.createOffer = vi.fn( () => new Promise((resolve) => { resolveOffer = resolve; })
        );
      }
    };
    const invitation = mesh.invite("guest");
  ...
  test("rejects excessive ICE candidates and invalid transfer messages", async () => {
    ...
    await expect( mesh.accept("guest", { candidate: "overflow" })
    ).rejects.toThrow("Получено слишком много ICE-кандидатов");
    expect(mesh.peers.has("guest")).toBe(false);
  ...
  test("cancels during backpressure and after asynchronously reading a chunk", async () => {
    ...
    await expect( nextMesh.sendFile("guest", new ClosingBlob(["x"]))
    ).rejects.toThrow("Передача файла отменена");
  });
  ...
  test("surfaces current invitation failures and rejects missing local offers", async () => {
    ...
    let rejectOffer;
    globalThis.RTCPeerConnection = class StaleFailingOfferPeer extends (
      FakePeer
    ) {
      constructor(configuration) {
        super(configuration);
        this.createOffer = vi.fn( () => new Promise((_resolve, reject) => { rejectOffer = reject; })
        );
      }
    };
  ...
  test("cancels from inside a backpressured transfer loop", async () => {
    ...
    const channel = new FakeChannel();
    channel.bufferedAmount = 600 * 1024;
    channel.send.mockImplementationOnce(() => { mesh.lifecycleVersion += 1; });
    mesh.setupDataChannel("guest", channel);
    await expect(mesh.sendFile("guest", new Blob(["x"]))).rejects.toThrow( "Передача файла отменена"
    );
  });
  ...
  test("cancels queued and direct ICE signals for replaced peers", async () => {
    const mesh = makeMesh();
    let releasePrevious;
    mesh.signalPromises.set( "queued", new Promise((resolve) => { releasePrevious = resolve; })
    );
    const queued = mesh.accept("queued", { candidate: "ice" });
  ...
  test("cancels description, candidate replay and answer work for replaced peers", async () => {
    ...
    globalThis.RTCPeerConnection = class DeferredIcePeer extends FakePeer {
      constructor(configuration) {
        super(configuration);
        this.addIceCandidate = vi.fn( () => new Promise((resolve) => { resolveIce = resolve; })
        );
      }
    };
    mesh = makeMesh();
    ...
    globalThis.RTCPeerConnection = class DeferredAnswerPeer extends FakePeer {
      constructor(configuration) {
        super(configuration);
        this.createAnswer = vi.fn( () => new Promise((resolve) => { resolveAnswer = resolve; })
        );
      }
    };
    mesh = makeMesh();
    ...
    globalThis.RTCPeerConnection = class MissingLocalAnswerPeer extends (
      FakePeer
    ) {
      constructor(configuration) {
        super(configuration);
