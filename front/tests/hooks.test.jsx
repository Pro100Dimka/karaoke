...
describe("navigation and karaoke hooks", () => {
  test("changes settings navigation", () => {
    ...
    expect(defaults.result.current).toMatchObject({
      tab: "audio",
      service: null
    });
    defaults.unmount();
  ...
  test("auto-hides and reveals karaoke controls", () => {
    ...
    expect(result.current.controlsVisible).toBe(true);
    act(() => {
      vi.setSystemTime(4399);
      checkVisibility();
    });
    expect(result.current.controlsVisible).toBe(true);
    act(() => {
      vi.setSystemTime(4400);
      checkVisibility();
    });
    expect(result.current.controlsVisible).toBe(false);
  ...
  test("dispatches scoped karaoke hotkeys", () => {
    ...
    expect(toggle).toHaveBeenCalledOnce();
    nextScope.dispatchEvent(
      new KeyboardEvent("keydown", { code: "Space", bubbles: true })
    );
    expect(toggle).toHaveBeenCalledTimes(2);

    const keydownRegistration = addEvent.mock.calls.find(
      ([event]) => event === "keydown"
    );
  ...
  test("dispatches every karaoke command with optional callbacks", () => {
    ...
    for (const action of [
      ...
      expect(() =>
        dispatchKaraokeHotkey(action, { currentTime: 0, duration: 0 })
      ).not.toThrow();
    }
  });
  ...
  test("loads, rejects and resets karaoke results safely", async () => {
    ...
    apiMocks.getResult.mockImplementation((id) =>
      id === "one" ? success : failed
    );
    const { result, rerender, unmount } = renderHook(
  ...
  test("starts karaoke result state from the exact idle contract", async () => {
    ...
    const initial = render(
      <Probe song={{ id: "initial", status: "done", updated_at: 0 }} />
    );
  ...
  test("ignores karaoke result completion after cancellation", async () => {
    ...
    await act(async () => Promise.resolve());
    expect(rejected.result.current).toEqual({
      result: null,
      loading: false,
      error: null
    });
  ...
  test("skips stage synchronization when any required node is missing", () => {
    ...
    expect(() =>
      renderHook(() => useKaraokeStageLayout({ current: stageWithoutShell }))
    ).not.toThrow();
    ...
    expect(() =>
      renderHook(() => useKaraokeStageLayout({ current: detachedStage }))
    ).not.toThrow();
