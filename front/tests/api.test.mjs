...
describe("API transport", () => {
  ...
  test("reports HTTP, malformed JSON and missing fetch failures", async () => {
    ...
    await assert.rejects(
      request("bad"),
      (error) => error.message === "bad" && error.status === 422
    );
    globalThis.fetch.mockResolvedValueOnce(
      response({
        ok: false,
...
describe("API domains", () => {
  test("routes audio, player and system operations", async () => {
    const calls = [
      audioApi.listAudioDevices(),
      audioApi.listAudioOutputDevices(),
      audioApi.listAsioDrivers(),
      audioApi.getAudioSettings(),
      audioApi.updateAudioSettings({ latency: 1 }),
      audioApi.startDirectMonitoring(),
      audioApi.stopDirectMonitoring(),
      audioApi.getSignalQuality(),
      playerApi.getSync("a/b"),
      playerApi.getTimeline("a"),
      playerApi.getPosition("a"),
      playerApi.seek("a", 2),
      playerApi.play("a"),
      playerApi.pause("a"),
      playerApi.stop("a"),
      systemApi.getCacheSize(),
      systemApi.getFreeSpace(),
      systemApi.clearCache(),
      systemApi.deleteTemp(),
      systemApi.optimizeSong("a"),
      systemApi.getHealth(),
      systemApi.getPipelineHealth(),
      systemApi.getAiModelsStatus(),
      systemApi.downloadAiModels(),
      systemApi.getVersions(),
      systemApi.getErrors(),
      systemApi.getHistory(),
      systemApi.getAbout()
    ];
  ...
  test("routes model and recording operations and normalizes collections", async () => {
    ...
    assert.deepEqual(await modelsApi.listWhisperModels(), []);
    for (const call of [
      modelsApi.downloadModel("x"),
      modelsApi.deleteModel("x"),
      modelsApi.selectModel("x")
    ])
      await call;
