...
for (const file of jsxFiles) {
  ...
  const checks = [
    [
      "native buttons declare type",
      () => openingTags(source, "button").filter((tag) => !/\btype=/.test(tag))
    ],
    [
      "no javascript URLs",
      () => (/href\s*=\s*["']javascript:/i.test(source) ? [true] : [])
    ],
    [
      "no empty href navigation",
      () => (/href\s*=\s*["'](?:#|)["']/.test(source) ? [true] : [])
    ],
    [
      "no duplicated aria-label",
      () =>
        openingTags(source).filter(
          (tag) => (tag.match(/\baria-label=/g) || []).length > 1
        )
    ],
    [
      "icon buttons are named",
      () =>
        [
          ...
          const text = body
            ...
            .trim();
          const dynamic =
            /<(?:span|strong|small|p|div)\b|\{(?:children|text|title|label|selected[^}]*)\}|\{[^}]*\?[^}]*:[^}]*\}/.test(
              body
            );
          return (
            !text &&
