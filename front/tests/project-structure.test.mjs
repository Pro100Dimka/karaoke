...
test("release gate executes unit verification and pins a supported Node runtime", () => {
  ...
  verify([releaseCheck, 'toContain', '["run", "verify"]'], [pkg.scripts["build:electron"], 'toContain', "release:check"], [pkg.engines?.node, 'toContain', "22.18.0"]);
  same([fs.readFileSync(path.join(root, ".nvmrc"), "utf8").trim(), "22.18.0"], [fs.readFileSync(path.join(root, ".node-version"), "utf8").trim(), "22.18.0"]);
});
