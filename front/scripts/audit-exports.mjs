...
const files = [src, path.join(root, "tests")].filter(fs.existsSync).flatMap(walk);
...
for (const [file, content] of contents) {
  for (const match of content.matchAll(exportPattern)) {
    const name = match[1];
    const occurrences = allSource.match(new RegExp(`\\b${name}\\b`, "g"))?.length ?? 0;
    if (occurrences <= 1) candidates.push({ name, file: path.relative(root, file) });
  }
}
