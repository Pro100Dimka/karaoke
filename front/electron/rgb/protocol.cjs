...
function parseController(body, id) {
  ...
  const take = (size) => {
    ...
    const result = body.subarray(offset, offset + size);
    offset += size;
    return result;
  };
  ...
  const colorCount = u16(),
    colors = Buffer.from(take(colorCount * 4));
  if (offset !== body.length || colorCount > 4096 || !modes[activeMode])
    throw new Error("Invalid RGB controller layout");
  return { id, type, name, modes, activeMode, colors, colorCount };
}
function colorBody(colors) {
  const header = Buffer.alloc(6);
  header.writeUInt32LE(6 + colors.length);
  header.writeUInt16LE(colors.length / 4, 4);
  return Buffer.concat([header, colors]);
}
function modeBody(index, raw) {
  return Buffer.concat([uint32(8 + raw.length), uint32(index), raw]);
}
