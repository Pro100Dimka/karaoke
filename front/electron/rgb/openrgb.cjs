...
const { VERSION, MAX_PACKET, uint32, packet, parseController, colorBody, modeBody } = require("./protocol.cjs");

class OpenRgb {
  constructor(connect = net.createConnection) {
    this.connect = connect; this.devices = []; this.socket = null; this.pending = null;
    this.active = false; this.state = "disconnected";
  }
  async start() {
    ...
    const fail = (error) => {
      ...
      this.pending?.reject(error); this.pending = null; socket.destroy();
    };
    ...
    socket.on("data", (chunk) => {
      try {
        ...
        while (buffer.length >= 16) {
          ...
          const id = buffer.readUInt32LE(8), device = buffer.readUInt32LE(4), body = buffer.subarray(16, 16 + size);
          ...
          if (this.pending?.id === id && this.pending.device === device) {
            const pending = this.pending; this.pending = null; pending.resolve(body);
          }
        }
    ...
    try {
      await new Promise((resolve, reject) => {
        const timeout = setTimeout(() => { reject(new Error("RGB connection timeout")); socket.destroy(); }, 1200);
        socket.once("connect", () => { clearTimeout(timeout); resolve(); });
        socket.once("error", (error) => { clearTimeout(timeout); reject(error); });
        socket.once("close", () => { clearTimeout(timeout); reject(new Error("RGB closed")); });
      });
      const version = await this.request(40, 0, uint32(VERSION));
      if (version.length !== 4 || version.readUInt32LE() < VERSION) throw new Error("Unsupported OpenRGB version");
      this.send(50, 0, Buffer.from("A&D Voice\0"));
      const count = await this.request(0);
      if (count.length !== 4 || count.readUInt32LE() > 128) throw new Error("Invalid RGB device count");
      this.devices = [];
      for (let id = 0; id < count.readUInt32LE(); id++) {
        const device = parseController(await this.request(1, id, uint32(VERSION)), id);
        if (device.type === 5 && device.colorCount && device.modes.some((m) => m.name.toLowerCase() === "direct")) this.devices.push(device);
      }
      this.state = this.devices.length ? "ready" : "no_devices";
      return this.devices.map(({ name }) => name);
    } catch (error) { fail(error); throw error; }
  }
  request(id, device = 0, body) {
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => { this.pending = null; reject(new Error("RGB request timeout")); this.socket.destroy(); }, 1500);
      this.pending = { id, device, resolve: (value) => { clearTimeout(timeout); resolve(value); }, reject: (error) => { clearTimeout(timeout); reject(error); } };
  ...
  frame(rgb) {
    ...
    for (const d of this.devices) {
      ...
      for (let i = 0; i < colors.length; i += 4) { colors[i] = rgb[0]; colors[i + 1] = rgb[1]; colors[i + 2] = rgb[2]; }
