...
Ведущий является единственным владельцем комнаты и не передаёт эту роль другому участнику. При сетевом обрыве комната сохраняется 45 секунд для восстановления ведущего с тем же host token. Явный выход ведущего закрывает комнату сразу, а истечение этих 45 секунд закрывает её с причиной `host-timeout`; во всех случаях оставшиеся клиенты получают `room-closed`, останавливают общее воспроизведение и возвращаются из комнаты с однозначным сообщением.
